/* Canei Subirats — Catalan column of the translation dictionary (ES → CA).
 *
 * Kept in its own file rather than as a third element inside i18n-dict.js's
 * pairs, for one reason: this is the column most likely to need correcting by
 * somebody who is not editing the application. A native speaker can review
 * and fix this file end to end without ever opening the ES/EN spine, and a
 * diff on it says exactly what changed about the Catalan and nothing else.
 *
 * The key is the SPANISH string, because Spanish is the language the
 * application is written in and therefore the only form every entry is
 * guaranteed to have (see the hub note in i18n.js).
 *
 * Completeness is enforced: tests/i18n/coverage.mjs fails when an entry here
 * is missing, empty or orphaned. Where the Catalan is genuinely identical to
 * the Spanish — which happens often, and more often in short technical
 * labels — it is written out in full rather than omitted, so the file reads
 * as a decision taken rather than a line forgotten.
 */
window.CANEI_DICT = window.CANEI_DICT || {};
window.CANEI_DICT.ca = {
  "Trabajador de obra": "Treballador d'obra",
  "Apunta sus propias horas y ve sólo las suyas. Ni importes, ni el resto del ERP.":
    "Apunta les seves pròpies hores i només veu les seves. Ni imports, ni la resta de l'ERP.",
  "Esta cuenta no está enlazada con ninguna ficha de trabajador, así que no tiene horas propias. La oficina puede enlazarla poniendo el mismo correo en la ficha.":
    "Aquest compte no està enllaçat amb cap fitxa de treballador, així que no té hores pròpies. L'oficina pot enllaçar-lo posant el mateix correu a la fitxa.",
  "Apuntar horas": "Apuntar hores",
  "Esperando aprobación": "Esperant aprovació",
  Aprobada: "Aprovada",
  Esperando: "Esperant",
  Cambiar: "Canviar",
  "Aprobada — pídeselo a la oficina": "Aprovada — demana-ho a l'oficina",
  "Guardar estas horas": "Desar aquestes hores",
  "Horas guardadas": "Hores desades",
  "Cambiar las horas": "Canviar les hores",
  "Deja 0 para borrar el apunte.": "Deixa 0 per esborrar l'apunt.",
  "De ellas extra": "D'aquestes, extra",
  "Sin obra": "Sense obra",
  "Elige la obra": "Tria l'obra",
  "horas extra": "hores extra",
  "sin aprobar todavía": "sense aprovar encara",
  "A dónde han ido mis horas": "On han anat les meves hores",
  "Sin horas en el periodo.": "Sense hores en el període.",
  "Todavía no has apuntado nada para este día.": "Encara no has apuntat res per a aquest dia.",
  "Esta semana ya está aprobada. Pide a la oficina que la reabra si falta algo.":
    "Aquesta setmana ja està aprovada. Demana a l'oficina que la reobri si falta res.",
  "La obra y la partida de ayer vienen puestas — cámbialas sólo si te has movido. Si la partida no tiene subpartidas, elige «— ninguna —».":
    "L'obra i la partida d'ahir vénen posades — canvia-les només si t'has mogut. Si la partida no té subpartides, tria «— cap —».",
  "No estás asignado a ninguna obra abierta este día. Díselo a la oficina.":
    "No estàs assignat a cap obra oberta aquest dia. Digues-ho a l'oficina.",
  "Sólo ves tus horas, y nunca un importe. La oficina revisa las imputaciones y aprueba la semana.":
    "Només veus les teves hores, i mai un import. L'oficina revisa les imputacions i aprova la setmana.",
  "Aquí no se ve nada que no sea tuyo: ni las horas de otras personas, ni ningún importe.":
    "Aquí no es veu res que no sigui teu: ni les hores d'altres persones, ni cap import.",
  "Revisa el proyecto y la partida, corrige si hace falta y aprueba.":
    "Revisa el projecte i la partida, corregeix si cal i aprova.",
  "Apunte movido a otro día": "Apunt mogut a un altre dia",
  "Parte diario por trabajador, con la semana al lado; y el resumen por proyecto y partida con la conciliación del mes.":
    "El part diari per treballador amb la setmana al costat; i el resum per projecte i partida amb la conciliació del mes.",
  Mías: "Meves",
  "Mis horas": "Les meves hores",
  Correcciones: "Correccions",
  "Ant.": "Ant.",
  "Sig.": "Seg.",
  Lun: "Dl",
  Mar: "Dt",
  Jue: "Dj",
  Vie: "Dv",
  Dom: "Dg",
  "Copiar ayer": "Copiar ahir",
  "Añadir trabajador": "Afegir treballador",
  "Añadir línea": "Afegir línia",
  "Aprobar semana": "Aprovar setmana",
  "Reabrir semana": "Reobrir setmana",
  "Todos los proyectos": "Tots els projectes",
  "Mostrar sólo favoritos": "Mostrar només favorits",
  "Mostrar todos los proyectos": "Mostrar tots els projectes",
  "Este servidor todavía no tiene buzón configurado.":
    "Aquest servidor encara no té bústia configurada.",
  "Este enlace no sale de este servidor": "Aquest enllaç no surt d'aquest servidor",
  "Borrador en el buzón de la empresa": "Esborrany a la bústia de l'empresa",
  "Informe de horas": "Informe d'hores",
  "Horas totales": "Hores totals",
  "Coste total": "Cost total",
  "Coste medio por hora": "Cost mitjà per hora",
  "Horas extra": "Hores extra",
  "Horas sin aprobar": "Hores sense aprovar",
  "Coste sin aprobar": "Cost sense aprovar",
  "Obras con horas": "Obres amb hores",
  "Días con parte": "Dies amb part",
  "Horas por persona": "Hores per persona",
  "Por obra": "Per obra",
  "% horas": "% hores",
  "% coste": "% cost",
  "€/hora": "€/hora",
  "Por obra y partida": "Per obra i partida",
  "Detalle de apuntes": "Detall d'apunts",
  "| Los importes son coste de trabajador, no precio facturado.":
    "| Els imports són cost de treballador, no preu facturat.",
  "La diferencia es normal: vacaciones, bajas, oficina y el tiempo que nadie apuntó. Lo que importa es su tamaño y su tendencia, no que sea cero.":
    "La diferència és normal: vacances, baixes, oficina i el temps que ningú va apuntar. El que importa és la seva mida i la seva tendència, no que sigui zero.",
  Día: "Dia",
  "Pagado en efectivo a trabajadores": "Pagat en efectiu a treballadors",
  "Sin horas apuntadas todavía.": "Encara no hi ha hores apuntades.",
  "Este parte muestra lo apuntado. Para añadir horas usa «Añadir línea»; para corregir o aprobar, ve a Revisión.":
    "Aquest part mostra el que s'ha apuntat. Per afegir hores fes servir «Afegir línia»; per corregir o aprovar, ves a Revisió.",
  "Partida y subpartida": "Partida i subpartida",
  "todo aprobado": "tot aprovat",
  "Este parte es para apuntar lo trabajado. Aprobar la semana se hace en Revisión, cuando la oficina ya ha comprobado las imputaciones.":
    "Aquest part és per apuntar el treballat. Aprovar la setmana es fa a Revisió, quan l'oficina ja ha comprovat les imputacions.",
  "Ficha del trabajador ›": "Fitxa del treballador ›",
  enviado: "enviat",
  "Exportar a Excel": "Exportar a Excel",
  "A dónde han ido las horas": "On han anat les hores",
  "Quién ha trabajado": "Qui ha treballat",
  "horas · extra · coste": "hores · extra · cost",
  "sin horas imputadas en el periodo": "sense hores imputades en el període",
  "Comparar con lo pagado en efectivo por trabajador ›":
    "Comparar amb el pagat en efectiu per treballador ›",
  "Ocultar lo pagado en efectivo por trabajador ▴": "Amagar el pagat en efectiu per treballador ▴",
  imputado: "imputat",
  pagado: "pagat",
  diferencia: "diferència",
  Abiertas: "Obertes",
  Aprobadas: "Aprovades",
  Mostrando: "Mostrant",
  "Editar apunte": "Editar apunt",
  "Aprobada y bloqueada": "Aprovada i bloquejada",
  "Editando este apunte — cualquier campo puede cambiar":
    "Editant aquest apunt — qualsevol camp pot canviar",
  "— ninguna —": "— cap —",
  "Cambiar la fecha mueve el apunte a otro día — la lista dirá a dónde ha ido. Un apunte no puede moverse a una semana ya aprobada.":
    "Canviar la data mou l'apunt a un altre dia — la llista dirà on ha anat. Un apunt no es pot moure a una setmana ja aprovada.",
  "Apunte corregido": "Apunt corregit",
  "Apunte eliminado": "Apunt eliminat",
  "No hay horas sin aprobar": "No hi ha hores sense aprovar",
  "Aquí irán las dos pantallas propias de cada persona: apuntar las horas del día y consultar las suyas. Todavía no están construidas; mientras tanto, usa «Todos».":
    "Aquí aniran les dues pantalles pròpies de cada persona: apuntar les hores del dia i consultar les seves. Encara no estan construïdes; mentrestant, fes servir «Tots».",
  // PRY-04 · Recorrido del cliente.
  de: "de",
  "Recorrido terminado": "Recorregut acabat",
  Completada: "Completada",
  Bloqueada: "Bloquejada",
  recorrido: "recorregut",
  recorridos: "recorreguts",
  "En qué fase está cada trabajo, qué falta para la siguiente y la pantalla real de cada paso.":
    "En quina fase és cada treball, què falta per a la següent i la pantalla real de cada pas.",
  "← Todos los recorridos": "← Tots els recorreguts",
  "Ningún recorrido coincide con la búsqueda.": "Cap recorregut coincideix amb la cerca.",
  "Abrir el presupuesto completo ↗": "Obrir el pressupost complet ↗",
  "Abrir la carta Gantt ↗": "Obrir la carta Gantt ↗",
  Falta: "Falta",
  "Ir a la fase": "Anar a la fase",
  "Todavía sin obra": "Encara sense obra",
  "Envío al cliente": "Enviament al client",
  "Inicio de obra": "Inici d'obra",
  "Cierre y reseña": "Tancament i ressenya",
  "Sin oportunidad registrada": "Sense oportunitat registrada",
  "Visitas realizadas": "Visites realitzades",
  Última: "Darrera",
  "Sin visita registrada": "Sense visita registrada",
  "Sin presupuesto": "Sense pressupost",
  "Enviado y congelado": "Enviat i congelat",
  "Todavía sin enviar al cliente": "Encara sense enviar al client",
  "Versión aceptada": "Versió acceptada",
  "Sin contrato formal": "Sense contracte formal",
  "Pendiente de firma": "Pendent de signatura",
  "Obra creada, sin iniciar": "Obra creada, sense iniciar",
  "Sin obra abierta": "Sense obra oberta",
  "Coste comprometido": "Cost compromès",
  "Sin pedidos de compra": "Sense comandes de compra",
  "Sin obra en ejecución": "Sense obra en execució",
  "Sin facturas emitidas": "Sense factures emeses",
  "Todo cobrado": "Tot cobrat",
  "Nada que cobrar todavía": "Res a cobrar encara",
  "Todo pagado": "Tot pagat",
  "Sin facturas de proveedor": "Sense factures de proveïdor",
  "Sin reseña": "Sense ressenya",
  "Registrar la visita": "Registrar la visita",
  "＋ Nuevo presupuesto": "＋ Nou pressupost",
  "Enviar y congelar": "Enviar i congelar",
  "Volver a enviar": "Tornar a enviar",
  "Registrar la respuesta": "Registrar la resposta",
  "Iniciar la obra": "Iniciar l'obra",
  "Abrir la obra": "Obrir l'obra",
  "＋ Nueva compra": "＋ Nova compra",
  "＋ Nuevo proveedor": "＋ Nou proveïdor",
  "Registrar reseña": "Registrar ressenya",
  "Cerrar la obra": "Tancar l'obra",
  Valoración: "Valoració",
  "Obra iniciada": "Obra iniciada",
  "Obra creada": "Obra creada",
  "Obra cerrada": "Obra tancada",
  "Reseña registrada": "Ressenya registrada",
  // «Lo hecho hasta ahora» — el registro de lo ya hecho y de sus documentos.
  "Lo hecho hasta ahora": "El que s'ha fet fins ara",
  anotaciones: "anotacions",
  "Todavía no se ha hecho nada en este recorrido": "Encara no s'ha fet res en aquest recorregut",
  "Oportunidad abierta": "Oportunitat oberta",
  "Visita programada": "Visita programada",
  "Presupuesto creado": "Pressupost creat",
  "Enviado al cliente": "Enviat al client",
  "Adicional enviado y congelado": "Addicional enviat i congelat",
  "Aceptado por el cliente": "Acceptat pel client",
  "Rechazado por el cliente": "Rebutjat pel client",
  "Contrato redactado": "Contracte redactat",
  "Pedido de compra": "Comanda de compra",
  "Material recibido": "Material rebut",
  "Factura de proveedor": "Factura de proveïdor",
  "Factura emitida": "Factura emesa",
  "Rectificativa emitida": "Rectificativa emesa",
  "Cobro registrado": "Cobrament registrat",
  "Pago a proveedor": "Pagament a proveïdor",
  "Recibo entregado": "Rebut lliurat",
  "Incidencia en obra": "Incidència a l'obra",
  "Entrega en obra": "Lliurament a l'obra",
  "Parte de obra": "Part d'obra",
  "Reseña del cliente": "Ressenya del client",
  "Ver el documento": "Veure el document",
  "En papel": "En paper",
  "Firma digital": "Signatura digital",
  "En mano": "En mà",
  "Un adicional se une a su obra, no abre otra":
    "Un addicional s'uneix a la seva obra, no n'obre una altra",
  "Antes hace falta una oportunidad": "Abans cal una oportunitat",
  "Antes hace falta un presupuesto": "Abans cal un pressupost",
  "Antes hace falta una obra": "Abans cal una obra",
  "El presupuesto todavía no se ha enviado": "El pressupost encara no s'ha enviat",
  "Hace falta un presupuesto aceptado": "Cal un pressupost acceptat",
  "El contrato no está firmado": "El contracte no està signat",
  "⚠ Antes hace falta una oportunidad": "⚠ Abans cal una oportunitat",
  "⚠ Antes hace falta un presupuesto": "⚠ Abans cal un pressupost",
  "⚠ Antes hace falta una obra": "⚠ Abans cal una obra",
  "⚠ El presupuesto todavía no se ha enviado": "⚠ El pressupost encara no s'ha enviat",
  "Elige una obra: venta, coste y margen partida a partida, y el coste que ha llegado sin partida asignada.":
    "Tria una obra: venda, cost i marge partida a partida, i el cost que ha arribat sense partida assignata.",
  "Busca por número o cliente, o crea uno nuevo desde una visita realizada o un lead abierto.":
    "Cerca per número o client, o crea'n un de nou des d'una visita realitzada o un lead obert.",
  "Buscar por número o cliente…": "Cerca per número o client…",
  "Canei Subirats — Plataforma de gestión": "Canei Subirats — Plataforma de gestió",
  "Plataforma de gestión": "Plataforma de gestió",
  "Abrir torre de control →": "Obrir torre de control →",
  "Proyectos activos": "Projectes actius",
  "obras en curso": "obres en curs",
  "Pendiente de cobro": "Pendent de cobrament",
  "facturas emitidas": "factures emeses",
  "Caja y bancos": "Caixa i bancs",
  "posición actual": "posició actual",
  Alertas: "Alertes",
  "requieren atención": "requereixen atenció",
  "Áreas de gestión": "Àrees de gestió",
  "un único entorno — nada se escribe dos veces": "un únic entorn — res no s'escriu dues vegades",
  "Torre de control": "Torre de control",
  "Avance físico": "Avanç físic",
  "Avance económico": "Avanç econòmic",
  Adicionales: "Addicionals",
  "Compras y pedidos": "Compres i comandes",
  "Facturas de proveedores": "Factures de proveïdors",
  "Consolidación bancaria": "Consolidació bancària",
  "Caja chica": "Caixa petita",
  "Reporte a gestoría": "Informe a la gestoria",
  "Personal interno": "Personal intern",
  "Partidas y subpartidas": "Partides i subpartides",
  "ENTORNO DE PRUEBAS · los datos aquí son inventados · TEST ENVIRONMENT":
    "ENTORN DE PROVES · les dades d'aquí són inventades · TEST ENVIRONMENT",
  Títulos: "Títols",
  "NIF / CIF / NIE / Pasaporte": "NIF / CIF / NIE / Passaport",
  "IVA intracomunitario": "IVA intracomunitari",
  "Pasaporte o documento extranjero — no es un NIF español":
    "Passaport o document estranger — no és un NIF espanyol",
  "DNI/NIF: la letra de control no cuadra": "DNI/NIF: la lletra de control no quadra",
  "No parece un número de documento": "No sembla un número de document",
  "12345678Z · B12345674 · pasaporte": "12345678Z · B12345674 · passaport",
  "Sin un documento no se puede facturar ni contratar — la ficha existe igual y se completa después":
    "Sense un document no es pot facturar ni contractar — la fitxa existeix igualment i es completa després",
  "Sin un documento no se puede facturar ni contratar — la ficha se crea igual y se completa después":
    "Sense un document no es pot facturar ni contractar — la fitxa es crea igualment i es completa després",
  "Un título agrupa partidas en el presupuesto. Al elegirlo se añade la primera partida debajo.":
    "Un títol agrupa partides al pressupost. En triar-lo s'afegeix la primera partida a sota.",
  "ya está en este presupuesto": "ja hi és en aquest pressupost",
  "＋ Nuevo título…": "＋ Nou títol…",
  "se añade al catálogo": "s'afegeix al catàleg",
  "Elementos de presupuesto": "Elements de pressupost",
  "Partidas en las que aparece": "Partides en què apareix",
  "Subpartidas de esta partida": "Subpartides d'aquesta partida",
  Posición: "Posició",
  título: "títol",
  títulos: "títols",
  "Todavía no hay partidas. Crea la subpartida y archívala luego desde la sección Partidas.":
    "Encara no hi ha partides. Crea la subpartida i arxiva-la després des de la secció Partides.",
  "Todavía no hay subpartidas. Créalas en la sección Subpartidas y vuelve a archivarlas aquí.":
    "Encara no hi ha subpartides. Crea-les a la secció Subpartides i torna a arxivar-les aquí.",
  "Nuevo título": "Nou títol",
  "＋ Nuevo título": "＋ Nou títol",
  "＋ Nueva partida": "＋ Nova partida",
  "Se genera solo a partir del nombre.": "Es genera sol a partir del nom.",
  "Importar el libro de precios": "Importar el llibre de preus",
  "Libro de precios importado": "Llibre de preus importat",
  "Presupuesto importado": "Pressupost importat",
  "Subir un presupuesto": "Pujar un pressupost",
  "sin completar": "sense completar",
  "Partidas del presupuesto": "Partides del pressupost",
  "Elegir archivo": "Triar fitxer",
  "No se pudo leer": "No s'ha pogut llegir",
  "No se pudo cargar": "No s'ha pogut carregar",
  "Recargue la página para actualizar la aplicación.":
    "Recarregueu la pàgina per actualitzar l'aplicació.",
  "Se actualizarán": "S'actualitzaran",
  "Se crearán": "Es crearan",
  "valor actual → valor del archivo": "valor actual → valor del fitxer",
  "Filas que no se pueden importar": "Files que no es poden importar",
  "Lo que cambia": "Què canvia",
  "⬆ Excel": "⬆ Excel",
  "⬆ Importar Excel": "⬆ Importar Excel",
  "⬇ Descargar plantilla": "⬇ Descarregar plantilla",
  "Subir un presupuesto en Excel: crea lo que falte en el libro de precios y redacta esta versión":
    "Pujar un pressupost en Excel: crea el que falti al llibre de preus i redacta aquesta versió",
  "Una sola hoja para los tres niveles. Una fila con sólo el título crea un título; con sólo la partida, una partida. La partida se repite sola: déjala en blanco y la subpartida entra en la partida de arriba.":
    "Un sol full per als tres nivells. Una fila amb només el títol crea un títol; amb només la partida, una partida. La partida es repeteix sola: deixa-la en blanc i la subpartida entra a la partida de dalt.",
  "Una fila por línea. El título y la partida se repiten solos: déjalos en blanco y la fila sigue en la misma partida. Las subpartidas que no estén en el libro de precios se crean al importar.":
    "Una fila per línia. El títol i la partida es repeteixen sols: deixa'ls en blanc i la fila segueix a la mateixa partida. Les subpartides que no siguin al llibre de preus es creen en importar.",
  " sin precio o sin coste": " sense preu o sense cost",
  "Correlativo automático.": "Correlatiu automàtic.",
  "Se archivan desde la subpartida. Pulse una para abrirla.":
    "S'arxiven des de la subpartida. Premi'n una per obrir-la.",
  "Ninguna subpartida está archivada en esta partida. Se archivan desde la subpartida, en la sección Subpartidas.":
    "Cap subpartida no està arxivada en aquesta partida. S'arxiven des de la subpartida, a la secció Subpartides.",
  "Se genera solo y no se teclea; una vez guardado, los registros lo conservan.":
    "Es genera sol i no es tecleja; un cop desat, els registres el conserven.",
  "✎ Modificar título": "✎ Modificar títol",
  "Eliminar título": "Eliminar títol",
  "Buscar título…": "Cercar títol…",
  "Buscar por código o nombre": "Cercar per codi o nom",
  "Todas las partidas": "Totes les partides",
  "Sin título": "Sense títol",
  "Nombre (ES) *": "Nom (ES) *",
  "Partidas de este título": "Partides d'aquest títol",
  "Títulos en los que aparece": "Títols on apareix",
  "No agrupa ninguna partida todavía.": "Encara no agrupa cap partida.",
  "Título creado": "Títol creat",
  "Título actualizado": "Títol actualitzat",
  "Título eliminado": "Títol eliminat",
  "Orden de títulos actualizado": "Ordre de títols actualitzat",
  "— sin título —": "— sense títol —",
  "Título bajo el que se agrupa esta partida": "Títol sota el qual s'agrupa aquesta partida",
  "Partida agrupada": "Partida agrupada",
  "Partida sin título": "Partida sense títol",
  "Partida creada": "Partida creada",
  "Partida actualizada": "Partida actualitzada",
  "Partida retirada": "Partida retirada",
  "Partida reactivada": "Partida reactivada",
  "· retirada": "· retirada",
  "· retirado": "· retirat",
  "Todavía no hay títulos. Créalos con «＋ Nuevo título».":
    "Encara no hi ha títols. Crea'ls amb «＋ Nou títol».",
  "Todavía no hay títulos. Crea el primero y arrastra las partidas que lo forman.":
    "Encara no hi ha títols. Crea el primer i arrossega-hi les partides que el formen.",
  "Fuentes de leads": "Fonts de leads",
  "Formas de pago": "Formes de pagament",
  Usuarios: "Usuaris",
  Visitas: "Visites",
  Comercial: "Comercial",
  Configuración: "Configuració",
  "Datos maestros": "Dades mestres",
  Maestros: "Mestres",
  "Config.": "Config.",
  "Admin.": "Admin.",
  Torre: "Torre",
  "En preparación": "En preparació",
  "Necesita atención": "Necessita atenció",
  "Margen acumulado": "Marge acumulat",
  "Pendiente de pago": "Pendent de pagament",
  Tesorería: "Tresoreria",
  "Nada vencido": "Res no vençut",
  Avance: "Avanç",
  Programación: "Programació",
  "Bandeja de captura": "Safata de captura",
  "Facturas registradas": "Factures registrades",
  "Cuentas y saldos": "Comptes i saldos",
  Conciliación: "Conciliació",
  "Toda la empresa de un vistazo: obras, avance, margen, facturado, cobrado, pagos, caja y alertas — cada indicador abre su detalle.":
    "Tota l'empresa d'un cop d'ull: obres, avanç, marge, facturat, cobrat, pagaments, caixa i alertes — cada indicador obre el seu detall.",
  "Indicadores en vivo": "Indicadors en viu",
  Clientes: "Clients",
  "Registro único de terceros con datos fiscales validados.":
    "Registre únic de tercers amb dades fiscals validades.",
  Proyectos: "Projectes",
  "Línea base congelada, avance por partida, coste real y extras.":
    "Línia base congelada, avanç per partida, cost real i extres.",
  Presupuestos: "Pressupostos",
  "Partidas, subpartidas y sub-mediciones con control de versiones.":
    "Partides, subpartides i subamidaments amb control de versions.",
  Proveedores: "Proveïdors",
  "Industriales y autónomos con IRPF correcto y vencimientos.":
    "Industrials i autònoms amb IRPF correcte i venciments.",
  "Precios y costes": "Preus i costos",
  "Base de precios por proveedor con origen, fecha y comparativa.":
    "Base de preus per proveïdor amb origen, data i comparativa.",
  "Facturación y cobros": "Facturació i cobraments",
  "Facturas sin huecos, recibos, cobros parciales y reclamación.":
    "Factures sense buits, rebuts, cobraments parcials i reclamació.",
  "Pagos, banco y caja": "Pagaments, banc i caixa",
  "Asigna un movimiento a su obra tecleando el nº de proyecto.":
    "Assigna un moviment a la seva obra escrivint el núm. de projecte.",
  Contratos: "Contractes",
  "Hitos de pago, plazos, penalizaciones y garantías como datos.":
    "Fites de pagament, terminis, penalitzacions i garanties com a dades.",
  "Captura de documentos": "Captura de documents",
  "Fotografía una factura o ticket y asígnala a su proyecto.":
    "Fotografia una factura o tiquet i assigna'l al seu projecte.",
  Gestoría: "Gestoria",
  "Paquete trimestral, IVA e IRPF conciliados y excepciones.":
    "Paquet trimestral, IVA i IRPF conciliats i excepcions.",
  "Mi día": "El meu dia",
  "la vista corta de cada perfil": "la vista curta de cada perfil",
  "🔨 Hoy en obra": "🔨 Avui a l'obra",
  "Visitas, hitos, extras por valorar y horas por registrar.":
    "Visites, fites, extres per valorar i hores per registrar.",
  "🗂️ Hoy en oficina": "🗂️ Avui a l'oficina",
  "Presupuestos, facturas, cobros, pagos y documentos pendientes.":
    "Pressupostos, factures, cobraments, pagaments i documents pendents.",
  "Herramientas y guías": "Eines i guies",
  "análisis, simulación y documentación": "anàlisi, simulació i documentació",
  "Zona de clientes": "Zona de clients",
  "Altas, ediciones y ficha completa del cliente (registro compartido).":
    "Altes, edicions i fitxa completa del client (registre compartit).",
  "Recorrido completo": "Recorregut complet",
  "Un proyecto de principio a fin, con documentos y correos.":
    "Un projecte de principi a fi, amb documents i correus.",
  "Panel clásico": "Tauler clàssic",
  "Cartera, correos y alertas del panel anterior.":
    "Cartera, correus i alertes del tauler anterior.",
  "Catálogo, numeración, IVA y configuración como datos.":
    "Catàleg, numeració, IVA i configuració com a dades.",
  "Datos financieros": "Dades financeres",
  "P&G, balance, tesorería y KPIs calculados.": "PiG, balanç, tresoreria i KPI calculats.",
  "Constructor de presupuestos": "Constructor de pressupostos",
  "Presupuesto por catálogo con opciones y comparativa.":
    "Pressupost per catàleg amb opcions i comparativa.",
  "Guía de operaciones": "Guia d'operacions",
  "Puesta en marcha, proceso diario y RACI, con PDF.":
    "Posada en marxa, procés diari i RACI, amb PDF.",
  "Interfaz de trabajo en castellano; los documentos del cliente se emiten en castellano (catalán en preparación). Nada sale del sistema sin revisión: los correos se redactan y se envían solo con su visto bueno.":
    "Interfície de treball en castellà; els documents del client s'emeten en castellà (català en preparació). Res no surt del sistema sense revisió: els correus es redacten i s'envien només amb el seu vistiplau.",
  "Un único entorno · numeración controlada · datos exportables":
    "Un únic entorn · numeració controlada · dades exportables",
  "todo asignado": "tot assignat",
  "al día": "al dia",
  "visitas pendientes": "visites pendents",
  "extras por valorar": "extres per valorar",
  "obras con horas por registrar": "obres amb hores per registrar",
  "tareas vencidas": "tasques vençudes",
  "presupuestos por preparar": "pressupostos per preparar",
  "cobros por reclamar": "cobraments per reclamar",
  "facturas sin asignar": "factures sense assignar",
  "documentos por validar": "documents per validar",
  "ERP · un único entorno": "ERP · un únic entorn",
  "← Inicio": "← Inici",
  "⤓ Exportar": "⤓ Exportar",
  "Volver a los datos de ejemplo": "Tornar a les dades d'exemple",
  Cerrar: "Tancar",
  Visión: "Visió",
  Obra: "Obra",
  Economía: "Economia",
  "Clientes y terceros": "Clients i tercers",
  "Oportunidades y visitas": "Oportunitats i visites",
  "Horas de obra": "Hores d'obra",
  "Proveedores y pagos": "Proveïdors i pagaments",
  "Banco y caja": "Banc i caixa",
  "Visión — Torre de control": "Visió — Torre de control",
  "Visión — Mi día": "Visió — El meu dia",
  "Comercial — Clientes y terceros": "Comercial — Clients i tercers",
  "Comercial — Oportunidades y visitas": "Comercial — Oportunitats i visites",
  "Comercial — Presupuestos": "Comercial — Pressupostos",
  "Comercial — Contratos": "Comercial — Contractes",
  "Obra — Proyectos": "Obra — Projectes",
  "Obra — Captura de documentos": "Obra — Captura de documents",
  "Obra — Horas de obra": "Obra — Hores d'obra",
  "Economía — Facturación y cobros": "Economia — Facturació i cobraments",
  "Economía — Proveedores y pagos": "Economia — Proveïdors i pagaments",
  "Economía — Banco y caja": "Economia — Banc i caixa",
  "Economía — Precios y costes": "Economia — Preus i costos",
  "Economía — Gestoría": "Economia — Gestoria",
  "ERP › Visión": "ERP › Visió",
  "ERP › Comercial": "ERP › Comercial",
  "ERP › Obra": "ERP › Obra",
  "ERP › Economía": "ERP › Economia",
  Hecho: "Fet",
  "En curso": "En curs",
  Planificadas: "Planificades",
  "Registros importados que necesitan revisión": "Registres importats que necessiten revisió",
  "ya existe en el sistema": "ja existeix al sistema",
  "datos no válidos": "dades no vàlides",
  "sin nombre": "sense nom",
  Registro: "Registre",
  Origen: "Origen",
  Motivo: "Motiu",
  "Datos más recientes que la aplicación": "Dades més recents que l'aplicació",
  Hechas: "Fetes",
  Bloqueadas: "Bloquejades",
  "Sin empezar": "Sense començar",
  "Indicadores consolidados de toda la empresa. Cada indicador y alerta abre su detalle.":
    "Indicadors consolidats de tota l'empresa. Cada indicador i alerta obre el seu detall.",
  Facturado: "Facturat",
  "reclamar según vencimientos": "reclamar segons venciments",
  "Pagos a proveedor": "Pagaments a proveïdor",
  "pendientes de pago": "pendents de pagament",
  "Presupuestos en decisión": "Pressupostos en decisió",
  "Extras sin aprobar": "Extres sense aprovar",
  "no facturables hasta aprobación": "no facturables fins a l'aprovació",
  "Documentos por revisar": "Documents per revisar",
  "captura pendiente de validar": "captura pendent de validar",
  "Proyectos — presupuesto vs. real": "Projectes — pressupost vs. real",
  "margen previsto": "marge previst",
  Proyecto: "Projecte",
  Venta: "Venda",
  "Coste real": "Cost real",
  "Margen prev.": "Marge prev.",
  "Sin alertas. Todo en orden.": "Sense alertes. Tot en ordre.",
  "La vista corta del día para cada perfil: obra y oficina.":
    "La vista curta del dia per a cada perfil: obra i oficina.",
  "perfil operativo": "perfil operatiu",
  "hitos de obra esta semana": "fites d'obra aquesta setmana",
  "perfil administración": "perfil administració",
  "seguimientos de presupuesto": "seguiments de pressupost",
  "hitos listos para facturar": "fites a punt per facturar",
  "facturas de proveedor sin asignar": "factures de proveïdor sense assignar",
  "pagos que vencen esta semana": "pagaments que vencen aquesta setmana",
  "documentos capturados por validar": "documents capturats per validar",
  Tareas: "Tasques",
  Tarea: "Tasca",
  Responsable: "Responsable",
  Vence: "Venç",
  Estado: "Estat",
  Oficina: "Oficina",
  Vencida: "Vençuda",
  Abierta: "Oberta",
  Hecha: "Feta",
  "Sin tareas.": "Sense tasques.",
  "Un único registro de terceros: clientes, proveedores, industriales y autónomos. Los datos obligatorios se validan antes de facturar o contratar.":
    "Un únic registre de tercers: clients, proveïdors, industrials i autònoms. Les dades obligatòries es validen abans de facturar o contractar.",
  "＋ Nuevo tercero": "＋ Nou tercer",
  Código: "Codi",
  Nombre: "Nom",
  "NIF/CIF": "NIF/CIF",
  Contacto: "Contacte",
  Línea: "Línia",
  Datos: "Dades",
  Cliente: "Client",
  Proveedor: "Proveïdor",
  Industrial: "Industrial",
  Autónomo: "Autònom",
  Empleado: "Empleat",
  Técnico: "Tècnic",
  Socio: "Soci",
  Otro: "Altre",
  Completo: "Complet",
  "Cada consulta crea una oportunidad con origen, estado y próxima acción. Las visitas capturan mediciones, fotos y el croquis a mano.":
    "Cada consulta crea una oportunitat amb origen, estat i propera acció. Les visites capturen amidaments, fotos i el croquis a mà.",
  "Pendiente de visita": "Pendent de visita",
  "Pendiente de presupuesto": "Pendent de pressupost",
  "Esperando al cliente": "Esperant el client",
  Ganada: "Guanyada",
  Perdida: "Perduda",
  Trabajo: "Treball",
  Días: "Dies",
  "Próxima acción": "Propera acció",
  "Sin oportunidades abiertas.": "Sense oportunitats obertes.",
  "Visitas de obra": "Visites d'obra",
  Fecha: "Data",
  Mediciones: "Amidaments",
  Fotos: "Fotos",
  Croquis: "Croquis",
  Notas: "Notes",
  "✓ adjunto": "✓ adjunt",
  Cerradas: "Tancades",
  "Presupuestos con partidas, subpartidas y sub-mediciones, opciones y pendientes separados, y control de versiones con una única versión aceptada.":
    "Pressupostos amb partides, subpartides i subamidaments, opcions i pendents separats, i control de versions amb una única versió acceptada.",
  Número: "Número",
  Versión: "Versió",
  Margen: "Marge",
  Pendientes: "Pendents",
  Validez: "Validesa",
  Aceptado: "Acceptat",
  Emitido: "Emès",
  Borrador: "Esborrany",
  "Términos clave como datos, no texto libre: hitos de pago, regla de inicio, duración, penalizaciones y garantías. Sin firma no hay obra ni primera factura.":
    "Termes clau com a dades, no text lliure: fites de pagament, regla d'inici, durada, penalitzacions i garanties. Sense signatura no hi ha obra ni primera factura.",
  Contrato: "Contracte",
  Valor: "Valor",
  "Hitos de pago": "Fites de pagament",
  Inicio: "Inici",
  Duración: "Durada",
  Garantías: "Garanties",
  Firma: "Signatura",
  Firmado: "Signat",
  "Sin firmar": "Sense signar",
  Enviado: "Enviat",
  "En vigor": "En vigor",
  Finalizado: "Finalitzat",
  Anulado: "Anul·lat",
  "Cada proyecto conserva su línea base aceptada y muestra venta, coste comprometido, coste real y margen por partida. Marca subpartidas hechas para certificar.":
    "Cada projecte conserva la seva línia base acceptada i mostra venda, cost compromès, cost real i marge per partida. Marca subpartides fetes per certificar.",
  "Venta actual": "Venda actual",
  Comprometido: "Compromès",
  Real: "Real",
  Reforma: "Reforma",
  Reparación: "Reparació",
  Humedades: "Humitats",
  Cerrado: "Tancat",
  Activo: "Actiu",
  "Fotografía o sube una factura, albarán o ticket y asígnalo a un proyecto o a gastos generales. Nada se valida sin confirmación humana.":
    "Fotografia o puja una factura, albarà o tiquet i assigna'l a un projecte o a despeses generals. Res no es valida sense confirmació humana.",
  Capturado: "Capturat",
  Extraído: "Extret",
  Validado: "Validat",
  Asignado: "Assignat",
  "Enviado a gestoría": "Enviat a la gestoria",
  Pagado: "Pagat",
  Documento: "Document",
  Tipo: "Tipus",
  Emisor: "Emissor",
  Importe: "Import",
  Asignación: "Assignació",
  "Factura proveedor": "Factura proveïdor",
  "Factura autónomo": "Factura autònom",
  Albarán: "Albarà",
  "Albarán valorado": "Albarà valorat",
  Ticket: "Tiquet",
  Oferta: "Oferta",
  Confirmación: "Confirmació",
  "Justificante pago": "Justificant pagament",
  Abono: "Abonament",
  "Por confirmar": "Per confirmar",
  "¿Duplicado?": "Duplicat?",
  "Sin documentos capturados.": "Sense documents capturats.",
  "Horas diarias por trabajador y proyecto, con tarifa histórica. El coste de mano de obra entra en el coste real de cada obra.":
    "Hores diàries per treballador i projecte, amb tarifa històrica. El cost de mà d'obra entra en el cost real de cada obra.",
  Trabajador: "Treballador",
  Partida: "Partida",
  Horas: "Hores",
  Coste: "Cost",
  "Sin asignar": "Sense assignar",
  "Total por proyecto": "Total per projecte",
  "Facturas generadas desde el presupuesto aceptado, los hitos del contrato o los extras aprobados. Numeración sin huecos; las emitidas son inmutables.":
    "Factures generades des del pressupost acceptat, les fites del contracte o els extres aprovats. Numeració sense buits; les emeses són immutables.",
  Facturas: "Factures",
  Pendiente: "Pendent",
  Cobrada: "Cobrada",
  "Registrar cobro": "Registrar cobrament",
  "Recibos emitidos": "Rebuts emesos",
  Efectivo: "Efectiu",
  Transferencia: "Transferència",
  "Sin recibos.": "Sense rebuts.",
  "Facturas de proveedor con base, IVA y retención IRPF según el perfil fiscal. Pagos parciales y un pago para varias facturas.":
    "Factures de proveïdor amb base, IVA i retenció d'IRPF segons el perfil fiscal. Pagaments parcials i un pagament per a diverses factures.",
  "Facturas de proveedor": "Factures de proveïdor",
  "Nº factura": "Núm. factura",
  Pagada: "Pagada",
  Parcial: "Parcial",
  "En disputa": "En disputa",
  Registrada: "Registrada",
  Asignada: "Assignada",
  Pagar: "Pagar",
  "Importa los movimientos y teclea el número de proyecto sobre cualquier movimiento: el coste queda asignado a esa obra sin volver a escribir nada.":
    "Importa els moviments i escriu el número de projecte sobre qualsevol moviment: el cost queda assignat a aquella obra sense tornar a escriure res.",
  caja: "caixa",
  banco: "banc",
  "Total disponible": "Total disponible",
  "bancos + caja": "bancs + caixa",
  Movimientos: "Moviments",
  "escribe el nº de proyecto para asignar (p. ej. P-2026-0001)":
    "escriu el núm. de projecte per assignar (p. ex. P-2026-0001)",
  Concepto: "Concepte",
  Clase: "Classe",
  "Coste de obra": "Cost d'obra",
  "Gasto general": "Despesa general",
  Impuestos: "Impostos",
  Nóminas: "Nòmines",
  Financiero: "Financer",
  "Traspaso interno": "Traspàs intern",
  "Cobro de cliente": "Cobrament de client",
  "Falta justificante": "Falta justificant",
  "Sin clasificar": "Sense classificar",
  "Nº proyecto": "Núm. projecte",
  "Mano de obra": "Mà d'obra",
  "Previsión de caja — próximas 6 semanas": "Previsió de caixa — properes 6 setmanes",
  Semana: "Setmana",
  "Cobros previstos": "Cobraments previstos",
  "Pagos previstos": "Pagaments previstos",
  Neto: "Net",
  "Catálogo de subpartidas y precios por proveedor con fecha y documento de origen. Un precio ausente se ve ausente — nunca cuenta como cero.":
    "Catàleg de subpartides i preus per proveïdor amb data i document d'origen. Un preu absent es veu absent — mai no compta com a zero.",
  "Comparativa de precios por subpartida": "Comparativa de preus per subpartida",
  "neto por proveedor": "net per proveïdor",
  Subpartida: "Subpartida",
  Mejor: "Millor",
  "Sin precio": "Sense preu",
  "Histórico de precios": "Històric de preus",
  "con origen y vigencia (nunca se sobrescribe)": "amb origen i vigència (mai no se sobreescriu)",
  Lista: "Llista",
  "Dto.": "Dte.",
  Vigencia: "Vigència",
  Tarifa: "Tarifa",
  "Paquetes de trabajo": "Paquets de treball",
  "rendimiento · compra mínima · envase": "rendiment · compra mínima · envàs",
  "El paquete trimestral se genera desde los datos, con resúmenes de IVA e IRPF conciliados y la lista de excepciones antes de enviar.":
    "El paquet trimestral es genera des de les dades, amb resums d'IVA i IRPF conciliats i la llista d'excepcions abans d'enviar.",
  T1: "T1",
  T2: "T2",
  T3: "T3",
  T4: "T4",
  "📦 Generar y registrar envío": "📦 Generar i registrar enviament",
  "Facturas emitidas": "Factures emeses",
  "Facturas recibidas": "Factures rebudes",
  "IVA neto (303)": "IVA net (303)",
  "IRPF retenido (111)": "IRPF retingut (111)",
  "a autónomos y profesionales": "a autònoms i professionals",
  "Lista de excepciones antes de enviar": "Llista d'excepcions abans d'enviar",
  "✓ limpio": "✓ net",
  "movimientos bancarios sin asignar": "moviments bancaris sense assignar",
  "recibos sin casar": "rebuts sense casar",
  "movimientos de caja sin justificante": "moviments de caixa sense justificant",
  "✅ Serie de facturas sin huecos": "✅ Sèrie de factures sense buits",
  "IVA por tipo": "IVA per tipus",
  "conciliado con los registros": "conciliat amb els registres",
  "Base repercutida": "Base repercutida",
  "IVA repercutido": "IVA repercutit",
  "Base soportada": "Base suportada",
  "IVA soportado": "IVA suportat",
  "Envíos registrados": "Enviaments registrats",
  "Aún no se ha enviado ningún paquete.": "Encara no s'ha enviat cap paquet.",
  Identificación: "Identificació",
  "✓ completa para facturar": "✓ completa per facturar",
  "· contable": "· comptable",
  "NIF/CIF:": "NIF/CIF:",
  "· Particular": "· Particular",
  "· Empresa": "· Empresa",
  "· Comunidad": "· Comunitat",
  "· Ente público": "· Ens públic",
  Historial: "Historial",
  "presupuestos ·": "pressupostos ·",
  "contratos ·": "contractes ·",
  proyectos: "projectes",
  "facturas ·": "factures ·",
  "recibos ·": "rebuts ·",
  "facturas recibidas": "factures rebudes",
  "Nuevo tercero": "Nou tercer",
  "Nombre o razón social *": "Nom o raó social *",
  "NIF / CIF / NIE *": "NIF / CIF / NIE *",
  Rol: "Rol",
  "Dirección de facturación *": "Adreça de facturació *",
  "C.P. *": "C.P. *",
  "Población *": "Població *",
  "Móvil *": "Mòbil *",
  "Origen del contacto *": "Origen del contacte *",
  "Línea de actividad": "Línia d'activitat",
  Reparaciones: "Reparacions",
  Otra: "Altra",
  "Guardar tercero": "Desar tercer",
  "Tercero creado": "Tercer creat",
  Versiones: "Versions",
  "automáticas, emitidas = congeladas": "automàtiques, emeses = congelades",
  Aceptada: "Acceptada",
  Emitida: "Emesa",
  "Base imponible": "Base imposable",
  "Calle y número": "Carrer i número",
  "Fijo o móvil": "Fix o mòbil",
  Municipio: "Municipi",
  "IVA (€)": "IVA (€)",
  "Total (€)": "Total (€)",
  "Guardar los datos del documento": "Desa les dades del document",
  "Quién emite el documento": "Qui emet el document",
  "NIF o CIF": "NIF o CIF",
  "Número de la factura o del ticket": "Número de la factura o del tiquet",
  "Datos del documento corregidos": "Dades del document corregides",
  "Ya está registrado como factura: estas cifras se corrigen en la factura, no aquí.":
    "Ja està registrat com a factura: aquestes xifres es corregeixen a la factura, no aquí.",
  "Lo que el lector propuso y una persona aceptó. Corríjalo aquí si el documento dice otra cosa: se vuelve a comprobar si hay un duplicado y se rehace el nombre del archivo.":
    "El que el lector va proposar i una persona va acceptar. Corregiu-ho aquí si el document diu una altra cosa: es torna a comprovar si hi ha un duplicat i es refà el nom de l'arxiu.",
  "↩ Deshacer importación": "↩ Desfer importació",
  "En un periodo cerrado": "Dins d'un període tancat",
  "Conciliado con un documento": "Conciliat amb un document",
  "Asignado a una obra": "Assignat a una obra",
  "Marcado sin factura, con motivo": "Marcat sense factura, amb motiu",
  "Con justificante adjunto": "Amb justificant adjunt",
  "Un periodo cerrado se reabre en la pestaña Conciliación, en «Periodos cerrados».":
    "Un període tancat es reobre a la pestanya Conciliació, a «Períodes tancats».",
  "Hay movimientos dentro de un periodo cerrado. Reábrelo en la pestaña Conciliación, en «Periodos cerrados», y vuelve a importar.":
    "Hi ha moviments dins d'un període tancat. Reobre'l a la pestanya Conciliació, a «Períodes tancats», i torna a importar.",
  "Deshacer importación ·": "Desfer importació ·",
  "Deshacer importación": "Desfer importació",
  Importaciones: "Importacions",
  Deshacer: "Desfer",
  "Movimientos en la cuenta": "Moviments al compte",
  "Se pueden borrar": "Es poden esborrar",
  "Se conservan": "Es conserven",
  "Sólo se borra lo que nadie ha tocado: un movimiento casado, asignado o marcado sin factura se conserva.":
    "Només s'esborra el que ningú ha tocat: un moviment conciliat, assignat o marcat sense factura es conserva.",
  "Elige primero la cuenta.": "Tria primer el compte.",
  "Importación deshecha": "Importació desfeta",
  "Movimientos borrados": "Moviments esborrats",
  Cuentas: "Comptes",
  "Tipo de hora": "Tipus d'hora",
  Extra: "Extra",
  Festivo: "Festiu",
  "Tipo de hora actualizado": "Tipus d'hora actualitzat",
  "＋ Crear caja": "＋ Crear caixa",
  "Crea la primera cuenta para empezar: banco, tarjeta o caja.":
    "Crea el primer compte per començar: banc, targeta o caixa.",
  Diferencia: "Diferència",
  doc: "doc",
  "⚠ Una obra cerrada no puede aceptar una variación":
    "⚠ Una obra tancada no pot acceptar una variació",
  "⚠ Una lista no puede quedarse sin entradas activas":
    "⚠ Una llista no pot quedar-se sense entrades actives",
  "Presupuestos de variación": "Pressupostos de variació",
  "＋ Presupuesto de variación": "＋ Pressupost de variació",
  "Presupuesto de variación creado": "Pressupost de variació creat",
  "Sin presupuestos de variación. El botón crea uno sobre esta obra; se construye y se acepta como cualquier presupuesto, y al aceptarse sus partidas entran en la economía, el avance y la certificación.":
    "Sense pressupostos de variació. El botó en crea un sobre aquesta obra; es construeix i s'accepta com qualsevol pressupost, i en acceptar-se els seus partides entren a l'economia, l'avanç i la certificació.",
  "· variación de": "· variació de",
  variación: "variació",
  "Coste acumulado": "Cost acumulat",
  "Costes ·": "Costos ·",
  "Ver las facturas de esta obra": "Veure les factures d'aquesta obra",
  "· ver facturas →": "· veure factures →",
  "sin subpartida": "sense subpartida",
  "Ningún coste imputado a esta partida todavía.": "Cap cost imputat a aquesta partida encara.",
  "Fin previsto actualizado": "Fi previst actualitzat",
  "Tarifa hora extra (€/hora)": "Tarifa hora extra (€/hora)",
  "como la normal si se deja vacío": "com la normal si es deixa buit",
  "Trabajador (si es un pago a su nombre)": "Treballador (si és un pagament al seu nom)",
  "Por trabajador": "Per treballador",
  "De ellas, extra": "D'elles, extra",
  "Pagos en efectivo por trabajador": "Pagaments en efectiu per treballador",
  "Horas del mes (coste)": "Hores del mes (cost)",
  "Pagado a su nombre": "Pagat al seu nom",
  "Sin horas ni pagos nominales este mes.": "Sense hores ni pagaments nominals aquest mes.",
  "— sin asignar todavía —": "— encara sense assignar —",
  "docs/": "docs/",
  "Motivo sin factura": "Motiu sense factura",
  "justificante(s) sin archivo detrás — el ZIP los nombra pero no puede incluirlos.":
    "justificant(s) sense arxiu al darrere — el ZIP els anomena però no els pot incloure.",
  "📎 Justificante": "📎 Justificant",
  "Justificante ·": "Justificant ·",
  "Justificante adjuntado": "Justificant adjuntat",
  "Factura del proveedor": "Factura del proveïdor",
  "El ticket o la factura simplificada detrás de este movimiento.":
    "El tiquet o la factura simplificada darrere d'aquest moviment.",
  "El PDF o la foto de la factura tal como llegó del proveedor.":
    "El PDF o la foto de la factura tal com va arribar del proveïdor.",
  "Documento ·": "Document ·",
  "Documento adjuntado": "Document adjuntat",
  "Traspaso entre cuentas propias": "Traspàs entre comptes propis",
  "Cargo de impuestos": "Càrrec d'impostos",
  Intereses: "Interessos",
  "Sin factura, con motivo": "Sense factura, amb motiu",
  "Movimiento explicado — sale de la cola sin factura":
    "Moviment explicat — surt de la cua sense factura",
  "Motivos sin factura (banco)": "Motius sense factura (banc)",
  "Comisión bancaria": "Comissió bancària",
  "Comissió bancària": "Comissió bancària",
  "Descompte per pagament ràpid": "Descompte per pagament ràpid",
  "sin factura · con motivo": "sense factura · amb motiu",
  "＋ Cuenta": "＋ Compte",
  "Nueva cuenta": "Compte nou",
  "Cuenta bancaria": "Compte bancari",
  "Tarjeta de crédito": "Targeta de crèdit",
  "Caja de efectivo": "Caixa d'efectiu",
  "Saldo inicial (€)": "Saldo inicial (€)",
  "BBVA · cuenta corriente": "BBVA · compte corrent",
  "Crear cuenta": "Crear compte",
  "La cuenta necesita un nombre": "El compte necessita un nom",
  "Cuenta creada": "Compte creat",
  "Liquidación de tarjeta": "Liquidació de targeta",
  "⬆ Importar extracto (.xlsx / .pdf)": "⬆ Importar extracte (.xlsx / .pdf)",
  "Importar extracto ·": "Importar extracte ·",
  "Importar extracto": "Importar extracte",
  "Extracto importado": "Extracte importat",
  "Elige primero la cuenta a la que pertenece el extracto.":
    "Tria primer el compte al qual pertany l'extracte.",
  "Periodo del extracto": "Període de l'extracte",
  "Movimientos nuevos": "Moviments nous",
  "Ya importados": "Ja importats",
  "Filas no legibles": "Files no llegibles",
  "El periodo se solapa con movimientos ya importados; los repetidos no se volverán a crear.":
    "El període se solapa amb moviments ja importats; els repetits no es tornaran a crear.",
  "No he encontrado la cabecera del extracto (Fecha / Concepto / Importe). ¿Es el export de movimientos de BBVA?":
    "No he trobat la capçalera de l'extracte (Fecha / Concepto / Importe). És l'export de moviments de BBVA?",
  "No he encontrado ningún movimiento con fecha en el PDF. ¿Es un extracto de BBVA?":
    "No he trobat cap moviment amb data al PDF. És un extracte de BBVA?",
  "Nueva factura de proveedor": "Nova factura de proveïdor",
  "Registrar documento como factura": "Registrar el document com a factura",
  "Registrar como factura": "Registrar com a factura",
  "Registrar factura": "Registrar factura",
  "Nº factura *": "Núm. factura *",
  "Base imponible (€) *": "Base imposable (€) *",
  "para poder conciliarla con el banco": "per poder conciliar-la amb el banc",
  "El NIF leído no coincide con ningún proveedor": "El NIF llegit no coincideix amb cap proveïdor",
  "No hay ningún proveedor dado de alta todavía.": "Encara no hi ha cap proveïdor donat d'alta.",
  "La factura necesita un número": "La factura necessita un número",
  "La factura necesita una base imponible": "La factura necessita una base imposable",
  "El reparto debe sumar la base imponible": "El repartiment ha de sumar la base imposable",
  "Factura registrada": "Factura registrada",
  "Las horas de obra necesitan partida y subpartida":
    "Les hores d'obra necessiten partida i subpartida",
  "⚠ Las horas de obra necesitan partida y subpartida":
    "⚠ Les hores d'obra necessiten partida i subpartida",
  "El coste de obra necesita partida y subpartida": "El cost d'obra necessita partida i subpartida",
  "⚠ El coste de obra necesita partida y subpartida":
    "⚠ El cost d'obra necessita partida i subpartida",
  "Reparta la factura: a una obra o a gastos generales":
    "Repartiu la factura: a una obra o a despeses generals",
  "Cada línea de obra necesita partida y subpartida":
    "Cada línia d'obra necessita partida i subpartida",
  "⚠ Cada línea de obra necesita partida y subpartida":
    "⚠ Cada línia d'obra necessita partida i subpartida",
  "⚠ Reparta la factura: a una obra o a gastos generales":
    "⚠ Repartiu la factura: a una obra o a despeses generals",
  "El movimiento ya está conciliado.": "El moviment ja està conciliat.",
  "Ha sido un pago parcial": "Ha estat un pagament parcial",
  "Pago parcial — el resto sigue debiéndose": "Pagament parcial — la resta se segueix devent",
  Registrar: "Registrar",
  "Un pago parcial no cierra nada: el resto sigue debiéndose y aparecerá como pendiente. Los demás motivos sí dan el documento por cerrado, y quedan anotados.":
    "Un pagament parcial no tanca res: la resta se segueix devent i apareixerà com a pendent. Els altres motius sí que donen el document per tancat, i queden anotats.",
  "Deshacer la conciliación": "Desfer la conciliació",
  "Conciliación deshecha": "Conciliació desfeta",
  "El movimiento vuelve a la cola sin explicar y se anula el pago que creó la conciliación.":
    "El moviment torna a la cua sense explicar i s'anul·la el pagament que va crear la conciliació.",
  "Si ha sido un pago parcial no hay nada más que hacer: el resto sigue debiéndose y aparecerá como pendiente.":
    "Si ha estat un pagament parcial no hi ha res més a fer: la resta continua devent-se i apareixerà com a pendent.",
  "Por subpartida": "Per subpartida",
  "Esta partida no tiene subpartidas en su línea base.":
    "Aquesta partida no té subpartides a la seva línia base.",
  "Acumulado real": "Acumulat real",
  "acumulado real": "acumulat real",
  "Las cifras son las reales: lo presupuestado, lo gastado y la diferencia entre ambos. El presupuesto no se ajusta desde aquí — se cambia con un adicional al contrato, que es lo que lo modifica.":
    "Les xifres són les reals: el pressupostat, el gastat i la diferència entre tots dos. El pressupost no s'ajusta des d'aquí — es canvia amb un addicional al contracte, que és el que el modifica.",
  "Para una factura que no llega como documento — la mayoría se suben o se fotografían en Documentos":
    "Per a una factura que no arriba com a document — la majoria es pugen o es fotografien a Documents",
  "Al firmarlo": "En signar-lo",
  "Sus partidas entran en el alcance de la obra.": "Les seves partides entren a l'abast de l'obra.",
  "Su hito de pago se añade al contrato.": "La seva fita de pagament s'afegeix al contracte.",
  "Sus días se suman a la fecha de fin.": "Els seus dies se sumen a la data de fi.",
  "Todavía no está en la obra: ni sus partidas, ni su hito de pago, ni sus días. Se incorpora al firmarlo.":
    "Encara no és a l'obra: ni les seves partides, ni la seva fita de pagament, ni els seus dies. S'incorpora en signar-lo.",
  Nuevos: "Nous",
  "＋ Adicional": "＋ Addicional",
  "Nuevo adicional": "Nou addicional",
  "Crear adicional": "Crear addicional",
  "Adicional creado": "Addicional creat",
  "Anexo sin firmar": "Annex sense signar",
  "Ningún adicional coincide con la búsqueda.": "Cap addicional coincideix amb la cerca.",
  "Ningún contrato en marcha": "Cap contracte en marxa",
  "¿A qué contrato?": "A quin contracte?",
  "Contrato en marcha": "Contracte en marxa",
  "Cómo sigue": "Com segueix",
  "Ir a Contratos": "Anar a Contractes",
  "Los adicionales de obras en marcha. Se crean sobre un contrato y se incorporan a la obra al firmar su anexo.":
    "Els addicionals d'obres en marxa. Es creen sobre un contracte i s'incorporen a l'obra en signar el seu annex.",
  "Un adicional modifica un contrato vivo, así que primero hace falta uno con su obra abierta.":
    "Un addicional modifica un contracte viu, així que primer cal un amb la seva obra oberta.",
  "Se abre el presupuestador vacío y el adicional se escribe partida a partida.":
    "S'obre el pressupostador buit i l'addicional s'escriu partida a partida.",
  "Cuando el cliente lo acepte, aparecerá en el contrato como anexo pendiente de firma.":
    "Quan el client l'accepti, apareixerà al contracte com a annex pendent de signatura.",
  "Se incorpora a la obra al firmar ese anexo, con sus días.":
    "S'incorpora a l'obra en signar aquest annex, amb els seus dies.",
  "Un adicional de una obra en marcha se crea en la pestaña Adicionales, sobre el contrato que modifica.":
    "Un addicional d'una obra en marxa es crea a la pestanya Addicionals, sobre el contracte que modifica.",
  "No encuentro ese anexo": "No trobo aquest annex",
  "No encuentro ese contrato": "No trobo aquest contracte",
  "Cómo se cobra": "Com es cobra",
  "Ir a Conciliación": "Anar a Conciliació",
  "El cobro se registra al conciliar el movimiento del banco con esta factura, no a mano desde aquí.":
    "El cobrament es registra en conciliar el moviment del banc amb aquesta factura, no a mà des d'aquí.",
  "Anexo firmado": "Annex signat",
  "✍ Firmar anexo": "✍ Signar l'annex",
  "Nombre y apellidos": "Nom i cognoms",
  "Anexo retirado": "Annex retirat",
  "Aprobado verbalmente": "Aprovat verbalment",
  "Cambiar la firma": "Canviar la signatura",
  "El documento firmado": "El document signat",
  "Firma retirada": "Signatura retirada",
  "Importe que sale del contrato": "Import que surt del contracte",
  "Lo aprobó": "Ho va aprovar",
  "Quién lo aprobó": "Qui ho va aprovar",
  "Qué se deshace": "Què es desfà",
  "Quitar anexo": "Treure l'annex",
  "Quitar la firma": "Treure la signatura",
  "Quitar todos los anexos": "Treure tots els annexos",
  "Y con ello": "I amb això",
  "El documento que firmó el cliente, como el contrato.":
    "El document que va signar el client, com el contracte.",
  "Queda registrado como lo que es: sin documento, y con el nombre de quien lo aprobó.":
    "Queda registrat com el que és: sense document, i amb el nom de qui ho va aprovar.",
  "El hito de pago que cada anexo añadió.": "La fita de pagament que cada annex va afegir.",
  "Los días que sumó a la fecha de fin.": "Els dies que va sumar a la data de fi.",
  "Sus partidas dejan de estar en el alcance de la obra.":
    "Les seves partides deixen d'estar a l'abast de l'obra.",
  "Se niega si el hito ya está facturado o si hay avance marcado sobre sus partidas.":
    "Es nega si la fita ja està facturada o si hi ha avanç marcat sobre les seves partides.",
  "Un anexo firmado necesita el documento firmado": "Un annex signat necessita el document signat",
  "Di quién lo aprobó verbalmente": "Digues qui ho va aprovar verbalment",
  "El hito de este anexo ya se ha facturado. Rectifica la factura antes de quitarlo.":
    "La fita d'aquest annex ja s'ha facturat. Rectifica la factura abans de treure'l.",
  "Hay avance marcado sobre las partidas de este anexo. Ponlo a cero antes de quitarlo.":
    "Hi ha avanç marcat sobre les partides d'aquest annex. Posa'l a zero abans de treure'l.",
  "Cuentas y tarjetas": "Comptes i targetes",
  "Ir a Configuración › Empresa": "Anar a Configuració › Empresa",
  "Primero crea la tarjeta en Configuración › Empresa":
    "Primer crea la targeta a Configuració › Empresa",
  "Las cuentas y las tarjetas se crean en Configuración › Empresa.":
    "Els comptes i les targetes es creen a Configuració › Empresa.",
  Desactivar: "Desactivar",
  Reactivar: "Reactivar",
  Desactivada: "Desactivada",
  "Borrar la cuenta": "Esborrar el compte",
  "＋ Días de los adicionales": "＋ Dies dels addicionals",
  "Lleva a la planificación los días de los adicionales aceptados de esta obra":
    "Porta a la planificació els dies dels addicionals acceptats d'aquesta obra",
  "Planificación actualizada con los días de los adicionales":
    "Planificació actualitzada amb els dies dels addicionals",
  "Módulo de planificación no disponible": "Mòdul de planificació no disponible",
  "Sin tarea derivada para la partida": "Sense tasca derivada per a la partida",
  "— deriva la planificación primero": "— deriva la planificació primer",
  "Aceptar el adicional": "Acceptar l'addicional",
  "Adenda firmada": "Addenda signada",
  "Adicional aceptado — entra en la obra": "Addicional acceptat — entra a l'obra",
  "Adicional de una obra en marcha": "Addicional d'una obra en marxa",
  "Ahora mismo no hay ninguno emitido esperando respuesta.":
    "Ara mateix no n'hi ha cap d'emès esperant resposta.",
  "Arrastra aquí la adenda firmada, el correo o la captura del WhatsApp.":
    "Arrossega aquí l'addenda signada, el correu o la captura del WhatsApp.",
  "Contratos vigentes": "Contractes vigents",
  Cuál: "Quin",
  Cómo: "Com",
  "Cómo lo aceptó el cliente": "Com ho va acceptar el client",
  "De palabra, en obra": "De paraula, a l'obra",
  "Efecto en coste": "Efecte en cost",
  "Efecto en plazo, por partida": "Efecte en termini, per partida",
  "Formalizar un adicional": "Formalitzar un addicional",
  "Margen después": "Marge després",
  "Nada que formalizar": "Res a formalitzar",
  "No hay ninguna obra con alcance aceptado": "No hi ha cap obra amb abast acceptat",
  "No hay ninguna obra con presupuesto aceptado todavía.":
    "Cap obra té encara un pressupost acceptat.",
  "Nombre de quien dio el visto bueno": "Nom de qui va donar el vistiplau",
  "Obra en marcha": "Obra en marxa",
  "Quién aceptó": "Qui va acceptar",
  "Total de días": "Total de dies",
  "Días de los adicionales": "Dies dels addicionals",
  "Las duraciones se vuelven a calcular a partir de las cantidades del presupuesto: una barra alargada o acortada a mano vuelve a su duración derivada.":
    "Les durades es tornen a calcular a partir de les quantitats del pressupost: una barra allargada o escurçada a mà torna a la seva durada derivada.",
  " Se conservan el avance registrado, las líneas base congeladas, las fechas fijadas a mano y los días de los adicionales ya aplicados.":
    " Es conserven l'avenç registrat, les línies base congelades, les dates fixades a mà i els dies dels addicionals ja aplicats.",
  "Aplicar a la planificación": "Aplicar a la planificació",
  "En la planificación: pendiente": "A la planificació: pendent",
  "Los días de cada partida alargan su barra en la planificación. El total mueve la fecha de fin de obra: se propone la suma, cámbielo si las partidas se solapan.":
    "Els dies de cada partida allarguen la seva barra a la planificació. El total mou la data de fi d'obra: es proposa la suma, canvieu-lo si les partides se solapen.",
  "El total mueve la fecha de fin. Se propone la suma; cámbielo si las partidas se solapan.":
    "El total mou la data de fi. Es proposa la suma; canvieu-lo si les partides se solapen.",
  "＋ Nuevo adicional": "＋ Nou addicional",
  "Se copia el alcance aceptado de la obra para modificarlo, ampliarlo o reducirlo. El presupuesto original se conserva.":
    "Es copia l'abast acceptat de l'obra per modificar-lo, ampliar-lo o reduir-lo. El pressupost original es conserva.",
  "Adenda al contrato": "Addenda al contracte",
  "Adenda guardada": "Addenda desada",
  Adenda: "Addenda",
  "Qué se añade": "Què s'afegeix",
  "Importe del adicional": "Import de l'addicional",
  "Días que añade a la obra": "Dies que afegeix a l'obra",
  "Fin previsto de la obra": "Fi previst de l'obra",
  "Cambiar este número mueve esa fecha por la diferencia, no por el total.":
    "Canviar aquest número mou aquesta data per la diferència, no pel total.",
  "Guardar la adenda": "Desar l'addenda",
  "Esta obra todavía no tiene contrato: la adenda se añadirá cuando lo tenga.":
    "Aquesta obra encara no té contracte: l'addenda s'afegirà quan en tingui.",
  "sin fecha": "sense data",
  "El importe, el cliente y la obra salen del presupuesto: no se teclean aquí. Se cobra como un hito propio, añadido a los del contrato, que conservan su importe.":
    "L'import, el client i l'obra surten del pressupost: no es teclegen aquí. Es cobra com un fita pròpia, afegida a les del contracte, que conserven el seu import.",
  "Contraer todo": "Contraure-ho tot",
  "Desplegar todo": "Desplegar-ho tot",
  "Sin documentos todavía.": "Encara sense documents.",
  "Todo el coste de esta obra tiene partida. La tabla cuadra con el proyecto.":
    "Tot el cost d'aquesta obra té partida. La taula quadra amb el projecte.",
  "La tabla no cuadra con el coste del proyecto.": "La taula no quadra amb el cost del projecte.",
  "Las cifras son las reales: lo presupuestado, lo gastado y la diferencia entre ambos. Pulse una partida para ver sus subpartidas, y el botón de documentos de una subpartida para ver lo que hay detrás. El presupuesto no se ajusta desde aquí — se cambia con un adicional al contrato, que es lo que lo modifica.":
    "Les xifres són les reals: el pressupostat, el gastat i la diferència entre tots dos. Premi una partida per veure les seves subpartides, i el botó de documents d'una subpartida per veure què hi ha darrere. El pressupost no s'ajusta des d'aquí — es canvia amb un addicional al contracte, que és el que el modifica.",
  Proyección: "Projecció",
  Proyectado: "Projectat",
  "Documentos ·": "Documents ·",
  "Esta subpartida": "Aquesta subpartida",
  "Costes sin subpartida": "Costos sense subpartida",
  "Sin documentos": "Sense documents",
  "Esta subpartida no tiene documentos todavía.": "Aquesta subpartida encara no té documents.",
  "Los documentos de abajo suman exactamente el acumulado de esta subpartida.":
    "Els documents de sota sumen exactament l'acumulat d'aquesta subpartida.",
  "Los documentos de abajo no suman el acumulado de esta subpartida.":
    "Els documents de sota no sumen l'acumulat d'aquesta subpartida.",
  "Esta vista llega hasta la partida y no baja a la subpartida: un pedido y una adjudicación se firman contra una partida, y repartirlos entre subpartidas sería inventar un dato que nadie ha registrado.":
    "Aquesta vista arriba fins a la partida i no baixa a la subpartida: una comanda i una adjudicació es signen contra una partida, i repartir-les entre subpartides seria inventar una dada que ningú ha registrat.",
  "El coste se asigna a la subpartida desde Gastos y desde Horas; desde aquí solo se consulta.":
    "El cost s'assigna a la subpartida des de Despeses i des d'Hores; des d'aquí només es consulta.",
  "Base (€)": "Base (€)",
  "Cuota IVA (€)": "Quota IVA (€)",
  "Cuota (€)": "Quota (€)",
  "Retención (€)": "Retenció (€)",
  "Pagado (€)": "Pagat (€)",
  "Cobrado (€)": "Cobrat (€)",
  "Facturas recibidas ·": "Factures rebudes ·",
  "Facturas emitidas ·": "Factures emeses ·",
  "Resumen del trimestre ·": "Resum del trimestre ·",
  "IVA repercutido por tipo": "IVA repercutit per tipus",
  "IVA soportado por tipo": "IVA suportat per tipus",
  Repercutido: "Repercutit",
  Soportado: "Suportat",
  "Resultado del trimestre": "Resultat del trimestre",
  IRPF: "IRPF",
  "Practicada a terceros": "Practicada a tercers",
  "Soportada por clientes": "Suportada per clients",
  "Sin movimientos de banco en este trimestre — importe el extracto en Conciliación. Las facturas del trimestre están en las otras hojas.":
    "Sense moviments de banc en aquest trimestre — importi l'extracte a Conciliació. Les factures del trimestre són a les altres pestanyes.",
  "El trimestre no tiene movimientos de banco todavía, así que la hoja de conciliación va vacía. Las facturas del trimestre sí van, en sus propias hojas.":
    "El trimestre encara no té moviments de banc, així que el full de conciliació va buit. Les factures del trimestre sí que hi van, en fulls propis.",
  "Efectivo pendiente de justificar": "Efectiu pendent de justificar",
  justificado: "justificat",
  devuelto: "retornat",
  "Un reintegro se explica con los tickets que pagó y con lo que vuelve al banco. Ábrelo para adjuntarle sus documentos; lo que sobre se devuelve identificando el ingreso como devolución de efectivo.":
    "Un reintegrament s'explica amb els tiquets que va pagar i amb el que torna al banc. Obri'l per adjuntar-hi els seus documents; el que sobri es torna identificant l'ingrés com a devolució d'efectiu.",
  "sin avance": "sense avanç",
  "Proyectado es a dónde va la partida a este ritmo: lo gastado más el presupuesto del trabajo que queda. Desviación es proyectado menos presupuestado.":
    "Projectat és cap a on va la partida a aquest ritme: el gastat més el pressupost de la feina que queda. Desviació és projectat menys pressupostat.",
  "Comprometido es lo ya pedido o adjudicado, esté facturado o no, y el proyectado nunca baja de ahí.":
    "Compromès és el que ja s'ha demanat o adjudicat, estigui facturat o no, i el projectat mai no baixa d'aquí.",
  "Una partida con gasto y sin avance registrado no se puede proyectar: se muestra el suelo, lo mayor entre lo presupuestado y lo gastado, y se marca «sin avance». Registra el avance en Avance físico para que la proyección signifique algo.":
    "Una partida amb despesa i sense avanç registrat no es pot projectar: es mostra el terra, el més gran entre el pressupostat i el gastat, i es marca «sense avanç». Registra l'avanç a Avanç físic perquè la projecció signifiqui alguna cosa.",
  "Buscar proveedor o industrial por nombre o NIF": "Cerca proveidor o industrial per nom o NIF",
  "Motivo del adicional": "Motiu de l'addicional",
  "Días de plazo que añade": "Dies de termini que afegeix",
  "Ampliación de la instalación eléctrica": "Ampliació de la instal·lació elèctrica",
  "Se abre el presupuestador con una versión nueva sobre esa obra. Al aceptarse, sus partidas entran en la economía, el avance y la certificación, la fecha de fin se amplía con los días indicados y se añade la adenda al contrato.":
    "S'obre el pressupostador amb una versió nova sobre aquesta obra. En acceptar-se, les seves partides entren a l'economia, l'avanç i la certificació, la data de fi s'amplia amb els dies indicats i s'afegeix l'addenda al contracte.",
  Adicional: "Addicional",
  "Bloquear el trimestre": "Bloquejar el trimestre",
  "🔒 Bloquear trimestre": "🔒 Bloquejar trimestre",
  "Trimestre bloqueado": "Trimestre bloquejat",
  "Qué se bloquea": "Què es bloqueja",
  "Este trimestre ya está bloqueado.": "Aquest trimestre ja està bloquejat.",
  "Todo lo del trimestre está conciliado y asignado.":
    "Tot el del trimestre està conciliat i assignat.",
  "Movimientos sin conciliar": "Moviments sense conciliar",
  "Costes sin destino — ni obra ni gasto general":
    "Costos sense destí — ni obra ni despesa general",
  "Costes de obra sin partida y subpartida": "Costos d'obra sense partida i subpartida",
  "Horas de obra sin partida y subpartida": "Hores d'obra sense partida i subpartida",
  "Efectivo retirado y todavía sin justificar": "Efectiu retirat i encara sense justificar",
  "Documentos capturados y sin registrar": "Documents capturats i sense registrar",
  "Bloquear un trimestre impide cambiar lo que hay dentro: conciliaciones, costes y horas quedan como están. Se puede reabrir, y la reapertura queda registrada con su motivo.":
    "Bloquejar un trimestre impedeix canviar el que hi ha dins: conciliacions, costos i hores queden com estan. Es pot reobrir, i la reobertura queda registrada amb el seu motiu.",
  "Reintegro de efectivo": "Reintegrament d'efectiu",
  "Devolución de efectivo": "Devolució d'efectiu",
  "Devuelto al banco": "Retornat al banc",
  "En efectivo todavía": "Encara en efectiu",
  "Justificado con documentos": "Justificat amb documents",
  "queda por justificar": "queda per justificar",
  "todo justificado": "tot justificat",
  conciliación: "conciliació",
  "Desactivar una cuenta la retira de las listas y conserva lo conciliado. Borrar la quita del todo: si tiene movimientos detrás, se avisa de cuántos y se van con ella.":
    "Desactivar un compte el retira de les llistes i conserva el conciliat. Esborrar-lo el treu del tot: si té moviments al darrere, s'avisa de quants i se'n van amb ell.",
  Borrar: "Esborrar",
  "Hay movimientos de otra cuenta que dicen liquidar esta tarjeta":
    "Hi ha moviments d'un altre compte que diuen liquidar aquesta targeta",
  "Tiene movimientos en un periodo cerrado — ábralo primero":
    "Té moviments en un període tancat — obri'l primer",
  "Tiene movimientos detrás": "Té moviments al darrere",
  "Tiene extractos importados detrás": "Té extractes importats al darrere",
  "Borrar la cuenta y sus movimientos": "Esborrar el compte i els seus moviments",
  "Se borra la cuenta y todos los movimientos que tiene detrás. Las conciliaciones se deshacen primero, así que ninguna factura queda diciendo que la pagó un movimiento que ya no existe. No se puede deshacer.":
    "S'esborra el compte i tots els moviments que té al darrere. Les conciliacions es desfan primer, així que cap factura queda dient que la va pagar un moviment que ja no existeix. No es pot desfer.",
  "Se borra la cuenta. No tiene ningún movimiento detrás, así que no se pierde nada conciliado.":
    "S'esborra el compte. No té cap moviment al darrere, així que no es perd res conciliat.",
  Elegida: "Triada",
  "el extracto se importa aquí": "l'extracte s'importa aquí",
  "elija una cuenta abajo para importar": "triï un compte a sota per importar",
  "Elegir la foto": "Triar la foto",
  "Cuenta borrada": "Compte esborrat",
  "Cuenta desactivada": "Compte desactivat",
  "Cuenta reactivada": "Compte reactivat",
  "No hay ninguna cuenta todavía.": "Encara no hi ha cap compte.",
  "Anular registro": "Anul·lar registre",
  "Anular el registro": "Anul·lar el registre",
  Anular: "Anul·lar",
  "Registro anulado": "Registre anul·lat",
  "Su pago liquida además otras facturas. Deshaga primero ese pago en Conciliación.":
    "El seu pagament liquida també altres factures. Desfeu primer aquest pagament a Conciliació.",
  "La factura sale del registro y el documento archivado vuelve a quedar libre para volver a registrarlo.":
    "La factura surt del registre i el document arxivat torna a quedar lliure per tornar a registrar-lo.",
  "La factura sale del registro con su reparto. No se puede deshacer.":
    "La factura surt del registre amb el seu repartiment. No es pot desfer.",
  "La factura sale del registro y el documento archivado vuelve a quedar sin registrar. No se puede deshacer.":
    "La factura surt del registre i el document arxivat torna a quedar sense registrar. No es pot desfer.",
  "La factura sale del registro con su reparto a las obras. No se puede deshacer.":
    "La factura surt del registre amb la seva assignació a les obres. No es pot desfer.",
  "Ir a la factura": "Anar a la factura",
  Corregir: "Corregir",
  "Corregir la factura": "Corregir la factura",
  "Al cambiar de proveedor se recalcula la retención según su perfil fiscal.":
    "En canviar de proveïdor es recalcula la retenció segons el seu perfil fiscal.",
  "Cambiar el proveedor": "Canviar el proveïdor",
  "Proveedor cambiado": "Proveïdor canviat",
  "Eliminar la factura": "Eliminar la factura",
  "Factura eliminada": "Factura eliminada",
  "Nadie ha tocado todavía esta factura. Al eliminarla, el documento archivado vuelve a quedar libre.":
    "Ningú no ha tocat encara aquesta factura. En eliminar-la, el document arxivat torna a quedar lliure.",
  "Se borra la factura registrada y el documento archivado vuelve a quedar sin registrar. No se puede deshacer.":
    "S'esborra la factura registrada i el document arxivat torna a quedar sense registrar. No es pot desfer.",
  "Se borra la factura registrada y su asignación a las obras. No se puede deshacer.":
    "S'esborra la factura registrada i la seva assignació a les obres. No es pot desfer.",
  "Ya tiene un pago registrado. Anule primero el pago.":
    "Ja té un pagament registrat. Anul·leu primer el pagament.",
  "Su trimestre ya se ha enviado al asesor. Registre un abono en su lugar.":
    "El seu trimestre ja s'ha enviat a l'assessor. Registreu un abonament al seu lloc.",
  "Está casada con un movimiento bancario. Deshaga primero la conciliación.":
    "Està casada amb un moviment bancari. Desfeu primer la conciliació.",
  "Tiene un abono que la referencia. Elimine primero el abono.":
    "Té un abonament que la referencia. Elimineu primer l'abonament.",
  "Vaciar la cuenta": "Buidar el compte",
  "Vaciar la cuenta entera": "Buidar el compte sencer",
  Vaciar: "Buidar",
  "Cuenta vaciada": "Compte buidat",
  "Se deshacen conciliaciones": "Es desfan conciliacions",
  "Pierden su reparto": "Perden el seu repartiment",
  "Hay movimientos dentro de un periodo cerrado. Reabra el periodo en Conciliación para poder vaciar la cuenta.":
    "Hi ha moviments dins d'un període tancat. Reobriu el període a Conciliació per poder buidar el compte.",
  "Para volver a empezar una prueba: borra TODOS los movimientos de esta cuenta, incluidos los ya conciliados.":
    "Per tornar a començar una prova: esborra TOTS els moviments d'aquest compte, inclosos els ja conciliats.",
  "Se borran todos los movimientos de esta cuenta y se deshacen sus conciliaciones, con los pagos y cobros que crearon. No se puede deshacer.":
    "S'esborren tots els moviments d'aquest compte i es desfan les seves conciliacions, amb els pagaments i cobraments que van crear. No es pot desfer.",
  "— Elija el proveedor —": "— Trieu el proveïdor —",
  "El emisor del documento no está dado de alta como proveedor":
    "L'emissor del document no està donat d'alta com a proveïdor",
  "Nombre del emisor": "Nom de l'emissor",
  "NIF/CIF del emisor": "NIF/CIF de l'emissor",
  "Dar de alta este proveedor": "Donar d'alta aquest proveïdor",
  "Elija el proveedor al que pertenece la factura": "Trieu el proveïdor a qui pertany la factura",
  "⚠ Elija el proveedor al que pertenece la factura":
    "⚠ Trieu el proveïdor a qui pertany la factura",
  "El proveedor necesita un nombre": "El proveïdor necessita un nom",
  /* The write that did not reach the server, in Catalan (S35/S145). */
  "esta sesión": "aquesta sessió",
  "⚠ NO se ha guardado:": "⚠ NO s'ha desat:",
  "· recargue la página": "· torni a carregar la pàgina",
  "Esta cuenta no puede guardar en este servidor.":
    "Aquest compte no pot desar en aquest servidor.",
  "El servidor está enviando un documento parcial para":
    "El servidor està enviant un document parcial per a",
  ". Consulte con un administrador antes de introducir nada.":
    ". Consulti amb un administrador abans d'introduir res.",
  "⚠ El proveedor necesita un nombre": "⚠ El proveïdor necessita un nom",
  "Proveedor dado de alta": "Proveïdor donat d'alta",
  "Opcionales (aparte)": "Opcionals (a part)",
  "Total por m²": "Total per m²",
  "⚠ Líneas pendientes de precio (no incluidas)": "⚠ Línies pendents de preu (no incloses)",
  "Margen interno": "Marge intern",
  "nunca aparece en el documento del cliente": "mai no apareix al document del client",
  "Coste interno base:": "Cost intern base:",
  "· Margen:": "· Marge:",
  "Margen previsto": "Marge previst",
  "Avance por partida": "Avanç per partida",
  "marca lo hecho — alimenta la certificación": "marca el que s'ha fet — alimenta la certificació",
  "✓ Hecho": "✓ Fet",
  "Coste por partida": "Cost per partida",
  "previsto vs. real": "previst vs. real",
  "Coste prev.": "Cost prev.",
  Desvío: "Desviació",
  Detectado: "Detectat",
  Valorado: "Valorat",
  Aprobado: "Aprovat",
  Ejecutado: "Executat",
  Rechazado: "Rebutjat",
  Aprobar: "Aprovar",
  "Sin extras registrados.": "Sense extres registrats.",
  "Facturación del proyecto": "Facturació del projecte",
  "· Cobrado": "· Cobrat",
  "· Pendiente": "· Pendent",
  "· Por facturar": "· Per facturar",
  "Avance actualizado": "Avanç actualitzat",
  "Extra aprobado — anexo generado": "Extra aprovat — annex generat",
  "Datos exportados": "Dades exportades",
  "¿Restablecer el entorno con los datos de ejemplo? Se sustituye todo lo actual.":
    "Voleu restablir l'entorn amb les dades d'exemple? Se substitueix tot el que hi ha ara.",
  "Datos de ejemplo cargados": "Dades d'exemple carregades",
  "Canei Subirats ERP — Torre de control": "Canei Subirats ERP — Torre de control",
  "Reformas, sencillamente complejas.": "Reformes, senzillament complexes.",
  "ERP · Torre de control": "ERP · Torre de control",
  "Recorrido ↗": "Recorregut ↗",
  "Generador de presupuestos ↗": "Generador de pressupostos ↗",
  Hoy: "Avui",
  "Recorrido del cliente": "Recorregut del client",
  "Proyectos y dinero": "Projectes i diners",
  "Cuadros de mando": "Taulers de comandament",
  Correos: "Correus",
  "Alertas y tareas": "Alertes i tasques",
  "Recorrido del cliente — del contacto al beneficio":
    "Recorregut del client — del contacte al benefici",
  "El ciclo de vida completo. Haga clic en una fase para ver los proyectos que contiene.":
    "El cicle de vida complet. Feu clic en una fase per veure els projectes que conté.",
  "Valor = cartera y obra en curso en cada fase": "Valor = cartera i obra en curs en cada fase",
  "Proyectos y control financiero": "Projectes i control financer",
  "Coste previsto → comprometido → real → ingresos → margen, por proyecto. Haga clic en una fila para ver la cascada de dinero.":
    "Cost previst → compromès → real → ingressos → marge, per projecte. Feu clic en una fila per veure la cascada de diners.",
  "Cartera de ventas (valor por fase)": "Cartera de vendes (valor per fase)",
  "Margen de presupuesto por proyecto": "Marge de pressupost per projecte",
  "Antigüedad de cobros": "Antiguitat de cobraments",
  "Pagos pendientes": "Pagaments pendents",
  "Compras vs coste previsto": "Compres vs cost previst",
  "Previsión de caja — próximas 6 semanas (entradas / salidas)":
    "Previsió de caixa — properes 6 setmanes (entrades / sortides)",
  "Entradas (cobros)": "Entrades (cobraments)",
  "Salidas (proveedores y nóminas)": "Sortides (proveïdors i nòmines)",
  "Rendimiento de la empresa — ingresos y margen": "Rendiment de l'empresa — ingressos i marge",
  Ingresos: "Ingressos",
  Gastos: "Despeses",
  "Validez · 30 días": "Validesa · 30 dies",
  "Desde la fecha de emisión": "Des de la data d'emissió",
  "Comparativa de proveedores — partida de baño (precio ausente ≠ cero)":
    "Comparativa de proveïdors — partida de bany (preu absent ≠ zero)",
  "Correos — redactados automáticamente, un clic para enviar":
    "Correus — redactats automàticament, un clic per enviar",
  "Cada evento del ciclo de vida redacta el correo por usted. Revíselo, ajústelo si hace falta y pulse":
    "Cada esdeveniment del cicle de vida redacta el correu per vosaltres. Reviseu-lo, ajusteu-lo si cal i premeu",
  Enviar: "Enviar",
  "No se envía nada sin usted:": "No s'envia res sense vosaltres:",
  "cada mensaje se redacta para su revisión — pulse":
    "cada missatge es redacta per a la vostra revisió — premeu",
  "cuando esté listo. El envío con un clic se activa al conectar su proveedor de correo.":
    "quan estigui a punt. L'enviament amb un clic s'activa en connectar el vostre proveïdor de correu.",
  Actividad: "Activitat",
  "— aún no se ha enviado ningún correo.": "— encara no s'ha enviat cap correu.",
  "Próximas acciones (por responsable)": "Properes accions (per responsable)",
  "Valor de cartera": "Valor de cartera",
  // ── COM-01/02: leads and the visit lifecycle (S4).
  Atrasada: "Endarrerida",
  "Buscar por cliente o trabajo": "Cerca per client o feina",
  "Buscar por cliente": "Cerca per client",
  "Cada consulta crea una oportunidad con origen, estado y próxima acción. La visita se programa y se captura desde su propia pantalla.":
    "Cada consulta crea una oportunitat amb origen, estat i propera acció. La visita es programa i es captura des de la seva pròpia pantalla.",
  "Fecha/hora": "Data/hora",
  Informe: "Informe",
  Inmueble: "Immoble",
  "Ninguna oportunidad coincide con la búsqueda.": "Cap oportunitat coincideix amb la cerca.",
  No: "No",
  Oportunidades: "Oportunitats",
  Programada: "Programada",
  Programadas: "Programades",
  Realizada: "Realitzada",
  Realizadas: "Realitzades",
  "Sin crear": "Sense crear",
  "Sin visitas programadas.": "Sense visites programades.",
  "Sin visitas realizadas.": "Sense visites realitzades.",
  "Visitas programadas y realizadas. Programar una visita no captura nada todavía; completarla registra mediciones, fotos y notas.":
    "Visites programades i realitzades. Programar una visita no captura res encara; completar-la registra amidaments, fotos i notes.",
  "⚠ No hay oportunidades abiertas para programar una visita":
    "⚠ No hi ha oportunitats obertes per programar una visita",
  "＋ Nueva oportunidad": "＋ Nova oportunitat",
  "＋ Programar visita": "＋ Programar visita",
  "Indica la fecha de la visita": "Indica la data de la visita",
  "Oportunidad marcada como perdida": "Oportunitat marcada com a perduda",
  "Presupuesto creado desde la visita": "Pressupost creat des de la visita",
  "Próxima acción actualizada": "Propera acció actualitzada",
  "Referencias, acceso, contacto en obra…": "Referències, accés, contacte a l'obra…",
  "Sin mediciones registradas": "Sense amidaments registrats",
  "Visita completada": "Visita completada",
  "⚠ Indica qué se mide y una cantidad": "⚠ Indica què es mesura i una quantitat",
  "⚠ Motivo no reconocido": "⚠ Motiu no reconegut",
  // ── COM-03: the presupuestador (S5).
  "El registro va agrupado por estado: primero lo que falta por escribir, después lo que espera al cliente, y al final lo cerrado.":
    "El registre va agrupat per estat: primer el que falta per escriure, després el que espera el client, i al final el que està tancat.",
  Borradores: "Esborranys",
  Enviados: "Enviats",
  Aceptados: "Acceptats",
  Rechazados: "Rebutjats",
  Caducados: "Caducats",
  "Todavía no hay presupuestos.": "Encara no hi ha pressupostos.",
  "Todos los presupuestos": "Tots els pressupostos",
  Congelada: "Congelada",
  "Nueva versión": "Nova versió",
  "Nueva versión creada": "Nova versió creada",
  "Motivo de la nueva versión": "Motiu de la nova versió",
  "Vista previa": "Vista prèvia",
  "Esta versión se envió y todavía no tiene respuesta.":
    "Aquesta versió es va enviar i encara no té resposta.",
  "Respuesta del cliente": "Resposta del client",
  Partidas: "Partides",
  Subpartidas: "Subpartides",
  Totales: "Totals",
  "Arrastrar para reordenar": "Arrossega per reordenar",
  "Número escrito a mano — se mantiene al reordenar": "Número escrit a mà — es manté en reordenar",
  "Número automático — cambia con la posición": "Número automàtic — canvia amb la posició",
  "P. venta": "P. venda",
  "P. coste": "P. cost",
  Cantidad: "Quantitat",
  "Idioma del documento": "Idioma del document",
  "Idioma del documento actualizado": "Idioma del document actualitzat",
  "Idioma de sus documentos": "Idioma dels seus documents",
  "— idioma de la empresa —": "— idioma de l'empresa —",
  "— según el cliente —": "— segons el client —",
  "El contrato firmado conserva su idioma": "El contracte signat conserva el seu idioma",
  "La adenda aprobada conserva su idioma": "L'adenda aprovada conserva el seu idioma",
  "en el buzón": "a la bústia",
  "sin buzón — solo registrado": "sense bústia — només registrat",
  "no archivado": "no arxivat",
  "Sin buzón configurado — el mensaje queda solo registrado":
    "Sense bústia configurada — el missatge queda només registrat",
  "Borrador archivado en el buzón": "Esborrany arxivat a la bústia",
  "Correo de la empresa": "Correu de l'empresa",
  "⚠ No se pudo generar el documento:": "⚠ No s'ha pogut generar el document:",
  "Sin documentos.": "Sense documents.",
  "⚠ No se pudo generar el documento: ": "⚠ No s'ha pogut generar el document: ",
  "abra cualquiera para descargar PDF, Word o Excel":
    "obriu-ne qualsevol per descarregar PDF, Word o Excel",
  "El mismo documento en los tres formatos. Se genera al pedirlo, siempre desde los datos de hoy.":
    "El mateix document en els tres formats. Es genera en demanar-lo, sempre amb les dades d'avui.",
  "documento del proveedor": "document del proveïdor",
  "Sin presupuestos todavía.": "Encara no hi ha pressupostos.",
  "Sin contratos todavía.": "Encara no hi ha contractes.",
  "Sin proyectos todavía.": "Encara no hi ha projectes.",
  "Sin facturas todavía.": "Encara no hi ha factures.",
  "Sin recibos todavía.": "Encara no hi ha rebuts.",
  "Sin facturas recibidas.": "Sense factures rebudes.",
  "⤓ PDF": "⤓ PDF",
  "⤓ Word": "⤓ Word",
  "⚠ Sin plantilla «Envío de presupuesto» — no se ha preparado ningún correo":
    "⚠ Sense plantilla «Enviament de pressupost» — no s'ha preparat cap correu",
  Reintentar: "Torna-ho a provar",
  "Archivando…": "Arxivant…",
  "Sin servidor": "Sense servidor",
  "Sin buzón configurado": "Sense bústia configurada",
  "Los borradores se archivan en:": "Els esborranys s'arxiven a:",
  "el buzón de la empresa": "la bústia de l'empresa",
  "Buzón conectado. Hay un borrador de prueba esperando en:":
    "Bústia connectada. Hi ha un esborrany de prova esperant a:",
  "Contraseña del correo": "Contrasenya del correu",
  "Conectar el buzón": "Connecta la bústia",
  conectado: "connectada",
  "sin conectar": "sense connectar",
  "comprobando…": "comprovant…",
  "solo en el servidor": "només al servidor",
  "no se ha podido comprobar": "no s'ha pogut comprovar",
  "Buzón conectado": "Bústia connectada",
  "Todavía no se archiva ningún borrador.": "Encara no s'arxiva cap esborrany.",
  "Probando la conexión con el buzón…": "Provant la connexió amb la bústia…",
  "El buzón no ha aceptado estos datos. No se ha guardado nada.":
    "La bústia no ha acceptat aquestes dades. No s'ha desat res.",
  "No se ha podido contactar con el servidor. No se ha guardado nada.":
    "No s'ha pogut contactar amb el servidor. No s'ha desat res.",
  "Esta copia funciona sin servidor: el buzón se conecta en la instalación de la empresa.":
    "Aquesta còpia funciona sense servidor: la bústia es connecta a la instal·lació de l'empresa.",
  "Desde este buzón se preparan los borradores de todo lo que genera el programa: presupuestos, facturas, recordatorios. El programa no envía nada — deja el borrador escrito y usted lo revisa y pulsa enviar.":
    "Des d'aquesta bústia es preparen els esborranys de tot el que genera el programa: pressupostos, factures, recordatoris. El programa no envia res — deixa l'esborrany escrit i vostè el revisa i prem enviar.",
  "La contraseña se guarda cifrada en el servidor y nunca en los datos de la empresa: no viaja a los móviles, no sale en las copias de seguridad y no se puede volver a leer desde esta pantalla. Antes de guardarla se prueba conectando de verdad al buzón.":
    "La contrasenya es desa xifrada al servidor i mai a les dades de l'empresa: no viatja als mòbils, no surt a les còpies de seguretat i no es pot tornar a llegir des d'aquesta pantalla. Abans de desar-la es prova connectant de veritat a la bústia.",
  "⚠ Escriba el correo de la empresa": "⚠ Escrigui el correu de l'empresa",
  "⚠ Escriba la contraseña del correo": "⚠ Escrigui la contrasenya del correu",
  "⚠ El buzón no ha aceptado estos datos": "⚠ La bústia no ha acceptat aquestes dades",
  "borrador(es) archivado(s) en el buzón": "esborrany(s) arxivat(s) a la bústia",
  "⚠ No se pudo archivar el borrador:": "⚠ No s'ha pogut arxivar l'esborrany:",
  "⚠ No se pudo contactar con el servidor para archivar el borrador":
    "⚠ No s'ha pogut contactar amb el servidor per arxivar l'esborrany",
  "message/rfc822": "message/rfc822",
  "viendo solo": "veient només",
  "Línea con importe y sin descripción": "Línia amb import i sense descripció",
  "Excede lo que este cliente debe por esta obra":
    "Excedeix el que aquest client deu per aquesta obra",
  "Contrato sin firmar": "Contracte sense signar",
  "La primera factura de la obra necesita el contrato firmado.":
    "La primera factura de l'obra necessita el contracte signat.",
  "El abono no dice qué factura rectifica": "L'abonament no diu quina factura rectifica",
  "Un abono siempre nombra la factura que corrige.":
    "Un abonament sempre anomena la factura que corregeix.",
  "Adicional sin aprobar": "Addicional sense aprovar",
  "Sólo se factura un adicional que el cliente ya ha aprobado.":
    "Només es factura un addicional que el client ja ha aprovat.",
  "El adicional no existe": "L'addicional no existeix",
  "Cantidad cero": "Quantitat zero",
  "Precio cero (marque pendiente si procede)": "Preu zero (marqui pendent si escau)",
  "Partida con margen negativo": "Partida amb marge negatiu",
  "Línea pendiente de precio": "Línia pendent de preu",
  "Coste superior al precio de venta": "Cost superior al preu de venda",
  "× ver todas": "× veure totes",
  "(IVA incl.)": "(IVA incl.)",
  "sobre el contratado (IVA incl.)": "sobre el contractat (IVA incl.)",
  Español: "Espanyol",
  "Condiciones de pago actualizadas": "Condicions de pagament actualitzades",
  "Validez actualizada": "Validesa actualitzada",
  "Superficie actualizada": "Superfície actualitzada",
  Validar: "Validar",
  "Fuera de presupuesto": "Fora de pressupost",
  "Pendientes de precio": "Pendents de preu",
  "Por m²": "Per m²",
  "Coste y margen no salen del documento": "Cost i marge no surten del document",
  "Anexo gráfico": "Annex gràfic",
  "Este presupuesto no tiene ninguna visita asociada.":
    "Aquest pressupost no té cap visita associada.",
  "Visita de este presupuesto": "Visita d'aquest pressupost",
  "Última visita del cliente — no vinculada a este presupuesto":
    "Última visita del client — no vinculada a aquest pressupost",
  "Partida movida": "Partida moguta",
  "Subpartida movida": "Subpartida moguda",
  "Subpartida movida de partida": "Subpartida moguda de partida",
  "⚠ Ese número ya está en uso": "⚠ Aquest número ja s'utilitza",
  "⚠ No se ha podido guardar": "⚠ No s'ha pogut desar",
  "Presupuesto enviado — versión congelada": "Pressupost enviat — versió congelada",
  "Alcance contratado": "Abast contractat",
  "adicional aceptado": "addicional acceptat",
  "Este contrato se firmó fuera del sistema, así que no tiene un presupuesto aceptado del que leer el alcance. Lo contratado es lo que diga el archivo firmado.":
    "Aquest contracte es va signar fora del sistema, així que no té un pressupost acceptat del qual llegir l'abast. El que s'ha contractat és el que digui l'arxiu signat.",
  "No se ha podido leer la versión aceptada del presupuesto.":
    "No s'ha pogut llegir la versió acceptada del pressupost.",
  "El alcance sale de la versión aceptada del presupuesto: son las mismas partidas y subpartidas que leyó el cliente. Los adicionales aceptados van aparte porque el importe vigente del contrato ya los incluye.":
    "L'abast surt de la versió acceptada del pressupost: són les mateixes partides i subpartides que va llegir el client. Els addicionals acceptats van a part perquè l'import vigent del contracte ja els inclou.",
  "Motivo de la rectificación *": "Motiu de la rectificació *",
  "Por qué se rectifica": "Per què es rectifica",
  "— elige la factura a rectificar —": "— tria la factura a rectificar —",
  "Quitar la línea": "Treure la línia",
  "⤓ Descargar PDF": "⤓ Descarregar PDF",
  "Rectificar…": "Rectificar…",
  "Presupuesto aceptado — obra creada": "Pressupost acceptat — obra creada",
  "Presupuesto rechazado": "Pressupost rebutjat",
  "Lo que va a recibir el cliente": "El que rebrà el client",
  "sin correo": "sense correu",
  "Subpartidas sin precio todavía": "Subpartides sense preu encara",
  "Valoradas provisionalmente en": "Valorades provisionalment en",
  "No están incluidas en el total y no se imprimen en el documento.":
    "No estan incloses en el total i no s'imprimeixen al document.",
  "no van en el total": "no van al total",
  Bloqueos: "Bloquejos",
  Bloquea: "Bloqueja",
  Avisos: "Avisos",
  Aviso: "Avís",
  "Al enviar, esta versión queda congelada: el documento del cliente se podrá reproducir siempre tal cual salió. Para cambiar algo después hay que crear una versión nueva.":
    "En enviar, aquesta versió queda congelada: el document del client es podrà reproduir sempre tal com va sortir. Per canviar res després cal crear una versió nova.",
  "Enviar y congelar la versión": "Enviar i congelar la versió",
  "Corrija los bloqueos primero": "Corregiu primer els bloquejos",
  "Aceptar crea el proyecto y da la oportunidad por ganada. La versión queda cerrada.":
    "Acceptar crea el projecte i dóna l'oportunitat per guanyada. La versió queda tancada.",
  "Rechazar da la oportunidad por perdida con el motivo elegido, que es el mismo que cuentan los informes de conversión.":
    "Rebutjar dóna l'oportunitat per perduda amb el motiu triat, el mateix que compten els informes de conversió.",
  Justificante: "Justificant",
  "Marcar como aceptado": "Marcar com a acceptat",
  "Marcar como rechazado": "Marcar com a rebutjat",
  "Qué dijo": "Què va dir",
  Exclusiones: "Exclusions",
  " (opcional — aparte)": " (opcional — a part)",
  " (fuera de presupuesto)": " (fora de pressupost)",
  " (extras)": " (extres)",
  Supuestos: "Supòsits",
  "Exclusiones y supuestos": "Exclusions i supòsits",
  "Lo que el precio NO incluye. Se imprime bajo los totales.":
    "El que el preu NO inclou. S'imprimeix sota els totals.",
  "Aquello que se ha dado por cierto al valorar; si cambia, cambia el precio.":
    "Allò que s'ha donat per cert en valorar; si canvia, canvia el preu.",
  "Ninguna todavía.": "Cap encara.",
  "Escribe y añade": "Escriu i afegeix",
  "Todavía no hay curva planificada contra la que medir: la obra empieza el":
    "Encara no hi ha corba planificada contra la qual mesurar: l'obra comença el",
  "La versión está congelada: sus condiciones se conservan tal como se enviaron.":
    "La versió està congelada: les seves condicions es conserven tal com es van enviar.",
  "Sin partidas todavía.": "Sense partides encara.",
  "Ninguna subpartida tiene imágenes todavía.": "Cap subpartida té imatges encara.",
  "Anexo gráfico activado": "Annex gràfic activat",
  "Anexo gráfico desactivado": "Annex gràfic desactivat",
  "Anexo actualizado": "Annex actualitzat",
  "Ver anexo gráfico": "Veure l'annex gràfic",
  Añadido: "Afegit",
  "40% a la firma, 60% a la entrega": "40% a la signatura, 60% al lliurament",
  Anexo: "Annex",
  versiones: "versions",
  imágenes: "imatges",
  activo: "actiu",
  desactivado: "desactivat",
  // ── ADM-03: document capture and validation (S6).
  "＋ Capturar documento": "＋ Capturar document",
  Cámara: "Càmera",
  "Preparar para trabajar sin cobertura": "Preparar per treballar sense cobertura",
  "Un PDF con texto se lee al instante y sin descargar nada. Una foto o un escaneado necesitan el lector de imágenes: unos 7 MB que se descargan la primera vez que hagan falta. Prepárelo con wifi si va a trabajar en una obra sin cobertura.":
    "Un PDF amb text es llegeix a l'instant i sense descarregar res. Una foto o un escaneig necessiten el lector d'imatges: uns 7 MB que es descarreguen el primer cop que calguin. Prepareu-ho amb wifi si heu de treballar en una obra sense cobertura.",
  "Descargando el lector de documentos…": "Descarregant el lector de documents…",
  "Listo para trabajar sin cobertura": "Llest per treballar sense cobertura",
  "Leyendo el documento…": "Llegint el document…",
  "Validar documento": "Validar document",
  "Volver a la bandeja": "Tornar a la safata",
  "Por revisar": "Per revisar",
  "Confirmar y guardar": "Confirmar i desar",
  "Documento guardado": "Document desat",
  "Leído del texto del PDF": "Llegit del text del PDF",
  "Datos leídos": "Dades llegides",
  Comprobado: "Comprovat",
  "Sin comprobar — revíselo": "Sense comprovar — reviseu-ho",
  "Un punto verde significa que algo ha comprobado el dato — un dígito de control, una fecha real, una suma que cuadra. Ámbar significa que nadie lo ha comprobado, por muy claro que se leyera.":
    "Un punt verd significa que alguna cosa ha comprovat la dada — un dígit de control, una data real, una suma que quadra. Ambre significa que ningú no ho ha comprovat, per clar que es llegís.",
  "No se ha leído nada del documento. Escriba los datos a mano; la imagen queda adjunta igualmente.":
    "No s'ha llegit res del document. Escriviu les dades a mà; la imatge queda adjunta igualment.",
  "Preparando la vista…": "Preparant la vista…",
  "No se ha podido mostrar el documento.": "No s'ha pogut mostrar el document.",
  "No se ha podido leer el documento": "No s'ha pogut llegir el document",
  "⚠ El lector de documentos no está disponible": "⚠ El lector de documents no està disponible",
  "⚠ El lector de documentos no está disponible — introdúzcalo a mano":
    "⚠ El lector de documents no està disponible — introduïu-ho a mà",
  "⚠ No se ha podido leer nada: rellénelo a mano": "⚠ No s'ha pogut llegir res: ompliu-ho a mà",
  "⚠ No se ha podido leer el documento: rellénelo a mano":
    "⚠ No s'ha pogut llegir el document: ompliu-ho a mà",
  "Sin lectura — escríbalo a mano": "Sense lectura — escriviu-ho a mà",
  "Ver el otro": "Veure l'altre",
  "Eliminar documento": "Eliminar document",
  "Eliminar el documento": "Eliminar el document",
  "Documento eliminado": "Document eliminat",
  "Ya hay otro documento de este emisor con el mismo número":
    "Ja hi ha un altre document d'aquest emissor amb el mateix número",
  "Registrado como factura: elimínala primero": "Registrat com a factura: elimina-la primer",
  "Se borra el documento y su imagen. No afecta a ninguna factura":
    "S'esborra el document i la seva imatge. No afecta cap factura",
  "Se borra el documento archivado y su imagen. No se puede deshacer.":
    "S'esborra el document arxivat i la seva imatge. No es pot desfer.",
  "Se borra el documento archivado, su imagen y su asignación a la obra. No se puede deshacer.":
    "S'esborra el document arxivat, la seva imatge i l'assignació a l'obra. No es pot desfer.",
  "⚠ El reparto debe sumar la base imponible": "⚠ El repartiment ha de sumar la base imposable",
  "Se reparte la base imponible": "Es reparteix la base imposable",
  "El IVA no se reparte: es IVA soportado, no coste de obra":
    "L'IVA no es reparteix: és IVA suportat, no cost d'obra",
  "La obra está vinculada a un contrato sin firmar, y hay otro ya firmado para este presupuesto: vincúlelo a la obra desde el contrato.":
    "L'obra està vinculada a un contracte sense signar, i n'hi ha un altre ja signat per a aquest pressupost: vinculeu-lo a l'obra des del contracte.",
  "Nº documento": "Núm. document",
  Retención: "Retenció",
  "Factura de autónomo": "Factura d'autònom",
  "Confirmación de pedido": "Confirmació de comanda",
  "Justificante de pago": "Justificant de pagament",
  // ── ADM-05 Consolidación bancaria and ADM-06 Caja chica (S11).
  "Entradas y salidas de la caja en efectivo, y el arqueo que las prueba. Una salida sin justificante se cuenta aparte: es el agujero clásico en las cuentas de una empresa pequeña.":
    "Entrades i sortides de la caixa en efectiu, i l'arqueig que les prova. Una sortida sense justificant es compta a part: és el forat clàssic als comptes d'una empresa petita.",
  "＋ Entrada": "＋ Entrada",
  "− Salida": "− Sortida",
  "en caja": "a caixa",
  "sin justificante": "sense justificant",
  "Todo justificado": "Tot justificat",
  "Marcar justificado": "Marca com a justificat",
  "Justificante registrado": "Justificant registrat",
  "Sin movimientos de caja en el periodo.": "Sense moviments de caixa al període.",
  Entradas: "Entrades",
  Salidas: "Sortides",
  "Saldo final": "Saldo final",
  "No hay ninguna caja en efectivo configurada.": "No hi ha cap caixa en efectiu configurada.",
  "Entrada de caja": "Entrada de caixa",
  "Salida de caja": "Sortida de caixa",
  "Reposición de caja": "Reposició de caixa",
  "Ferretería del barrio": "Ferreteria del barri",
  "Falta el justificante": "Falta el justificant",
  Registrar: "Registra",
  "Entrada registrada": "Entrada registrada",
  "Salida registrada": "Sortida registrada",
  "Indica el importe": "Indiqueu l'import",
  "El concepto es obligatorio": "El concepte és obligatori",
  "Saldos, movimientos y clasificación. La clase y el destino se editan en la propia fila; lo que sigue sin casar lleva una barra ámbar y dice cuántos días lleva ahí.":
    "Saldos, moviments i classificació. La classe i el destí s'editen a la mateixa fila; el que continua sense casar porta una barra ambre i diu quants dies fa que hi és.",
  "Todas las cuentas": "Tots els comptes",
  "en la cuenta seleccionada": "al compte seleccionat",
  "en todas las cuentas": "a tots els comptes",
  "total disponible": "total disponible",
  "sin casar": "sense casar",
  "Todo casado": "Tot casat",
  Casado: "Casat",
  Antigüedad: "Antiguitat",
  "Sin movimientos en el periodo.": "Sense moviments al període.",
  "No hay cuentas configuradas.": "No hi ha comptes configurats.",
  "Movimiento clasificado": "Moviment classificat",
  "Movimiento asignado": "Moviment assignat",
  "Ir a Conciliación →": "Ves a Conciliació →",
  "Días sin casar": "Dies sense casar",
  // ── PK7-C: la cua pagina i l'emparellament passa en un plafó al costat.
  "La cola de movimientos sin explicar, diez a diez. Abre uno para ver qué documento lo explica; el coste de obra se asigna en Gastos.":
    "La cua de moviments sense explicar, de deu en deu. Obre'n un per veure quin document l'explica; el cost d'obra s'assigna a Despeses.",
  "Conciliar movimiento": "Concilia el moviment",
  Conciliados: "Conciliats",
  "Quedan movimientos sin conciliar — no se puede cerrar el periodo":
    "Queden moviments sense conciliar — no es pot tancar el període",
  // ── PK7-D: Conciliats, les vies de tornada i els traspassos com a parells.
  "Lo que ya está explicado, y cómo. Cada línea puede deshacerse: deshacer una conciliación deshace también el pago que creó, y no toca el coste de obra.":
    "El que ja està explicat, i com. Cada línia es pot desfer: desfer una conciliació desfà també el pagament que va crear, i no toca el cost d'obra.",
  "Buscar por concepto, importe o explicación…": "Cerca per concepte, import o explicació…",
  "Todavía no hay nada explicado en este periodo.":
    "Encara no hi ha res explicat en aquest període.",
  "movimientos explicados en el periodo": "moviments explicats al període",
  Explicación: "Explicació",
  "↩ Deshacer": "↩ Desfés",
  "Deshacer la explicación": "Desfés l'explicació",
  "Reabre el periodo": "Reobre el període",
  "Conciliados →": "Conciliats →",
  "← Volver a la cola": "← Torna a la cua",
  "Un traspaso son dos movimientos. Deshacerlo devuelve LOS DOS a la cola.":
    "Un traspàs són dos moviments. Desfer-lo torna ELS DOS a la cua.",
  "Traspaso deshecho — los dos movimientos vuelven a la cola":
    "Traspàs desfet — els dos moviments tornen a la cua",
  "Traspaso marcado — los dos movimientos salen de la cola":
    "Traspàs marcat — els dos moviments surten de la cua",
  "Traspasos entre cuentas propias": "Traspassos entre comptes propis",
  "Marcar como traspaso": "Marca com a traspàs",
  "Marcar los": "Marca els",
  seguros: "segurs",
  "movimientos encajaban igual de bien. Compruébalo antes de marcarlo.":
    "moviments encaixaven igual de bé. Comprova-ho abans de marcar-lo.",
  "Con justificante": "Amb justificant",
  Clasificado: "Classificat",
  "importe opuesto exacto": "import oposat exacte",
  "importe casi opuesto": "import quasi oposat",
  "cuentas distintas": "comptes diferents",
  "＋ Añadir gasto": "＋ Afegeix despesa",
  // ── PK7-E: tancar el trimestre — selecciona'n molts, una sola acció.
  "Seleccionar toda esta página": "Selecciona tota aquesta pàgina",
  seleccionados: "seleccionats",
  Vaciar: "Buida",
  "Clasificar en bloque": "Classifica en bloc",
  "Los movimientos seleccionados se identifican como gasto general, con el motivo elegido. Cada uno puede deshacerse por separado desde Conciliados.":
    "Els moviments seleccionats s'identifiquen com a despesa general, amb el motiu triat. Cada un es pot desfer per separat des de Conciliats.",
  "Los movimientos seleccionados se marcan sin respaldo, con una tarea para reclamar el justificante de cada uno. Cada uno puede deshacerse por separado desde Conciliados.":
    "Els moviments seleccionats es marquen sense justificant, amb una tasca per reclamar el justificant de cada un. Cada un es pot desfer per separat des de Conciliats.",
  "Elige primero la categoría de gasto general.": "Tria primer la categoria de despesa general.",
  // Els noms dels registres, ara claus estables.
  clientes: "clients",
  documento: "document",
  documentos: "documents",
  industrial: "industrial",
  industriales: "industrials",
  oportunidades: "oportunitats",
  proveedores: "proveïdors",
  trabajadores: "treballadors",
  "visita programada": "visita programada",
  "visitas programadas": "visites programades",
  "visita realizada": "visita realitzada",
  "visitas realizadas": "visites realitzades",
  movimiento: "moviment",
  movimientos: "moviments",
  // Els motius de l'emparellador, mai al diccionari fins ara.
  "importe casi exacto": "import quasi exacte",
  "misma fecha": "mateixa data",
  "referencia citada": "referència citada",
  "tercero coincidente": "tercer coincident",
  "signo correcto": "signe correcte",
  difiere: "difereix",
  "Buscar por concepto, importe o quién…": "Cerca per concepte, import o qui…",
  "Buscar documento por número, quién o importe…": "Cerca el document per número, qui o import…",
  "Sin explicar": "Sense explicar",
  "Sin respaldo": "Sense justificant",
  "Sin documento": "Sense document",
  "mismo día": "mateix dia",
  "importe exacto": "import exacte",
  "tarjeta citada": "targeta citada",
  "periodo en el concepto": "període en el concepte",
  Liquidación: "Liquidació",
  "movimientos entre": "moviments entre",
  "Liquidación de tarjeta registrada": "Liquidació de targeta registrada",
  "Pagar la tarjeta no es un gasto: lo son las compras que ya están en su cuenta. Se marca como traspaso interno.":
    "Pagar la targeta no és una despesa: ho són les compres que ja són al seu compte. Es marca com a traspàs intern.",
  "pago parcial": "pagament parcial",
  "más. Busca para acotar.": "més. Cerca per acotar.",
  "El extracto no nombra a la contraparte. Compruébalo antes de conciliar.":
    "L'extracte no anomena la contrapart. Comprova-ho abans de conciliar.",
  "Ningún documento explica este movimiento. Búscalo abajo o explícalo sin factura.":
    "Cap document explica aquest moviment. Cerca'l a sota o explica'l sense factura.",
  // ── A2: el que queda, ¿es deu encara o es dóna per tancat?
  "¿El resto sigue pendiente?": "La resta continua pendent?",
  "este documento": "aquest document",
  "estos documentos": "aquests documents",
  "Sigue debiendo": "Encara deu",
  "Sigue pendiente": "Continua pendent",
  "Se da por cerrado": "Es dóna per tancat",
  "Cerrado con su motivo": "Tancat amb el seu motiu",
  "Motivos de cierre (diferencias)": "Motius de tancament (diferències)",
  "Descuento por pronto pago": "Descompte per pagament ràpid",
  Redondeo: "Arrodoniment",
  "Abono pendiente del proveedor": "Abonament pendent del proveïdor",
  // ── PK7-A: l'extracte demostra la seva pròpia aritmètica abans d'importar-se.
  "Saldo inicial del extracto": "Saldo inicial de l'extracte",
  "Saldo final del extracto": "Saldo final de l'extracte",
  "El extracto no cuadra consigo mismo: el saldo inicial más los movimientos no dan el saldo final. No se importa nada — puede faltar alguna fila o haberse leído mal una columna.":
    "L'extracte no quadra amb si mateix: el saldo inicial més els moviments no donen el saldo final. No s'importa res — pot faltar alguna fila o s'ha llegit malament una columna.",
  "Este extracto no trae columna de saldo, así que no se puede comprobar que esté completo.":
    "Aquest extracte no porta columna de saldo, així que no es pot comprovar que estigui complet.",
  // ── PK7-B: el pantalla de saldos deixa de classificar, la cua identifica.
  "Cuántas cuentas hay y cuánto hay en cada una. Los movimientos se explican en Conciliación; el coste de obra se asigna en Gastos.":
    "Quants comptes hi ha i quants diners hi ha a cadascun. Els moviments s'expliquen a Conciliació; el cost d'obra s'assigna a Despeses.",
  "Últimos movimientos": "Darrers moviments",
  "Movimientos del periodo": "Moviments del període",
  "Sin movimientos todavía.": "Encara no hi ha moviments.",
  "Sin movimientos en este periodo.": "Cap moviment en aquest període.",
  "Ir a Gastos": "Anar a Despeses",
  "Nombre o razón social": "Nom o raó social",
  "Abrir su ficha": "Obre la seva fitxa",
  "Lo que dice el documento": "El que diu el document",
  "Dar de alta el proveedor": "Dona d'alta el proveïdor",
  "Si el documento lo trae": "Si el document el porta",
  "Reparta antes el coste: a una obra o a gastos generales":
    "Repartiu abans el cost: a una obra o a despeses generals",
  "El NIF del emisor falta o no es válido": "Falta el NIF de l'emissor o no és vàlid",
  "El documento no dice quién lo emite": "El document no diu qui l'emet",
  "El NIF del emisor falta o no es válido: corríjalo en el documento":
    "Falta el NIF de l'emissor o no és vàlid: corregiu-lo al document",
  "⚠ No se ha podido leer el documento: escríbalo a mano":
    "⚠ No s'ha pogut llegir el document: escriviu-lo a mà",
  "⚠ Elija el proveedor al que pertenece el documento":
    "⚠ Trieu el proveïdor a qui pertany el document",
  "Factura registrada — ya se puede conciliar": "Factura registrada — ja es pot conciliar",
  "Factura registrada — el coste entra en la obra y ya se puede conciliar":
    "Factura registrada — el cost entra a l'obra i ja es pot conciliar",
  "Alta de proveedor desde documento": "Alta de proveïdor des d'un document",
  "Falta el emisor: sin él no se puede dar de alta el proveedor":
    "Falta l'emissor: sense ell no es pot donar d'alta el proveïdor",
  "El NIF del emisor falta o no es válido: compruébelo en el documento":
    "Falta el NIF de l'emissor o no és vàlid: comproveu-lo al document",
  "En el fichero": "Al fitxer",
  "Se dará de alta": "Es donarà d'alta",
  "Ya está en el fichero": "Ja és al fitxer",
  "El NIF no es válido": "El NIF no és vàlid",
  "Compruébelo en el documento: el dígito de control no cuadra.":
    "Comproveu-ho al document: el dígit de control no quadra.",
  "revíselo antes de darlo de alta": "reviseu-ho abans de donar-lo d'alta",
  "El emisor no está en el fichero": "L'emissor no és al fitxer",
  "Darlo de alta con lo leído": "Dona'l d'alta amb el que s'ha llegit",
  "Es uno que ya está dado de alta": "És un que ja està donat d'alta",
  "＋ Alta desde documento": "＋ Alta des d'un document",
  "Sin conciliar": "Sense conciliar",
  Explicado: "Explicat",
  "Movimientos por explicar": "Moviments per explicar",
  "Conciliar →": "Concilia →",
  "Identificar como…": "Identifica com a…",
  Identificar: "Identifica",
  "Elige primero qué es este movimiento.": "Tria primer què és aquest moviment.",
  "Movimiento identificado": "Moviment identificat",
  "Saldo inicial": "Saldo inicial",
  // ── ADM-01 Facturación (S10).
  "Facturas emitidas desde el presupuesto aceptado, los hitos del contrato o los extras aprobados. Numeración sin huecos, las emitidas son inmutables, y un día de retraso ya se ve en rojo.":
    "Factures emeses des del pressupost acceptat, les fites del contracte o els extres aprovats. Numeració sense buits, les emeses són immutables, i un dia de retard ja es veu en vermell.",
  facturas: "factures",
  "Ninguna factura coincide con la búsqueda.": "Cap factura coincideix amb la cerca.",
  "Días de retraso": "Dies de retard",
  "Retención IRPF": "Retenció IRPF",
  Forma: "Forma",
  "Indica el importe cobrado": "Indiqueu l'import cobrat",
  "Un cobro parcial deja el resto pendiente y el contador de vencido lo sigue contando.":
    "Un cobrament parcial deixa la resta pendent i el comptador de vençut el continua comptant.",
  // Four more from the ES/EN spine that are on this screen.
  Vencido: "Vençut",
  factura: "factura",
  Saldo: "Saldo",
  Vencimiento: "Venciment",
  // ── COM-04 Contrato and PRY-03 Adicionales (S9).
  "Contratos vigentes e históricos. El importe vigente se marca en ámbar cuando ya no coincide con el original: significa que hay anexos.":
    "Contractes vigents i històrics. L'import vigent es marca en ambre quan ja no coincideix amb l'original: vol dir que hi ha annexos.",
  Vigentes: "Vigents",
  Históricos: "Històrics",
  contrato: "contracte",
  contratos: "contractes",
  "Buscar por número, cliente u obra…": "Cerca per número, client o obra…",
  "Ningún contrato coincide con la búsqueda.": "Cap contracte coincideix amb la cerca.",
  "Importe original": "Import original",
  "Importe vigente": "Import vigent",
  Anexos: "Annexos",
  Català: "Català",
  Plazo: "Termini",
  "Sin anexos. El importe vigente es el original.": "Sense annexos. L'import vigent és l'original.",
  "Total anexos": "Total annexos",
  "Suma de hitos": "Suma de fites",
  "de la planificación": "de la planificació",
  "cuadra con el contratado": "quadra amb el contractat",
  "Una fecha marcada «de la planificación» la movió el calendario de obra, no una persona. Una fecha fija y un hito ya facturado nunca se mueven solos.":
    "Una data marcada «de la planificació» l'ha moguda el calendari d'obra, no una persona. Una data fixa i una fita ja facturada no es mouen mai soles.",
  "Este contrato no tiene hitos de pago.": "Aquest contracte no té fites de pagament.",
  anexo: "annex",
  anexos: "annexos",
  "Extras y modificaciones: detectados, valorados, aprobados, ejecutados y facturados. El trabajo no aprobado nunca es facturable — la barra ámbar lo dice desde lejos.":
    "Extres i modificacions: detectats, valorats, aprovats, executats i facturats. La feina no aprovada mai és facturable — la barra ambre ho diu des de lluny.",
  Identificado: "Identificat",
  extra: "extra",
  extras: "extres",
  "Sin aprobar": "Sense aprovar",
  "Extras y modificaciones de todas las obras. El trabajo no aprobado nunca es facturable — la barra ámbar lo dice desde lejos.":
    "Extres i modificacions de totes les obres. El treball no aprovat mai és facturable — la barra ambre ho diu des de lluny.",
  Todas: "Totes",
  Críticas: "Crítiques",
  Altas: "Altes",
  Medias: "Mitjanes",
  Permisos: "Permisos",
  Detectada: "Detectada",
  Valorada: "Valorada",
  Ejecutada: "Executada",
  Facturada: "Facturada",
  "Buscar por descripción, obra o partida": "Cerca per descripció, obra o partida",
  modificación: "modificació",
  modificaciones: "modificacions",
  "Ninguna modificación en esta etapa.": "Cap modificació en aquesta etapa.",
  "Ninguna modificación coincide con la búsqueda.": "Cap modificació coincideix amb la cerca.",
  Modificación: "Modificació",
  Adenda: "Addenda",
  "Modificación ·": "Modificació ·",
  "Al 10 % de avance": "Al 10 % d'avanç",
  "Al 20 % de avance": "Al 20 % d'avanç",
  "Al 30 % de avance": "Al 30 % d'avanç",
  "Al 40 % de avance": "Al 40 % d'avanç",
  "Al 50 % de avance": "Al 50 % d'avanç",
  "Al 60 % de avance": "Al 60 % d'avanç",
  "Al 70 % de avance": "Al 70 % d'avanç",
  "Al 80 % de avance": "Al 80 % d'avanç",
  "Al 90 % de avance": "Al 90 % d'avanç",
  vencida: "vençuda",
  "en curso": "en curs",
  previsto: "previst",
  "Vincular obra": "Vincular obra",
  Vincular: "Vincular",
  "Obra vinculada": "Obra vinculada",
  "El contrato pasa a gobernar esa obra: la firma, los anexos, los cobros previstos y el importe contratado de la torre de control leen este vínculo.":
    "El contracte passa a governar aquesta obra: la signatura, els annexos, els cobraments previstos i l'import contractat de la torre de control llegeixen aquest vincle.",
  "No hay obras abiertas sin contrato. Una obra ya vinculada conserva su contrato.":
    "No hi ha obres obertes sense contracte. Una obra ja vinculada conserva el seu contracte.",
  "Sin aprobar: este trabajo todavía no es facturable.":
    "Sense aprovar: aquest treball encara no és facturable.",
  "Nuevo presupuesto de variación": "Nou pressupost de variació",
  "Obra *": "Obra *",
  "Se abre el presupuestador con una versión nueva sobre esa obra. Al aceptarse, sus partidas entran en la economía, el avance y la certificación.":
    "S'obre el pressupostador amb una versió nova sobre aquesta obra. En acceptar-se, les seves partides entren a l'economia, l'avanç i la certificació.",
  "Crear y abrir": "Crear i obrir",
  "No hay obras abiertas sobre las que crear un presupuesto de variación.":
    "No hi ha obres obertes sobre les quals crear un pressupost de variació.",
  "La obra es obligatoria": "L'obra és obligatòria",
  "＋ Nueva modificación": "＋ Nova modificació",
  Modificaciones: "Modificacions",
  "no facturable sin aprobación": "no facturable sense aprovació",
  "Ningún extra en esta etapa.": "Cap extra en aquesta etapa.",
  "Sin modificaciones.": "Sense modificacions.",
  Foto: "Foto",
  "Hacer foto": "Fes una foto",
  "Sin foto": "Sense foto",
  "No se ha podido guardar la foto": "No s'ha pogut desar la foto",
  "Foto adjunta": "Foto adjunta",
  "Nueva modificación / extra": "Nova modificació / extra",
  "Registrar como detectada": "Registra com a detectada",
  "Extra registrado": "Extra registrat",
  // ── PRY-01 Avance físico and PRY-02 Avance económico (S8).
  "Cada obra conserva su línea base aceptada. Marca el estado de cada partida — sin empezar, en ejecución o terminado — y el porcentaje sólo cuando esté en ejecución.":
    "Cada obra conserva la seva línia base acceptada. Marqueu l'estat de cada partida — sense començar, en execució o acabat — i el percentatge només quan estigui en execució.",
  "Buscar por código o cliente…": "Cerca per codi o client…",
  "Ninguna obra coincide con la búsqueda.": "Cap obra coincideix amb la cerca.",
  obras: "obres",
  "Fin comprometido": "Fi compromès",
  "Fin real": "Fi real",
  Ficha: "Fitxa",
  "En ejecución": "En execució",
  "Esta obra no tiene partidas aceptadas: el avance se registra sobre las tareas del gráfico.":
    "Aquesta obra no té partides acceptates: l'avanç es registra sobre les tasques del gràfic.",
  "Avance registrado": "Avanç registrat",
  "Fin de obra previsto": "Fi d'obra previst",
  "Desvío sobre línea base": "Desviació sobre la línia base",
  "Tareas fuera de plazo": "Tasques fora de termini",
  "Abrir la carta Gantt": "Obre la carta Gantt",
  "Recalcular los cobros previstos": "Recalcula els cobraments previstos",
  "La carta Gantt se abre a pantalla completa: arrastrar una barra, alargarla o unir dos tareas necesita la anchura entera.":
    "La carta Gantt s'obre a pantalla completa: arrossegar una barra, allargar-la o unir dues tasques necessita tota l'amplada.",
  "Línea de negocio": "Línia de negoci",
  Cobrado: "Cobrat",
  "Por facturar": "Per facturar",
  "Extras aprobados": "Extres aprovats",
  "Venta, coste y margen de la obra, partida a partida, y el coste que ha llegado a la obra sin partida asignada. Repartirlo aquí es lo que hace que la tabla cuadre con el proyecto.":
    "Venda, cost i marge de l'obra, partida a partida, i el cost que ha arribat a l'obra sense partida assignata. Repartir-lo aquí és el que fa que la taula quadri amb el projecte.",
  "contratada más extras aprobados": "contractada més extres aprovats",
  comprometido: "compromès",
  "sobre la venta": "sobre la venda",
  "Por partida": "Per partida",
  Acumulado: "Acumulat",
  "Esta obra no tiene partidas en su línea base.":
    "Aquesta obra no té partides a la seva línia base.",
  "Pendiente de repartir": "Pendent de repartir",
  nada: "res",
  Repartir: "Reparteix",
  "Todo el coste de esta obra tiene partida. La tabla de arriba cuadra con el proyecto.":
    "Tot el cost d'aquesta obra té partida. La taula de dalt quadra amb el projecte.",
  Ajustar: "Ajusta",
  "La proyección de cada partida parte del coste real y del avance registrado. «Ajustar» la sustituye a mano, y la cifra calculada se sigue mostrando junto a la ajustada.":
    "La projecció de cada partida parteix del cost real i de l'avanç registrat. «Ajusta» la substitueix a mà, i la xifra calculada es continua mostrant al costat de l'ajustada.",
  "＋ Otra partida": "＋ Una altra partida",
  "Cuadra con el coste": "Quadra amb el cost",
  "Un parte de horas va a una sola partida: son las horas de una persona en un día, y partirlas aquí sería inventar un segundo parte.":
    "Un part d'hores va a una sola partida: són les hores d'una persona en un dia, i partir-les aquí seria inventar un segon part.",
  "Coste repartido por partida": "Cost repartit per partida",
  "Sin partida": "Sense partida",
  "Esta obra no tiene contrato, así que no hay hitos de cobro que recalcular.":
    "Aquesta obra no té contracte, així que no hi ha fites de cobrament per recalcular.",
  "Lo que dice la planificación": "El que diu la planificació",
  "hito se mueve": "fita es mou",
  "hitos se mueven": "fites es mouen",
  Prevista: "Prevista",
  "Pasaría a": "Passaria a",
  "A la firma": "A la signatura",
  "Al inicio de obra": "A l'inici d'obra",
  "Al llegar a una fase": "En arribar a una fase",
  "A la finalización": "A la finalització",
  "Fecha fija": "Data fixa",
  facturado: "facturat",
  "fecha fija": "data fixa",
  "sin cambio": "sense canvi",
  "Un hito ya facturado no se mueve, y una fecha fija tampoco: es lo que el cliente firmó. El resto sigue a la planificación, y la previsión de caja lo nota en cuanto se guarda.":
    "Una fita ja facturada no es mou, i una data fixa tampoc: és el que el client va signar. La resta segueix la planificació, i la previsió de caixa ho nota tan bon punt es desa.",
  "Aplicar a la previsión": "Aplica a la previsió",
  "Cobros previstos actualizados": "Cobraments previstos actualitzats",
  "No hay ningún hito que mover": "No hi ha cap fita per moure",
  "Volver a la obra": "Torna a l'obra",
  // Eight more that were on the ES/EN spine with no Catalan and are on S8's
  // two panels — the ratchet comes down again rather than merely holding.
  obra: "obra",
  Terminado: "Acabat",
  "Ruta crítica": "Ruta crítica",
  Presupuesto: "Pressupost",
  Desviación: "Desviació",
  Factura: "Factura",
  Hito: "Fita",
  Disparador: "Activador",
  // ── The shared list primitive's chrome and ADM-02's two surviving blocks,
  //    both found untranslated in S7 by driving its screens under CA and EN.
  "⬇ Exportar": "⬇ Exporta",
  "Excel (.xlsx)": "Excel (.xlsx)",
  "CSV (.csv)": "CSV (.csv)",
  "Filas por pantalla": "Files per pantalla",
  "‹ Anterior": "‹ Anterior",
  "Siguiente ›": "Següent ›",
  "＋ Nuevo": "＋ Nou",
  Necesidades: "Necessitats",
  "lo comprometido frente a lo pendiente, por partida":
    "el compromès davant del pendent, per partida",
  "Calendario de llegadas": "Calendari d'arribades",
  "Sin llegadas previstas.": "Sense arribades previstes.",
  "Todas las partidas están comprometidos.": "Totes les partides estan compromeses.",
  "＋ Nueva orden de compra": "＋ Nova comanda de compra",
  "＋ OC": "＋ CC",
  Presupuestado: "Pressupostat",
  "Buscar…": "Cerca…",
  // ── ADM-03 the inbox beside the register, ADM-02 compras (S7).
  Documentos: "Documents",
  Bandeja: "Safata",
  "Nada pendiente de asignar.": "Res pendent d'assignar.",
  "Emisor por confirmar": "Emissor per confirmar",
  "Buscar por emisor, número o referencia…": "Cerca per emissor, número o referència…",
  "Ningún documento coincide con la búsqueda.": "Cap document coincideix amb la cerca.",
  Archivo: "Arxiu",
  "de dónde viene y con qué se relaciona": "d'on ve i amb què es relaciona",
  "Ruta de origen": "Ruta d'origen",
  "Guardar datos de archivo": "Desa les dades d'arxiu",
  "＋ Repartir": "＋ Reparteix",
  Asignar: "Assignar",
  Destino: "Destí",
  Obras: "Obres",
  "Gastos generales": "Despeses generals",
  Naturaleza: "Naturalesa",
  "Importe (€)": "Import (€)",
  "Posible duplicado": "Possible duplicat",
  "ya hay un documento con el mismo NIF y número":
    "ja hi ha un document amb el mateix NIF i número",
  "Cuadra con el documento": "Quadra amb el document",
  "Sin repartir": "Sense repartir",
  "Documento sin total confirmado": "Document sense total confirmat",
  "Nº de pedido, obra o expediente": "Núm. de comanda, obra o expedient",
  "Carpeta o archivo del que procede": "Carpeta o arxiu del qual prové",
  "Lo que la siguiente persona necesita saber": "El que la següent persona necessita saber",
  Total: "Total",
  NIF: "NIF",
  Vehículos: "Vehicles",
  Combustible: "Combustible",
  Marketing: "Màrqueting",
  "Gastos financieros": "Despeses financeres",
  Inmovilizado: "Immobilitzat",
  Renting: "Rènting",
  "Otros gastos generales": "Altres despeses generals",
  Material: "Material",
  Otros: "Altres",
  "Datos de archivo guardados": "Dades d'arxiu desades",
  "Documento asignado": "Document assignat",
  "Documento actualizado": "Document actualitzat",
  "Ofertas, pedidos y facturado por obra. Cada orden se abre junto a la oferta del proveedor para comprobarla contra lo pedido.":
    "Ofertes, comandes i facturat per obra. Cada comanda s'obre al costat de l'oferta del proveïdor per comprovar-la amb el que s'ha demanat.",
  Pedido: "Comanda",
  Anulada: "Anul·lada",
  orden: "comanda",
  órdenes: "comandes",
  "Buscar por número, proveedor o descripción…": "Cerca per número, proveïdor o descripció…",
  "Ninguna orden coincide con la búsqueda.": "Cap comanda coincideix amb la cerca.",
  "Volver a la lista": "Torna a la llista",
  "Cambiar documento": "Canvia el document",
  "Adjuntar documento": "Adjunta un document",
  "Sin la oferta del proveedor. Captúrela en Facturas de proveedores y adjúntela aquí para comprobar el pedido contra lo que ofreció.":
    "Sense l'oferta del proveïdor. Captureu-la a Factures de proveïdors i adjunteu-la aquí per comprovar la comanda amb el que va oferir.",
  "El documento adjunto ya no tiene archivo.": "El document adjunt ja no té arxiu.",
  "El archivo no está en este dispositivo.": "L'arxiu no és en aquest dispositiu.",
  "Llegada prevista": "Arribada prevista",
  "Cant.": "Quant.",
  Precio: "Preu",
  IVA: "IVA",
  Devuelto: "Retornat",
  Conciliada: "Conciliada",
  Diferencias: "Diferències",
  "Enviar al proveedor": "Envia al proveïdor",
  "Fecha de llegada": "Data d'arribada",
  "Registrar aceptación": "Registra l'acceptació",
  "Cantidad recibida": "Quantitat rebuda",
  "Registrar recepción": "Registra la recepció",
  Duplicar: "Duplica",
  Anular: "Anul·la",
  "Importe a devolver (€)": "Import a retornar (€)",
  "Registrar devolución": "Registra la devolució",
  "Documento capturado": "Document capturat",
  "— sin documento —": "— sense document —",
  "Se adjunta un documento ya capturado, no se sube otra copia: así la lectura, la fecha y el origen del archivo siguen siendo los mismos en las dos pantallas.":
    "S'adjunta un document ja capturat, no se'n puja una altra còpia: així la lectura, la data i l'origen de l'arxiu continuen sent els mateixos a les dues pantalles.",
  // Eight entries that were on the ES/EN spine with no Catalan and are now on
  // screen in S7's two panels. The backlog is a ratchet, so closing them here
  // lowers its ceiling rather than merely holding it.
  Referencia: "Referència",
  Quitar: "Treu",
  Alquiler: "Lloguer",
  Seguros: "Assegurances",
  Subcontrata: "Subcontracta",
  Sí: "Sí",
  Descripción: "Descripció",
  Guardar: "Desa",
  // ── S12: the three new admin surfaces, plus entries they put on screen
  //    that were still on the ES/EN spine with no Catalan.
  "Parte diario": "Part diari",
  "Trimestre y contenido": "Trimestre i contingut",
  "IVA, IRPF y envío": "IVA, IRPF i enviament",
  "Por proyecto y partida": "Per projecte i partida",
  "Conciliación del mes": "Conciliació del mes",
  "Horas imputadas a obra": "Hores imputades a obra",
  "Nóminas pagadas por banco": "Nòmines pagades pel banc",
  "Diferencia sin imputar": "Diferència sense imputar",
  "Horas del periodo": "Hores del període",
  "Coste de esas horas": "Cost d'aquestes hores",
  "Añadir horas": "Afegir hores",
  "a coste de trabajador": "a cost de treballador",
  "Excepciones abiertas": "Excepcions obertes",
  "Completitud del paquete": "Completesa del paquet",
  "Documentos extemporáneos": "Documents extemporanis",
  "Consultas con la gestoría": "Consultes amb la gestoria",
  "Activos, vehículos y renting": "Actius, vehicles i rènting",
  Excepciones: "Excepcions",
  Previsión: "Previsió",
  "Saldo hoy": "Saldo avui",
  "Mínimo previsto": "Mínim previst",
  "Al final del horizonte": "Al final de l'horitzó",
  "Saldo acumulado": "Saldo acumulat",
  "Neto del periodo": "Net del període",
  "Facturas emitidas pendientes": "Factures emeses pendents",
  "Hitos de contrato previstos": "Fites de contracte previstes",
  "Facturas recibidas pendientes": "Factures rebudes pendents",
  "Toda la empresa": "Tota l'empresa",
  Horizonte: "Horitzó",
  Meses: "Mesos",
  "bancos y caja": "bancs i caixa",
  "Esta semana": "Aquesta setmana",
  "Ruta desconocida": "Ruta desconeguda",
  Resumen: "Resum",
  Semanas: "Setmanes",
  "Foto y avance de obra": "Foto i avanç d'obra",
  "Parte de horas de hoy": "Part d'hores d'avui",
  "Capturar un gasto": "Capturar una despesa",
  "Acciones de obra": "Accions d'obra",
  // ── Merged from main: the planning screen's strings and the reports note,
  //    which arrived on the ES/EN spine without Catalan. Translated here
  //    rather than raising the ceiling — the backlog only ever falls.
  "Ruta crítica — un día perdido aquí es un día perdido en la obra":
    "Ruta crítica — un dia perdut aquí és un dia perdut a l'obra",
  "Sin tareas planificadas. Añade una tarea o genera la planificación desde las partidas del proyecto.":
    "Sense tasques planificades. Afegeix una tasca o genera la planificació des de les partides del projecte.",
  "Ninguna. Arrastra el punto de otra barra hasta ésta para crear una.":
    "Cap. Arrossega el punt d'una altra barra fins a aquesta per crear-ne una.",
  "Planificación de obra con dependencias, ruta crítica y línea base. Arrastra una barra para moverla, tira del borde derecho para alargarla y arrastra el punto de una barra a otra para crear una dependencia.":
    "Planificació d'obra amb dependències, ruta crítica i línia base. Arrossega una barra per moure-la, estira la vora dreta per allargar-la i arrossega el punt d'una barra a una altra per crear una dependència.",
  "Una línea base guarda la planificación tal como está hoy. No se puede modificar después: es la referencia contra la que se mide el avance real.":
    "Una línia base desa la planificació tal com és avui. No es pot modificar després: és la referència amb què es mesura l'avanç real.",
  "La planificación necesita el módulo de capacidades, que no se ha cargado. Recarga la página.":
    "La planificació necessita el mòdul de capacitats, que no s'ha carregat. Torna a carregar la pàgina.",
  "No se desarrolla por ahora. Los informes se construirán sobre datos ya registrados, sin pedir información nueva.":
    "De moment no es desenvolupa. Els informes es construiran sobre dades ja registrades, sense demanar informació nova.",
  // P1-P5 i18n backfill.
  Aceptar: "Accepta",
  "Anular la modificación": "Anul·la la modificació",
  "Anular la orden de compra": "Anul·la la comanda de compra",
  "Asignar la alerta": "Assigna l'alerta",
  "Añadida a la lista": "Afegida a la llista",
  "Buscar en el catálogo": "Cerca al catàleg",
  andamio: "bastida",
  "plataforma elevadora": "plataforma elevadora",
  seguridad: "seguretat",
  "protección de suelos": "protecció de terres",
  demolición: "enderroc",
  escombro: "runa",
  contenedor: "contenidor",
  retirada: "retirada",
  "fábrica de ladrillo": "obra de maó",
  "tabique de placa": "envà de placa",
  "enfoscado y guarnecido": "arrebossat i lliscat",
  aislamiento: "aïllament",
  impermeabilización: "impermeabilització",
  cubierta: "coberta",
  teja: "teula",
  canalón: "canaló",
  fachada: "façana",
  solado: "paviment",
  tarima: "parquet",
  "solera y mortero": "solera i morter",
  alicatado: "enrajolat",
  "falso techo": "fals sostre",
  canalización: "canalització",
  "punto de agua": "punt d'aigua",
  desagüe: "desguàs",
  caldera: "caldera",
  inodoro: "inodor",
  lavabo: "lavabo",
  "plato de ducha": "plat de dutxa",
  mecanismo: "mecanisme",
  "cuadro eléctrico": "quadre elèctric",
  luminaria: "lluminària",
  "línea eléctrica": "línia elèctrica",
  climatización: "climatització",
  radiador: "radiador",
  conducto: "conducte",
  puerta: "porta",
  ventana: "finestra",
  vidriería: "vidrieria",
  armario: "armari",
  "mueble de cocina": "moble de cuina",
  encimera: "encimera",
  pintura: "pintura",
  brocha: "brotxa",
  limpieza: "neteja",
  "horas de trabajo": "hores de treball",
  "maquinaria menor": "maquinària menor",
  "pequeño material": "material petit",
  "tasas y licencia": "taxes i llicència",
  "proyecto y dirección": "projecte i direcció",
  levantamiento: "aixecament",
  "contador y suministro": "comptador i subministrament",
  subpartida: "subpartida",
  "— elegir del catálogo —": "— triar del catàleg —",
  "Elegir una subpartida del catálogo": "Triar una subpartida del catàleg",
  "Completar los datos ahora": "Completa les dades ara",
  "Consulta a la gestoría": "Consulta a la gestoria",
  Continuar: "Continua",
  "Convertir en tarea": "Converteix en tasca",
  "Corrige los bloqueos de arriba y vuelve a abrir este paso.":
    "Corregeix els bloquejos de dalt i torna a obrir aquest pas.",
  "Crear versión": "Crea versió",
  "Cómo se cobra (formas de pago) y en qué reparto (condiciones de pago). El vencimiento real lo calcula cada documento; aquí se mantienen las listas.":
    "Com es cobra (formes de pagament) i en quin repartiment (condicions de pagament). El venciment real el calcula cada document; aquí es mantenen les llistes.",
  "Datos reales en el servidor": "Dades reals al servidor",
  "De dónde llegan los clientes, qué toca hacer con cada oportunidad y por qué se pierden. Las tres listas alimentan los informes de origen y de conversión.":
    "D'on arriben els clients, què toca fer amb cada oportunitat i per què es perden. Les tres llistes alimenten els informes d'origen i de conversió.",
  "Del catálogo": "Del catàleg",
  Destinatario: "Destinatari",
  "Déjalo en blanco para dejar el parte sin partida asignada.":
    "Deixa-ho en blanc per deixar el part sense partida assignata.",
  "Encuadra y dispara": "Enquadra i dispara",
  Entendido: "Entesos",
  "Enviar consulta": "Envia la consulta",
  "Esta versión ya está congelada: se conserva tal como se envió.":
    "Aquesta versió ja està congelada: es conserva tal com es va enviar.",
  "Este dispositivo no ofrece cámara al navegador":
    "Aquest dispositiu no ofereix càmera al navegador",
  "Estos son los datos reales de la empresa. No se pueden sustituir por datos de demostración desde aquí.":
    "Aquestes són les dades reals de l'empresa. No es poden substituir per dades de demostració des d'aquí.",
  Evidencia: "Evidència",
  "Faltan datos del cliente": "Falten dades del client",
  "Fecha límite": "Data límit",
  "Fuera del catálogo": "Fora del catàleg",
  "Hacer la foto": "Fes la foto",
  "Hay subpartidas que bloquean el envío": "Hi ha subpartides que bloquegen l'enviament",
  Justificar: "Justifica",
  "Justificar la excepción": "Justifica l'excepció",
  "La versión enviada se conserva intacta; los cambios van en la nueva.":
    "La versió enviada es conserva intacta; els canvis van a la nova.",
  "La visita se captura igual — pero sin estos datos no se puede emitir ni contrato ni factura.":
    "La visita es captura igual — però sense aquestes dades no es pot emetre ni contracte ni factura.",
  "Llamar la semana que viene": "Truca la setmana que ve",
  "Las partidas salen del catálogo para que se llamen igual en todos los presupuestos.":
    "Les partides surten del catàleg perquè es diguin igual a tots els pressupostos.",
  "Los que aún no se han visitado van primero, y dentro de cada grupo los que llevan más tiempo esperando.":
    "Els que encara no s'han visitat van primer, i dins de cada grup els que porten més temps esperant.",
  "Marcar como perdida": "Marca com a perduda",
  "Margen sobre el precio de venta. Al escribirlo, el precio unitario se recalcula.":
    "Marge sobre el preu de venda. En escriure'l, es recalcula el preu unitari.",
  "Margen unit. %": "Marge unit. %",
  "Motivo de la anulación": "Motiu de l'anul·lació",
  "Motivo de la reapertura": "Motiu de la reobertura",
  "Motivo de la rescisión": "Motiu de la rescissió",
  "Motivo del rechazo": "Motiu del rebuig",
  "Nada que corregir: todas las subpartidas tienen cantidad, precio y coste coherentes.":
    "Res a corregir: totes les subpartides tenen quantitat, preu i cost coherents.",
  "No se ha dado permiso de cámara. Puedes autorizarlo en el candado de la barra de direcciones.":
    "No s'ha donat permís de càmera. Ho pots autoritzar des del cadenat de la barra d'adreces.",
  "No se puede eliminar": "No es pot eliminar",
  "No se puede programar una visita en una fecha pasada":
    "No es pot programar una visita en una data passada",
  "Nom (CA)": "Nom (CA)",
  "Nombre (ES)": "Nom (ES)",
  "Nombre de la partida": "Nom de la partida",
  "Nombre o puesto": "Nom o càrrec",
  "Nota de resolución": "Nota de resolució",
  "Nueva entrada": "Nova entrada",
  "Nueva subpartida": "Nova subpartida",
  "Nueva partida": "Nova partida",
  "Nº de factura, documento, referencia… (opcional)":
    "Núm. de factura, document, referència… (opcional)",
  "Opcional, pero es lo que explica la espera…": "Opcional, però és el que explica l'espera…",
  "Otro nombre…": "Un altre nom…",
  "P. unitario venta": "P. unitari venda",
  "Subpartida tomada del catálogo": "Subpartida agafada del catàleg",
  "Rendimiento de la cartera": "Rendiment de la cartera",
  "Coste incurrido": "Cost incorregut",
  "Pendiente hasta terminar": "Pendent fins a acabar",
  "Por encima del presupuesto": "Per sobre del pressupost",
  "Presupuesto aprobado": "Pressupost aprovat",
  "⚠ Pendiente": "⚠ Pendent",
  "Sin NIF no se puede facturar ni contratar": "Sense NIF no es pot facturar ni contractar",
  "Sin NIF no se puede facturar ni contratar — la ficha se crea igual y se completa después":
    "Sense NIF no es pot facturar ni contractar — la fitxa es crea igualment i es completa després",
  "Sin NIF no se puede facturar ni contratar — la ficha existe igual y se completa después":
    "Sense NIF no es pot facturar ni contractar — la fitxa existeix igualment i es completa després",
  "Ir a la Torre de control": "Anar a la Torre de control",
  "+ partida": "+ partida",
  "Coste ud.": "Cost ut.",
  "P. venta ud.": "P. venda ut.",
  "Imág.": "Imat.",
  "Mostrar u ocultar las partidas": "Mostrar o amagar les partides",
  "Mostrar u ocultar los totales": "Mostrar o amagar els totals",
  "+ subpartida del catálogo": "+ subpartida del catàleg",
  "+ subpartida en blanco": "+ subpartida en blanc",
  "Una subpartida que todavía no está en el catálogo": "Una subpartida que encara no és al catàleg",
  "Marca las subpartidas que entran en la partida. Cada una llega con su unidad, su coste y su precio.":
    "Marca les subpartides que entren a la partida. Cadascuna arriba amb la seva unitat, el seu cost i el seu preu.",
  "ninguna seleccionada": "cap de seleccionada",
  "Esta partida no tiene subpartidas todavía. Elige «Todas las partidas» o crea la primera.":
    "Aquesta partida encara no té subpartides. Tria «Totes les partides» o crea la primera.",
  "Subpartida en blanco añadida": "Subpartida en blanc afegida",
  "mano de obra propia": "mà d'obra pròpia",
  maquinaria: "maquinària",
  honorarios: "honoraris",
  residuos: "residus",
  "Por código, descripción o cómo se le dice al cliente.":
    "Per codi, descripció o com se li diu al client.",
  Posponer: "Posposa",
  "Posponer la alerta": "Posposa l'alerta",
  "Precio total": "Preu total",
  Programar: "Programa",
  "Programar seguimiento": "Programa seguiment",
  "Próximas acciones": "Pròximes accions",
  "Queda registrado en el trimestre.": "Queda registrat al trimestre.",
  "Queda registrado en la oportunidad y alimenta el análisis de pérdidas.":
    "Queda registrat a l'oportunitat i alimenta l'anàlisi de pèrdues.",
  "Queda registrado en la orden y no se puede deshacer.":
    "Queda registrat a la comanda i no es pot desfer.",
  "Qué han contestado": "Què han contestat",
  Reabrir: "Reobre",
  "Reabrir el periodo": "Reobre el període",
  "Reabrir un periodo cerrado queda registrado con su motivo.":
    "Reobrir un període tancat queda registrat amb el seu motiu.",
  Reasignar: "Reassigna",
  "Reasignar el parte": "Reassigna el part",
  Recargar: "Recarrega",
  Rechazar: "Rebutja",
  "Rechazar la modificación": "Rebutja la modificació",
  Rescindir: "Rescindeix",
  "Rescindir la subcontrata": "Rescindeix la subcontracta",
  Resolver: "Resol",
  "Resolver la alerta": "Resol l'alerta",
  "Respuesta de la gestoría": "Resposta de la gestoria",
  Retirar: "Retira",
  "Retirar una entrada": "Retira una entrada",
  "Se sustituye TODO lo que haya ahora en este dispositivo por el conjunto de ejemplo: dos años de actividad (2024-2025) más la situación actual de 2026.":
    "Se substitueix TOT el que hi hagi ara en aquest dispositiu pel conjunt d'exemple: dos anys d'activitat (2024-2025) més la situació actual de 2026.",
  "Siguiente paso": "Següent pas",
  "Siguiente paso →": "Següent pas →",
  "Sin visita todavía": "Sense visita encara",
  "Sólo es posible si no tiene documentos económicos asociados. Esta acción no se puede deshacer.":
    "Només és possible si no té documents econòmics associats. Aquesta acció no es pot desfer.",
  "Sólo para una subpartida que no se va a repetir. Si se va a usar más veces, añádela al catálogo en Configuración → Elementos de presupuesto.":
    "Només per a una partida que no es tornarà a fer servir. Si s'ha d'utilitzar més vegades, afegeix-la al catàleg a Configuració → Subpartides.",
  "Una alerta no se cierra sin decir cómo se resolvió.":
    "Una alerta no es tanca sense dir com s'ha resolt.",
  "Vence el": "Venç el",
  "Ya visitados — sería un seguimiento": "Ja visitats — seria un seguiment",
  seguimiento: "seguiment",
  "sin partida": "sense partida",
  "sin subpartidas todavía": "sense subpartides encara",
  "¿A quién se asigna?": "A qui s'assigna?",
  "¿Para qué lead?": "Per a quin lead?",
  "¿Por qué puede enviarse el paquete así?": "Per què es pot enviar el paquet així?",
  "¿Por qué se ha perdido?": "Per què s'ha perdut?",
  "¿Recargar los datos de demostración?": "Recarregar les dades de demostració?",
  "⤓ Descargar": "⤓ Descarrega",
  "＋ Nueva subpartida": "＋ Nova subpartida",
  "＋ Nueva…": "＋ Nova…",
  "＋ Nuevo cliente…": "＋ Nou client…",
  "＋ Programar visita de seguimiento": "＋ Programar visita de seguiment",
  "Condiciones de pago": "Condicions de pagament",
  "Coste unitario": "Cost unitari",
  "sin precio": "sense preu",
  Unidad: "Unitat",
  // PK2-A: the evidence field and the viewer that reads a PDF.
  "Abriendo el documento…": "Obrint el document…",
  "Abrir en una pestaña": "Obre en una pestanya",
  "Buscar el archivo": "Cerca el fitxer",
  "Guardando el archivo…": "Desant el fitxer…",
  "No se ha podido dibujar el documento aquí.": "No s'ha pogut dibuixar el document aquí.",
  "Página ›": "Pàgina ›",
  "‹ Página": "‹ Pàgina",
  Quién: "Qui",
  "Quién lo ha aceptado": "Qui ho ha acceptat",
  Rechazo: "Rebuig",
  Ver: "Veure",
  "sin adjuntar": "sense adjuntar",
  "Arrastra aquí el PDF o la imagen, o búscalo en el dispositivo.":
    "Arrossega aquí el PDF o la imatge, o cerca'l al dispositiu.",
  "Arrastra aquí el correo o el documento de aceptación (PDF o imagen), o búscalo en el dispositivo.":
    "Arrossega aquí el correu o el document d'acceptació (PDF o imatge), o cerca'l al dispositiu.",
  "⚠ Sólo se puede adjuntar un PDF o una imagen": "⚠ Només es pot adjuntar un PDF o una imatge",
  "⚠ No se ha podido guardar el archivo": "⚠ No s'ha pogut desar el fitxer",
  "⚠ No se ha podido guardar el archivo — el documento no se ha registrado":
    "⚠ No s'ha pogut desar el fitxer — el document no s'ha registrat",
  Aceptación: "Acceptació",
  "Fecha de aceptación": "Data d'acceptació",
  "Ir a otra versión": "Ves a una altra versió",
  "Fecha de envío": "Data d'enviament",
  "Envío de presupuesto": "Enviament del pressupost",
  "Sin móvil registrado — añádalo en Datos maestros para poder abrir WhatsApp":
    "Sense mòbil registrat — afegeix-lo a Dades mestres per poder obrir el WhatsApp",
  // S4 (Package 8 · 28/08): channel honesty, execution time, Excel export.
  "Sin correo registrado — añádalo en Datos maestros para poder enviar por correo":
    "Sense correu registrat — afegeix-lo a Dades mestres per poder enviar per correu",
  "Añada un correo para enviar por email": "Afegeix un correu per enviar per correu electrònic",
  "Se prepara un borrador en el buzón de la empresa. No se envía nada al cliente todavía.":
    "Es prepara un esborrany a la bústia de l'empresa. Encara no s'envia res al client.",
  "Se abre WhatsApp con el mensaje escrito. Usted decide cuándo enviarlo.":
    "S'obre el WhatsApp amb el missatge escrit. Vostè decideix quan enviar-lo.",
  "Queda registrado como entregado en mano en la fecha y hora que indique.":
    "Queda registrat com a lliurat en mà a la data i hora que indiqui.",
  "Plazo de ejecución": "Termini d'execució",
  "Plazo de ejecución actualizado": "Termini d'execució actualitzat",
  "⤓ Excel": "⤓ Excel",
  "Precio ud.": "Preu un.",
  viendo: "veient",
  // PK2-C: contract layout, garantías fix, anexo approval evidence.
  "Efecto en venta": "Efecte en venda",
  "Foto de origen": "Foto d'origen",
  "Marcar como aprobada": "Marca com a aprovada",
  "Aprobar genera la adenda del contrato y añade el importe al importe vigente.":
    "Aprovar genera l'addenda del contracte i afegeix l'import a l'import vigent.",
  "Arrastra aquí el respaldo de la aprobación (WhatsApp, correo, adenda firmada…), o búscalo en el dispositivo.":
    "Arrossega aquí el respatller de l'aprovació (WhatsApp, correu, addenda signada…), o cerca'l al dispositiu.",
  "Modificación aprobada — adenda generada": "Modificació aprovada — addenda generada",
  // PK2-D: creating a contract, and recording one signed elsewhere.
  "Nuevo contrato": "Nou contracte",
  "＋ Nuevo contrato": "＋ Nou contracte",
  "¿De dónde sale este contrato?": "D'on surt aquest contracte?",
  "De un presupuesto aceptado": "D'un pressupost acceptat",
  // S6 (Package 8 · 28/08): a contract comes only from an accepted quote.
  "Presupuesto aceptado": "Pressupost acceptat",
  "Referencia de la otra parte": "Referència de l'altra part",
  "El importe, el IVA, el cliente y el inmueble salen del presupuesto: no se teclean dos veces.":
    "L'import, l'IVA, el client i l'immoble surten del pressupost: no es teclegen dues vegades.",
  "Todavía no hay de dónde": "Encara no hi ha d'on",
  "Un contrato sale siempre de un presupuesto aceptado — de ahí vienen el importe, el IVA, el cliente y el inmueble.":
    "Un contracte surt sempre d'un pressupost acceptat — d'allà vénen l'import, l'IVA, el client i l'immoble.",
  "No hay ningún presupuesto aceptado sin contrato en este momento. Registre la respuesta del cliente sobre el presupuesto que haya aceptado y vuelva aquí.":
    "No hi ha cap pressupost acceptat sense contracte en aquest moment. Registri la resposta del client sobre el pressupost que hagi acceptat i torni aquí.",
  "⚠ No hay ningún presupuesto aceptado sin contrato":
    "⚠ No hi ha cap pressupost acceptat sense contracte",
  "Firmado fuera de este sistema": "Signat fora d'aquest sistema",
  "Datos del contrato": "Dades del contracte",
  "Importe sin IVA (€)": "Import sense IVA (€)",
  "Fecha del contrato": "Data del contracte",
  "Referencia externa": "Referència externa",
  "Plazo de ejecución (días)": "Termini d'execució (dies)",
  "Firmado el": "Signat el",
  "Déjalo en blanco si todavía no está firmado.": "Deixa-ho en blanc si encara no està signat.",
  "Déjalo en blanco si todavía no está firmado. Puedes firmarlo más tarde desde el propio contrato.":
    "Deixa-ho en blanc si encara no està signat. El pots signar més tard des del mateix contracte.",
  "Sólo si ya está firmado: arrastra aquí el contrato firmado (PDF o imagen).":
    "Només si ja està signat: arrossega aquí el contracte signat (PDF o imatge).",
  "⚠ Adjunta el contrato firmado, o deja «Firmado el» en blanco":
    "⚠ Adjunta el contracte signat, o deixa «Signat el» en blanc",
  "Firma del cliente": "La signatura del client",
  "⚠ Adjunta el contrato firmado": "⚠ Adjunta el contracte signat",
  "Sin el documento firmado no se puede firmar: es el justificante de que el cliente aceptó estas condiciones.":
    "Sense el document signat no es pot signar: és el justificant que el client va acceptar aquestes condicions.",
  "＋ Añadir hito": "＋ Afegeix una fita",
  "Sin hitos de pago.": "Sense fites de pagament.",
  "Crear contrato": "Crea el contracte",
  "Contrato creado": "Contracte creat",
  "Contrato registrado": "Contracte registrat",
  "Firmar contrato": "Signar contracte",
  Firmar: "Signar",
  "Al firmar, el contrato entra en vigor y la obra puede facturarse.":
    "En signar, el contracte entra en vigor i l'obra es pot facturar.",
  "Contrato firmado": "Contracte signat",
  "Contrato firmado fuera de este sistema.": "Un contracte signat fora d'aquest sistema.",
  "No se ha adjuntado el contrato firmado. Los datos están en el panel de al lado.":
    "No s'ha adjuntat el contracte signat. Les dades són al plafó del costat.",
  "En papel, por un abogado, o antes de usar este programa. Se guarda el archivo firmado y sus datos.":
    "En paper, per un advocat, o abans d'utilitzar aquest programa. Es desa el fitxer signat i les seves dades.",
  "El importe, el IVA y el cliente salen del presupuesto: no se teclean dos veces.":
    "L'import, l'IVA i el client surten del pressupost: no es teclegen dues vegades.",
  "No hay ningún presupuesto aceptado sin contrato todavía.":
    "Encara no hi ha cap pressupost acceptat sense contracte.",
  "firmado fuera": "signat fora",
  "Inicio comprometido": "Inici compromès",
  Ejecución: "Execució",

  /* The strings the rendered-page audit found untranslated. Added here in the
     same change that added their English, so the backlog ceiling does not
     move: a string that ships with English and without Catalan is exactly the
     silent monolingual fallback coverage.mjs exists to stop. */
  "ERP › Principal": "ERP › Principal",
  "Marcar prioritario": "Marcar com a prioritari",
  "Calculado a las": "Calculat a les",
  Recalcular: "Recalcular",
  "⚙ Tarjetas": "⚙ Targetes",
  "⚙ Reglas": "⚙ Regles",
  "🖨️ Imprimir / PDF": "🖨️ Imprimir / PDF",
  "↻ Recargar datos de demostración": "↻ Recarregar dades de demostració",
  "Resultado operativo del mes": "Resultat operatiu del mes",
  "Resultado operativo del trimestre": "Resultat operatiu del trimestre",
  "Saldo bancos": "Saldo bancs",
  "Proyección de caja": "Projecció de caixa",
  "Oportunidades abiertas": "Oportunitats obertes",
  "avance · venta · coste · margen · desviación": "avanç · venda · cost · marge · desviació",
  "Margen acum.": "Marge acum.",
  "Próx. fecha": "Pròx. data",
  "Por tipo": "Per tipus",
  "Por proyecto": "Per projecte",
  Documental: "Documental",
  Económica: "Econòmica",
  Técnica: "Tècnica",
  "Precio de proveedor caducado": "Preu de proveïdor caducat",
  "Movimiento de caja sin justificante": "Moviment de caixa sense justificant",
  "En tareas": "En tasques",
  "→ Tarea": "→ Tasca",
  "Nombre / Razón social": "Nom / Raó social",
  Móvil: "Mòbil",
  "C.P.": "C.P.",
  Alta: "Alta",
  "Buscar cliente por nombre o NIF": "Cercar client per nom o NIF",
  "NIF / CIF": "NIF / CIF",
  "← ERP": "← ERP",
  "Cobros / Pagos (DSO / DPO)": "Cobraments / Pagaments (DSO / DPO)",
  "Crear nuevo proyecto": "Crear projecte nou",
  "Proyecto existente": "Projecte existent",
  Subtotal: "Subtotal",
  "Mampara de ducha de vidrio templado": "Mampara de dutxa de vidre trempat",
  "Backend ↗": "Backend ↗",
  /* Master Data and Financial Data, the two standalone screens on the
     operator walk-through. Both declare <html lang="en"> while their content
     is Spanish, so the runtime composes EN→ES→CA — which means a missing
     Catalan entry here is what makes the whole screen come out Spanish, not a
     missing English one. Measured by rendering the pages, not by reading this
     file (tests/i18n/audit.mjs). */
  Importar: "Importar",
  "Exportar JSON": "Exportar JSON",
  "Añadir registro": "Afegir registre",
  Organización: "Organització",
  "Empresa / Entidad legal": "Empresa / Entitat legal",
  "Sucursales / Ubicaciones": "Sucursals / Ubicacions",
  "Cuentas bancarias": "Comptes bancaris",
  "Series de numeración": "Sèries de numeració",
  Contactos: "Contactes",
  "Oportunidades / Pipeline": "Oportunitats / Pipeline",
  Suministro: "Subministrament",
  "Proveedores y subcontratistas": "Proveïdors i subcontractistes",
  "Catálogo y precios": "Catàleg i preus",
  "Artículos — materiales y trabajos": "Articles — materials i treballs",
  "Partidas del catálogo": "Partides del catàleg",
  "Listas de precios / Tarifas": "Llistes de preus / Tarifes",
  "Códigos de IVA / impuestos": "Codis d'IVA / impostos",
  "Unidades de medida": "Unitats de mesura",
  "Operaciones y recursos": "Operacions i recursos",
  "Almacenes / Depósitos": "Magatzems / Dipòsits",
  "Stock / Inventario": "Stock / Inventari",
  "Equipo / Recursos": "Equip / Recursos",
  "Equipos y vehículos": "Equipaments i vehicles",
  "Proyectos / Obras": "Projectes / Obres",
  "Gobernanza y automatización": "Governança i automatització",
  "Documentos y cumplimiento": "Documents i compliment",
  "Reglas de automatización": "Regles d'automatització",
  "Datos maestros ›": "Dades mestres ›",
  "Todos a quienes vende — identidad de facturación, condiciones, tarifa y consentimiento que rigen presupuestos, facturación y cobros.":
    "Tothom a qui ven — identitat de facturació, condicions, tarifa i consentiment que regeixen pressupostos, facturació i cobraments.",
  "Una única fuente de verdad, construida para la automatización.":
    "Una única font de veritat, construïda per a l'automatització.",
  "Cada registro tiene un código estable y campos tipados. Presupuestos, compras, facturación, cobros, planificación y los flujos de recordatorios/reseñas leen todo de aquí — nada se introduce dos veces.":
    "Cada registre té un codi estable i camps tipats. Pressupostos, compres, facturació, cobraments, planificació i els fluxos de recordatoris/ressenyes ho llegeixen tot d'aquí — res no s'introdueix dues vegades.",
  "entrega todo el conjunto de datos a cualquier automatización posterior.":
    "lliura tot el conjunt de dades a qualsevol automatització posterior.",
  "Organización — Empresa / Entidad legal": "Organització — Empresa / Entitat legal",
  "Organización — Sucursales / Ubicaciones": "Organització — Sucursals / Ubicacions",
  "Organización — Cuentas bancarias": "Organització — Comptes bancaris",
  "Organización — Series de numeración": "Organització — Sèries de numeració",
  "Comercial — Clientes": "Comercial — Clients",
  "Comercial — Contactos": "Comercial — Contactes",
  "Comercial — Oportunidades / Pipeline": "Comercial — Oportunitats / Pipeline",
  "Suministro — Proveedores y subcontratistas": "Subministrament — Proveïdors i subcontractistes",
  "Suministro — Condiciones de pago": "Subministrament — Condicions de pagament",
  "Catálogo y precios — Artículos — materiales y trabajos":
    "Catàleg i preus — Articles — materials i treballs",
  "Catálogo y precios — Partidas": "Catàleg i preus — Partides",
  "Catálogo y precios — Listas de precios / Tarifas":
    "Catàleg i preus — Llistes de preus / Tarifes",
  "Catálogo y precios — Códigos de IVA / impuestos": "Catàleg i preus — Codis d'IVA / impostos",
  "Catálogo y precios — Unidades de medida": "Catàleg i preus — Unitats de mesura",
  "Operaciones y recursos — Almacenes / Depósitos": "Operacions i recursos — Magatzems / Dipòsits",
  "Operaciones y recursos — Stock / Inventario": "Operacions i recursos — Stock / Inventari",
  "Operaciones y recursos — Equipo / Recursos": "Operacions i recursos — Equip / Recursos",
  "Operaciones y recursos — Equipos y vehículos": "Operacions i recursos — Equipaments i vehicles",
  "Operaciones y recursos — Proyectos / Obras": "Operacions i recursos — Projectes / Obres",
  "Gobernanza y automatización — Documentos y cumplimiento":
    "Governança i automatització — Documents i compliment",
  "Gobernanza y automatización — Reglas de automatización":
    "Governança i automatització — Regles d'automatització",
  "↻ Datos iniciales": "↻ Dades inicials",
  "Razón social / nombre completo": "Raó social / nom complet",
  "Correo electrónico": "Correu electrònic",
  Teléfono: "Telèfon",
  Prospecto: "Prospecte",
  Editar: "Editar",
  Eliminar: "Eliminar",
  "✕ partida": "✕ partida",
  "Eliminar subpartida": "Eliminar subpartida",
  "Eliminar esta subpartida": "Eliminar aquesta subpartida",
  "Eliminar la partida y todas sus subpartidas":
    "Eliminar la partida i totes les seves subpartides",
  "Partida eliminada": "Partida eliminata",
  "La partida está vacío. No se puede deshacer.": "La partida és buit. No es pot desfer.",
  Cancelar: "Cancel·lar",
  "Guardar registro": "Desar registre",
  "Buscar registros…": "Cercar registres…",
  "Cargar el conjunto de datos inicial": "Carregar el conjunt de dades inicial",
  "Canei Subirats — Datos maestros": "Canei Subirats — Dades mestres",
  Periodo: "Període",
  "Acumulado 2026 (ene–jul)": "Acumulat 2026 (gen–jul)",
  "T1 2026": "T1 2026",
  "T2 2026": "T2 2026",
  "Julio 2026": "Juliol 2026",
  "Plan anual (presupuesto)": "Pla anual (pressupost)",
  Exportar: "Exportar",
  Añadir: "Afegir",
  "Panel de KPI": "Panell de KPI",
  "Estados financieros": "Estats financers",
  "Cuenta de resultados": "Compte de resultats",
  Balance: "Balanç",
  "Flujo de caja": "Flux de caixa",
  "Capital circulante": "Capital circulant",
  "Antigüedad cobros / pagos": "Antiguitat cobraments / pagaments",
  "Cuentas a cobrar": "Comptes a cobrar",
  "Cuentas a pagar": "Comptes a pagar",
  "IVA e impuestos": "IVA i impostos",
  "Plan de cuentas": "Pla de comptes",
  "Libro de PyG": "Llibre de PiG",
  "Entrada de balance": "Entrada de balanç",
  "Bancos y caja": "Bancs i caixa",
  Préstamos: "Préstecs",
  Variables: "Variables",
  "Datos financieros ›": "Dades financeres ›",
  "Panel en vivo para Acumulado 2026 (ene–jul). Todo se calcula a partir de los libros — edita una entrada y todas las cifras se actualizan.":
    "Panell en viu per a Acumulat 2026 (gen–jul). Tot es calcula a partir dels llibres — edita una entrada i totes les xifres s'actualitzen.",
  "Ingresos (acumulado)": "Ingressos (acumulat)",
  "Margen bruto": "Marge brut",
  "Beneficio neto": "Benefici net",
  "Posición de caja": "Posició de caixa",
  "Ratio de circulante": "Ràtio de circulant",
  "Días de cobro frente a días de pago": "Dies de cobrament respecte a dies de pagament",
  "Cartera de pedidos": "Cartera de comandes",
  "Puente de rentabilidad": "Pont de rendibilitat",
  "Coste de ventas": "Cost de vendes",
  "Beneficio bruto": "Benefici brut",
  "Gastos de explotación": "Despeses d'explotació",
  Amortizaciones: "Amortitzacions",
  Intereses: "Interessos",
  "Comprobaciones de salud": "Comprovacions de salut",
  "El balance cuadra": "El balanç quadra",
  "A = P + PN ✓": "A = P + PN ✓",
  "El flujo de caja concilia": "El flux de caixa concilia",
  "cuadra con la caja ✓": "quadra amb la caixa ✓",
  "Ingresos / empleado": "Ingressos / empleat",
  "Margen bruto vs objetivo": "Marge brut vs objectiu",
  "por debajo": "per sota",
  "Deuda neta / EBITDA": "Deute net / EBITDA",
  "Canei Subirats — Datos financieros": "Canei Subirats — Dades financeres",
  Libros: "Llibres",
  Tax: "Impostos",
  "Sin datos del ERP.": "Sense dades de l'ERP.",
  "Esta sesión no tiene estado del ERP cargado, así que las cifras que se ven son las de demostración de esta página.":
    "Aquesta sessió no té l'estat de l'ERP carregat, així que les xifres que es veuen són les de demostració d'aquesta pàgina.",
  "Resumen — KPI cockpit": "Resum — Panell de KPI",
  "Estados financieros — Profit & Loss": "Estats financers — Compte de resultats",
  "Estados financieros — Balance sheet": "Estats financers — Balanç",
  "Estados financieros — Cash flow": "Estats financers — Flux de caixa",
  "Capital circulante — AR / AP aging": "Capital circulant — Antiguitat cobraments / pagaments",
  "Capital circulante — Receivables": "Capital circulant — Comptes a cobrar",
  "Capital circulante — Payables": "Capital circulant — Comptes a pagar",
  "Capital circulante — VAT & tax": "Capital circulant — IVA i impostos",
  "Libros — Chart of accounts": "Llibres — Pla de comptes",
  "Libros — P&L ledger": "Llibres — Llibre de PiG",
  "Libros — Balance sheet input": "Llibres — Entrada de balanç",
  "Libros — Bank & cash": "Llibres — Bancs i caixa",
  "Libros — Loans": "Llibres — Préstecs",
  "Libros — Drivers": "Llibres — Variables",
  "Los cuatro indicadores de la empresa, el estado de cada proyecto y lo que necesita atención. Todo abre su detalle; aquí no se edita nada.":
    "Els quatre indicadors de l'empresa, l'estat de cada projecte i el que necessita atenció. Tot obre el seu detall; aquí no s'edita res.",
  Abiertos: "Oberts",
  Cerrados: "Tancats",
  Favoritos: "Preferits",
  "Marcar como favorito": "Marcar com a preferit",
  "Venta contratada": "Venda contractada",
  "Margen actual": "Marge actual",
  "Próximas fechas": "Pròximes dates",
  "Sin programar": "Sense programar",
  "Boca a boca": "Boca-orella",
  Prescriptor: "Prescriptor",
  "Web propia": "Web propi",
  "Administrador de fincas": "Administrador de finques",
  "Plataforma de leads": "Plataforma de leads",
  Leads: "Leads",
  "＋ Crear": "＋ Crear",
  "🗒️ Nueva tarea": "🗒️ Nova tasca",
  Año: "Any",
  Trimestre: "Trimestre",
  Mes: "Mes",
  Rango: "Interval",
  "🏁 Guía de puesta en marcha": "🏁 Guia de posada en marxa",
  "📘 Guía de operaciones": "📘 Guia d'operacions",
  "⤓ Exportar datos": "⤓ Exportar dades",
  "Obras asignadas": "Obres assignades",
  "＋ Asignar a la obra": "＋ Assignar a l'obra",
  "Sin obras asignadas.": "Sense obres assignades.",
  "Trabajador asignado a la obra": "Treballador assignat a l'obra",
  "Asignación retirada": "Assignació retirada",
  "Asignación no encontrada": "Assignació no trobada",
  "Sin una obra asignada, esta persona no puede apuntar horas: sólo recibe las obras en las que está.":
    "Sense una obra assignada, aquesta persona no pot apuntar hores: només rep les obres on és.",
  Quitar: "Treure",
  "Todavía no estás asignado a ninguna obra. Habla con la oficina para que te asignen y podrás apuntar tus horas aquí.":
    "Encara no estàs assignat a cap obra. Parla amb l'oficina perquè t'assignin i podràs apuntar les teves hores aquí.",
  "⇥ Cerrar sesión": "⇥ Tancar la sessió",
  "＋ Oportunidad/visita": "＋ Oportunitat/visita",
  "＋ Tarea": "＋ Tasca",
  "＋ Presupuesto": "＋ Pressupost",
  "＋ Factura": "＋ Factura",
  "Buscar cliente, proyecto, presupuesto, factura…":
    "Cercar client, projecte, pressupost, factura…",
  "Buscador universal": "Cercador universal",
  Desde: "Des de",
  Hasta: "Fins a",
  "Perfil y ayuda": "Perfil i ajuda",
  Secciones: "Seccions",
  Subsecciones: "Subseccions",
  "🎯 Nueva oportunidad": "🎯 Nova oportunitat",
  "👤 Nuevo cliente": "👤 Nou client",
  Visita: "Visita",
  "Perdida · Plazos": "Perduda · Terminis",
  "Perdida · Competencia": "Perduda · Competència",
  "Perdida · Precio": "Perduda · Preu",
  Dirección: "Adreça",
  "🏗️ Nuevo proyecto": "🏗️ Nou projecte",
  Todos: "Tots",
  Administración: "Administració",
  Población: "Població",
  "A revisar": "A revisar",
  "no aplica": "no s'aplica",
  aplica: "s'aplica",
  Particular: "Particular",
  Empresa: "Empresa",
  Comunidad: "Comunitat",
  "Ente público": "Ens públic",
  "Precio unitario (€)": "Preu unitari (€)",
  "Industrial / proveedor *": "Industrial / proveïdor *",
  "Oficio *": "Ofici *",
  Formato: "Format",
  "Importe adjudicado (€)": "Import adjudicat (€)",
  "Retención (%)": "Retenció (%)",
  "Inicio previsto": "Inici previst",
  "Certificar avance (€)": "Certificar avanç (€)",
  "Aprobar factura — nº": "Aprovar factura — núm.",
  "Base imponible (€)": "Base imposable (€)",
  "Efecto en venta (€)": "Efecte en venda (€)",
  "Efecto en coste (€)": "Efecte en cost (€)",
  "Efecto en plazo (días)": "Efecte en termini (dies)",
  "Trabajador *": "Treballador *",
  "Proyecto *": "Projecte *",
  "Horas *": "Hores *",
  Marca: "Marca",
  Modelo: "Model",
  Calidad: "Qualitat",
  "Coste de referencia (€)": "Cost de referència (€)",
  "Precio de referencia (€)": "Preu de referència (€)",
  "Subpartida *": "Subpartida *",
  "Fecha *": "Data *",
  "Código del artículo en el proveedor": "Codi de l'article al proveïdor",
  "Precio de lista (€) *": "Preu de llista (€) *",
  "Descuento (%)": "Descompte (%)",
  "Precio neto (€)": "Preu net (€)",
  "Transporte (€)": "Transport (€)",
  "Gestión de residuos (€)": "Gestió de residus (€)",
  "Pedido mínimo": "Comanda mínima",
  "Vigente hasta": "Vigent fins a",
  "Documento de origen": "Document d'origen",
  "Proyecto para el que se pidió": "Projecte per al qual es va demanar",
  "NIF / NIE": "NIF / NIE",
  "Tarifa inicial (€/hora) *": "Tarifa inicial (€/hora) *",
  Cuerpo: "Cos",
  "Variables — pulsa para insertar": "Variables — prem per inserir",
  "Adjunto automático": "Adjunt automàtic",
  Familia: "Família",
  "Evento *": "Esdeveniment *",
  "Plantilla *": "Plantilla *",
  "Días tras el evento": "Dies després de l'esdeveniment",
  Modo: "Mode",
  Permiso: "Permís",
  "Categoría (proveedores)": "Categoria (proveïdors)",
  "Nombres originales / alias (separados por coma)": "Noms originals / àlies (separats per coma)",
  Qué: "Què",
  "Coste a fin de obra (€)": "Cost a fi d'obra (€)",
  "Motivo * — se guarda con el ajuste y es lo único revisable después":
    "Motiu * — es desa amb l'ajust i és l'únic revisable després",
  "Concepto *": "Concepte *",
  "Orden de trabajo": "Ordre de treball",
  "Contrato de subcontrata": "Contracte de subcontracta",
  "— sin partida —": "— sense partida —",
  "Borrador para aprobar": "Esborrany per aprovar",
  Automático: "Automàtic",
  Opcional: "Opcional",
  "Todas las subpartidas": "Totes les subpartides",
  Desactivado: "Desactivat",
  "— sin inmueble asociado —": "— sense immoble associat —",
  "＋ Nuevo inmueble…": "＋ Nou immoble…",
  "＋ Nuevo inmueble": "＋ Nou immoble",
  "Editar inmueble": "Editar immoble",
  Inmuebles: "Immobles",
  "Crear inmueble": "Crear immoble",
  "Inmueble actualizado": "Immoble actualitzat",
  "Inmueble creado": "Immoble creat",
  "Sin inmuebles asociados todavía.": "Sense immobles associats encara.",
  "La dirección es obligatoria": "L'adreça és obligatòria",
  Vivienda: "Habitatge",
  "Zona común / comunidad": "Zona comuna / comunitat",
  "Local comercial": "Local comercial",
  "Dirección *": "Adreça *",
  "NIF/CIF no válido — No tiene la longitud de un DNI, NIE o CIF español (9 caracteres) ni la de un IVA intracomunitario (dos letras de país + dígitos).":
    "NIF/CIF no vàlid — no té la longitud d'un DNI, NIE o CIF espanyol (9 caràcters) ni la d'un IVA intracomunitari (dues lletres de país + dígits).",
  "NIF/CIF no válido — El formato no corresponde a un DNI/NIF (8 dígitos y una letra), un NIE (X, Y o Z seguido de 7 dígitos y una letra) ni un CIF (una letra seguida de 7 dígitos y un dígito o letra).":
    "NIF/CIF no vàlid — el format no correspon a un DNI/NIF (8 dígits i una lletra), un NIE (X, Y o Z seguit de 7 dígits i una lletra) ni un CIF (una lletra seguida de 7 dígits i un dígit o lletra).",
  // The 52 province names a C.P.'s first two digits resolve to — proper
  // nouns, kept identical across ES/EN/CA, same as the EN dict entry beside
  // it explains.
  "Araba/Álava": "Araba/Álava",
  Albacete: "Albacete",
  "Alicante/Alacant": "Alicante/Alacant",
  Almería: "Almería",
  Ávila: "Ávila",
  Badajoz: "Badajoz",
  "Illes Balears": "Illes Balears",
  Barcelona: "Barcelona",
  Burgos: "Burgos",
  Cáceres: "Cáceres",
  Cádiz: "Cádiz",
  "Castellón/Castelló": "Castellón/Castelló",
  "Ciudad Real": "Ciudad Real",
  Córdoba: "Córdoba",
  "A Coruña": "A Coruña",
  Cuenca: "Cuenca",
  Girona: "Girona",
  Granada: "Granada",
  Guadalajara: "Guadalajara",
  Gipuzkoa: "Gipuzkoa",
  Huelva: "Huelva",
  Huesca: "Huesca",
  Jaén: "Jaén",
  León: "León",
  Lleida: "Lleida",
  "La Rioja": "La Rioja",
  Lugo: "Lugo",
  Madrid: "Madrid",
  Málaga: "Málaga",
  Murcia: "Murcia",
  Navarra: "Navarra",
  Ourense: "Ourense",
  Asturias: "Asturias",
  Palencia: "Palencia",
  "Las Palmas": "Las Palmas",
  Pontevedra: "Pontevedra",
  Salamanca: "Salamanca",
  "Santa Cruz de Tenerife": "Santa Cruz de Tenerife",
  Cantabria: "Cantabria",
  Segovia: "Segovia",
  Sevilla: "Sevilla",
  Soria: "Soria",
  Tarragona: "Tarragona",
  Teruel: "Teruel",
  Toledo: "Toledo",
  "Valencia/València": "Valencia/València",
  Valladolid: "Valladolid",
  Bizkaia: "Bizkaia",
  Zamora: "Zamora",
  Zaragoza: "Zaragoza",
  Ceuta: "Ceuta",
  Melilla: "Melilla",
  "— sin clientes todavía —": "— encara no hi ha clients —",
  "No se ha podido conectar con el servidor": "No s'ha pogut connectar amb el servidor",
  "Ir a la Torre": "Anar a la Torre",
  "Hay datos de demostración más recientes": "Hi ha dades de demostració més recents",
  "↻ Recargar": "↻ Recarregar",
  "Ahora no": "Ara no",
  "Fechas previstas": "Dates previstes",
  "Crear orden": "Crear ordre",
  Oficio: "Ofici",
  Adjudicado: "Adjudicat",
  "Crear subcontrata": "Crear subcontracta",
  "Documentación obligatoria": "Documentació obligatòria",
  Renovar: "Renovar",
  Certificaciones: "Certificacions",
  Acciones: "Accions",
  "Iniciar en obra": "Iniciar a l'obra",
  Certificar: "Certificar",
  "Aprobar factura": "Aprovar factura",
  "Marcar completada": "Marcar completada",
  Ampliar: "Ampliar",
  "Liberar retención": "Alliberar retenció",
  Valorar: "Valorar",
  "Ver adenda": "Veure addenda",
  "Aplicar a planificación": "Aplicar a la planificació",
  "Guardar valoración": "Desar valoració",
  "← Semana anterior": "← Setmana anterior",
  "Semana siguiente →": "Setmana següent →",
  "↻ Repetir día anterior": "↻ Repetir dia anterior",
  "＋ Línea": "＋ Línia",
  "🔓 Reabrir semana": "🔓 Reobrir setmana",
  "🔒 Aprobar semana": "🔒 Aprovar setmana",
  "Añadir / renovar": "Afegir / renovar",
  "Retirar del catálogo": "Retirar del catàleg",
  Imágenes: "Imatges",
  "Guardar precio": "Desar preu",
  "El precio": "El preu",
  "Por qué no se puede editar": "Per què no es pot editar",
  "＋ Registrar un precio nuevo de esta subpartida":
    "＋ Registrar un preu nou d'aquesta subpartida",
  "Guardar trabajador": "Desar treballador",
  Tarifas: "Tarifes",
  "＋ Añadir tarifa": "＋ Afegir tarifa",
  Documentación: "Documentació",
  "🪪 Añadir / renovar documento": "🪪 Afegir / renovar document",
  "Marcar como internos": "Marcar com a interns",
  "🔒 Cerrar periodo": "🔒 Tancar període",
  Extracto: "Extracte",
  Propuestas: "Propostes",
  Conciliar: "Conciliar",
  "Asignar a proyecto": "Assignar a projecte",
  Clasificar: "Classificar",
  "Marcar sin respaldo": "Marcar sense justificant",
  Candidatos: "Candidats",
  "Periodos cerrados": "Períodes tancats",
  "Encolar pendientes": "Encuar pendents",
  Biblioteca: "Biblioteca",
  "Guardar como nueva versión": "Desar com a versió nova",
  Previsualización: "Previsualització",
  "Crear plantilla": "Crear plantilla",
  Reglas: "Regles",
  Regla: "Regla",
  Momento: "Moment",
  Simular: "Simular",
  "Crear regla": "Crear regla",
  Cola: "Cua",
  "Registrar envío": "Registrar enviament",
  "+ subpartida": "+ subpartida",
  "En uso": "En ús",
  "＋ Añadir": "＋ Afegir",
  "↻ Derivar del presupuesto": "↻ Derivar del pressupost",
  "Curva S": "Corba S",
  Desviaciones: "Desviacions",
  Merma: "Minva",
  Envase: "Envàs",
  Componentes: "Components",
  "Coste 10 ud": "Cost 10 u.",
  "Dónde se usa": "On s'utilitza",
  "El símbolo no se puede cambiar.": "El símbol no es pot canviar.",
  "Retirar, no borrar": "Retirar, no esborrar",
  Fuentes: "Fonts",
  "Motivos de pérdida": "Motius de pèrdua",
  "Antes existían, pero en el código": "Abans existien, però al codi",
  "Retirar una forma de pago": "Retirar una forma de pagament",
  "el más barato": "el més barat",
  "＋ Nuevo precio": "＋ Nou preu",
  "Ver todas": "Veure-les totes",
  "Cód. art.": "Cod. art.",
  Transporte: "Transport",
  Residuos: "Residus",
  "Reabrir trimestre": "Reobrir trimestre",
  "Consulta a gestoría": "Consulta a la gestoria",
  "Excepciones →": "Excepcions →",
  Responder: "Respondre",
  "← Contenido": "← Contingut",
  "IVA, IRPF y envío →": "IVA, IRPF i enviament →",
  "← Excepciones": "← Excepcions",
  "Qué puede hacer cada permiso": "Què pot fer cada permís",
  "Sólo en el servidor": "Només al servidor",
  "No se ha podido leer la lista": "No s'ha pogut llegir la llista",
  "＋ Nuevo usuario": "＋ Nou usuari",
  Correo: "Correu",
  "Dado de alta por": "Donat d'alta per",
  "Copiar enlace": "Copiar enllaç",
  "Crear e invitar": "Crear i convidar",
  "🗑 Eliminar": "🗑 Eliminar",
  "Guardar próxima acción": "Desar propera acció",
  "＋ Añadir medición": "＋ Afegir mesurament",
  "Notas de la visita": "Notes de la visita",
  "Completar visita": "Completar visita",
  "＋ Crear presupuesto": "＋ Crear pressupost",
  "Coste a fin de obra": "Cost a fi d'obra",
  "Guardar ajuste": "Desar ajust",
  "Volver a la calculada": "Tornar a la calculada",
  "La versión está congelada": "La versió està congelada",
  "Las imágenes no se imprimen en la fila de la subpartida":
    "Les imatges no s'imprimeixen a la fila de la subpartida",
  "Coste interno base": "Cost intern base",
  "Sólo entra lo que tiene fecha comprometida": "Només hi entra el que té data compromesa",
  "Petición del cliente": "Petició del client",
  "estándar / alta": "estàndard / alta",
  "el suyo, no el nuestro": "el seu, no el nostre",
  "se calcula solo": "es calcula sol",
  "sin dato": "sense dada",
  "sin mínimo": "sense mínim",
  "nº de oferta, albarán o factura": "núm. d'oferta, albarà o factura",
  "código de obra, si el precio es específico": "codi d'obra, si el preu és específic",
  "Nº de proyecto": "Núm. de projecte",
  "Seguimiento de presupuesto": "Seguiment de pressupost",
  "Aviso de factura vencida a 3 días": "Avís de factura vençuda a 3 dies",
  "Pie de imagen (opcional)": "Peu d'imatge (opcional)",
  "Buscar partida…": "Cercar partida…",
  "Buscar por código, descripción o marca": "Cercar per codi, descripció o marca",
  "Cómo aparecerá en el registro de actividad": "Com apareixerà al registre d'activitat",
  "p. ej. Reformas": "p. ex. Reformes",
  "p. ej. Materiales": "p. ex. Materials",
  "Croquis a mano, estado del inmueble, supuestos…": "Croquis a mà, estat de l'immoble, supòsits…",
  "Copia las horas del día anterior al día seleccionado":
    "Copia les hores del dia anterior al dia seleccionat",
  "Las líneas de presupuesto guardan el código; no se cambia.":
    "Les línies de pressupost desen el codi; no es canvia.",
  "Bloquea el periodo; exige cero movimientos sin conciliar":
    "Bloqueja el període; exigeix zero moviments sense conciliar",
  días: "dies",
  "Vuelve a derivar la planificación de las partidas y subpartidas del presupuesto aceptado, conservando el avance registrado":
    "Torna a derivar la planificació de les partides i subpartides del pressupost acceptat, conservant l'avanç registrat",
  "Desvío frente a la línea base, en días laborables":
    "Desviació respecte a la línia base, en dies laborables",
  "Congelar la planificación actual como referencia":
    "Congelar la planificació actual com a referència",
  "Arrastra para reordenar": "Arrossega per reordenar",
  "Hay excepciones sin justificar": "Hi ha excepcions sense justificar",
  "La retención depende del perfil del proveedor (profesional / empresarial / módulos). Pendiente de confirmar con la gestoría — ver LEGAL_REVIEW.md §3.":
    "La retenció depèn del perfil del proveïdor (professional / empresarial / mòduls). Pendent de confirmar amb la gestoria — vegeu LEGAL_REVIEW.md §3.",
  "Curva S de avance": "Corba S d'avanç",
  "El proyecto no tiene presupuesto aceptado del que derivar":
    "El projecte no té pressupost acceptat del qual derivar",
  "Indica la cantidad recibida": "Indica la quantitat rebuda",
  "Indica el importe a devolver": "Indica l'import a retornar",
  "Selecciona un proveedor": "Selecciona un proveïdor",
  "Selecciona un industrial": "Selecciona un industrial",
  "El oficio es obligatorio": "L'ofici és obligatori",
  "Indica el importe certificado": "Indica l'import certificat",
  "Indica el número y el importe de la factura": "Indica el número i l'import de la factura",
  "La descripción es obligatoria": "La descripció és obligatòria",
  "El día anterior no tiene horas que repetir": "El dia anterior no té hores per repetir",
  "Selecciona el proyecto": "Selecciona el projecte",
  "Indica las horas": "Indica les hores",
  "Indica el tipo de documento": "Indica el tipus de document",
  "El código es obligatorio": "El codi és obligatori",
  "El precio de lista debe ser mayor que cero": "El preu de llista ha de ser més gran que zero",
  "Indica la tarifa inicial": "Indica la tarifa inicial",
  "Indica la tarifa": "Indica la tarifa",
  "Escribe el número de proyecto": "Escriu el número de projecte",
  "La clave es obligatoria": "La clau és obligatòria",
  "Crea una plantilla antes de la regla": "Crea una plantilla abans de la regla",
  "⚠ Módulo de planificación no disponible": "⚠ Mòdul de planificació no disponible",
  "⚠ Primero hay que dar de alta alguna subpartida": "⚠ Primer cal donar d'alta alguna subpartida",
  "⚠ Primero hay que dar de alta algún proveedor": "⚠ Primer cal donar d'alta algun proveïdor",
  "Exportado a Excel": "Exportat a Excel",
  "Exportado a CSV": "Exportat a CSV",
  Guardado: "Desat",
  "Entrada añadida": "Entrada afegida",
  "Enlace copiado": "Enllaç copiat",
  "Seleccionado — copie con Ctrl+C": "Seleccionat — copieu amb Ctrl+C",
  "Copia archivada": "Còpia arxivada",
  "Sin más detalle": "Sense més detall",
  "Página de acceso": "Pàgina d'accés",
  "Contraseña temporal": "Contrasenya temporal",
  "Copiar contraseña": "Copiar contrasenya",
  "Copiar todo": "Copiar-ho tot",
  "Contraseña copiada": "Contrasenya copiada",
  "Datos de acceso copiados": "Dades d'accés copiades",
  "Perfil y ayuda —": "Perfil i ajuda —",
  "Funciona ya. Es temporal y tú la conoces, así que pídele que la cambie al entrar.":
    "Funciona ja. És temporal i tu la coneixes, així que demana-li que la canviï en entrar.",
  "No se ha podido asignar una contraseña a esta cuenta, así que la única entrada es el enlace de abajo.":
    "No s'ha pogut assignar una contrasenya a aquest compte, així que l'única entrada és l'enllaç de sota.",
  "Se creará la cuenta con una contraseña temporal y se dejará el correo escrito en Borradores.":
    "Es crearà el compte amb una contrasenya temporal i es deixarà el correu escrit a Esborranys.",
  "Usuario actualizado": "Usuari actualitzat",
  "Cliente eliminado": "Client eliminat",
  "Datos de demostración recargados — 2024 a 2026":
    "Dades de demostració recarregades — 2024 a 2026",
  "Origen *": "Origen *",
  WhatsApp: "WhatsApp",
  Normal: "Normal",
  Docs: "Docs",
  Evento: "Esdeveniment",
  Plantilla: "Plantilla",
  Base: "Base",
  Por: "Per",
  "Un PDF con texto se lee al instante y sin descargar nada. Una foto o un escaneado necesitan el lector de imágenes":
    "Un PDF amb text es llegeix a l'instant i sense descarregar res. Una foto o un escaneig necessiten el lector d'imatges",
  "Suelo radiante en baño": "Terra radiant al bany",
  "PRL, alta S.S., seguro…": "PRL, alta S.S., assegurança…",
  "Reforma integral de baño": "Reforma integral de bany",
  baño: "bany",
  "Reparación de humedades": "Reparació d'humitats",
  "Llamar al cliente con la valoración": "Trucar al client amb la valoració",
  "La parte cara ya está ejecutada": "La part cara ja està executada",
  "¿Recargar los datos de demostración?\n\nSe sustituye TODO lo que haya ahora en este dispositivo por el conjunto de ejemplo: dos años de actividad (2024-2025) más la situación actual de 2026.":
    "Recarregar les dades de demostració?\n\nEs substitueix TOT el que hi hagi ara en aquest dispositiu pel conjunt d'exemple: dos anys d'activitat (2024-2025) més la situació actual del 2026.",
  Empieza: "Comença",
  "Duración (días laborables)": "Durada (dies laborables)",
  "Nombre *": "Nom *",
  "Ya congeladas": "Ja congelades",
  "Avance %": "Avanç %",
  "Proveedor *": "Proveïdor *",
  "Fin previsto": "Fi prevista",
  "Nueva fecha fin": "Nova data de fi",
  "Descripción *": "Descripció *",
  "Código *": "Codi *",
  "Partida *": "Partida *",
  "Nueva tarifa (€/hora)": "Nova tarifa (€/hora)",
  "Vigente desde": "Vigent des de",
  Asunto: "Assumpte",
  Idioma: "Idioma",
  "Clave *": "Clau *",
  Canal: "Canal",
  Roles: "Rols",
  Fijo: "Fix",
  "Origen del contacto": "Origen del contacte",
  "IBAN (opcional — el cambio queda registrado)": "IBAN (opcional — el canvi queda registrat)",
  "Cliente *": "Client *",
  "Trabajo solicitado *": "Treball sol·licitat *",
  "Valor estimado (€)": "Valor estimat (€)",
  Hora: "Hora",
  "Descripción del trabajo *": "Descripció del treball *",
  "Valor de venta (€) *": "Valor de venda (€) *",
  "Tarea *": "Tasca *",
  "Añadir a la planificación": "Afegir a la planificació",
  Congelar: "Congelar",
  "Soltar fecha": "Alliberar la data",
  "Depende de": "Depèn de",
  "Eliminar de la planificación": "Eliminar de la planificació",
  "＋ Nueva subcontrata": "＋ Nova subcontracta",
  Subcontratas: "Subcontractes",
  Certificado: "Certificat",
  "＋ Asignar trabajador": "＋ Assignar treballador",
  "Guardar cambios": "Desar canvis",
  "＋ Nueva regla": "＋ Nova regla",
  "◆ Hito": "◆ Fita",
  "Línea base": "Línia base",
  "❄ Congelar": "❄ Congelar",
  "Mín.": "Mín.",
  "✏️ Editar datos": "✏️ Editar dades",
  "Crear oportunidad": "Crear oportunitat",
  Oportunidad: "Oportunitat",
  "Crear proyecto": "Crear projecte",
  "Crear tarea": "Crear tasca",
  "Valor estimado": "Valor estimat",
  Fontanería: "Lampisteria",
  "Ponle un nombre": "Posa-li un nom",
  "La línea base necesita un nombre": "La línia base necessita un nom",
  "El nombre es obligatorio": "El nom és obligatori",
  "El tercero necesita al menos un rol": "El tercer necessita almenys un rol",
  "Hace falta un cliente": "Cal un client",
  "Describe el trabajo solicitado": "Descriu el treball sol·licitat",
  "Describe el trabajo": "Descriu el treball",
  "El valor de venta debe ser mayor que cero": "El valor de venda ha de ser més gran que zero",
  "La tarea necesita un título": "La tasca necessita un títol",
  Facturación: "Facturació",
  "Lista de precios": "Llista de preus",
  Comunicaciones: "Comunicacions",
  "Pavimento cerámico": "Paviment ceràmic",
  "Estado actual": "Estat actual",
  "Carta de color": "Carta de color",
  "Detalle interno": "Detall intern",
  "en preparación": "en preparació",
  "ERP › Torre de control": "ERP › Torre de control",
  "Periodo aplicado:": "Període aplicat:",
  "Planificación derivada del presupuesto": "Planificació derivada del pressupost",
  "duraciones derivadas de las cantidades": "durades derivades de les quantitats",
  "Avance de la subpartida registrado": "Avanç de la subpartida registrat",
  "Cantidad ejecutada registrada": "Quantitat executada registrada",
  "Alta S.S.": "Alta S.S.",
  "Recibida parcial": "Rebuda parcial",
  "Orden enviada": "Comanda enviada",
  "Aceptación registrada": "Acceptació registrada",
  "Recepción registrada": "Recepció registrada",
  "Orden duplicada": "Comanda duplicada",
  "Orden anulada": "Comanda anul·lada",
  "Devolución registrada": "Devolució registrada",
  "Documento de": "Document de",
  "Orden de compra creada": "Comanda de compra creada",
  "Subcontratas por oficio: adjudicado, certificado, facturado y pendiente, con la documentación obligatoria y su vigencia.":
    "Subcontractes per ofici: adjudicat, certificat, facturat i pendent, amb la documentació obligatòria i la seva vigència.",
  "Subcontrata creada": "Subcontracta creada",
  "sin registrar": "sense registrar",
  "Documento renovado": "Document renovat",
  "Subcontrata enviada": "Subcontracta enviada",
  "Inicio registrado": "Inici registrat",
  "Certificación registrada": "Certificació registrada",
  "Factura aprobada": "Factura aprovada",
  "Subcontrata completada": "Subcontracta completada",
  "Fecha ampliada": "Data ampliada",
  "Subcontrata rescindida": "Subcontracta rescindida",
  "Retención liberada": "Retenció alliberada",
  "Modificación enviada": "Modificació enviada",
  "Modificación rechazada": "Modificació rebutjada",
  "Modificación anulada": "Modificació anul·lada",
  "La partida": "La partida",
  "no tiene todavía una tarea derivada — deriva la planificación primero":
    "encara no té una tasca derivada — deriva la planificació primer",
  "Planificación actualizada": "Planificació actualitzada",
  "Valorar ·": "Valorar ·",
  "Modificación valorada": "Modificació valorada",
  Mié: "Dc",
  Sáb: "Ds",
  "Jornada repetida desde el día anterior": "Jornada repetida del dia anterior",
  "Horas actualizadas": "Hores actualitzades",
  "Semana aprobada": "Setmana aprovada",
  "Semana reabierta": "Setmana reoberta",
  "Añadir horas ·": "Afegir hores ·",
  "Horas registradas": "Hores registrades",
  "Hay más horas imputadas que nóminas pagadas: falta cargar una nómina del mes, o hay horas fechadas en el mes equivocado.":
    "Hi ha més hores imputades que nòmines pagades: falta carregar una nòmina del mes, o hi ha hores datades al mes equivocat.",
  "Proyecto (código) — deja igual para no cambiar":
    "Projecte (codi) — deixa-ho igual per no canviar-ho",
  "Proyecto no encontrado:": "Projecte no trobat:",
  "Horas reasignadas": "Hores reassignades",
  "gestor, no aviso": "gestor, no avís",
  "Sin proyecto": "Sense projecte",
  "¿A quién se asigna esta alerta?": "A qui s'assigna aquesta alerta?",
  "Alerta asignada": "Alerta assignada",
  "Fecha límite (AAAA-MM-DD):": "Data límit (AAAA-MM-DD):",
  "Fecha límite guardada": "Data límit desada",
  "Posponer hasta (AAAA-MM-DD):": "Ajornar fins a (AAAA-MM-DD):",
  "Motivo (opcional):": "Motiu (opcional):",
  "Alerta pospuesta": "Alerta ajornada",
  "Nota de resolución (obligatoria):": "Nota de resolució (obligatòria):",
  "Alerta resuelta": "Alerta resolta",
  "Vence (AAAA-MM-DD):": "Venç (AAAA-MM-DD):",
  "Convertida en tarea": "Convertida en tasca",
  "editor de reglas: condición, umbral, destinatario y canal":
    "editor de regles: condició, llindar, destinatari i canal",
  "Reglas de alertas": "Regles d'alertes",
  "Umbral actualizado": "Llindar actualitzat",
  "Destinatario actualizado": "Destinatari actualitzat",
  "Canal actualizado": "Canal actualitzat",
  "filas por pantalla": "files per pantalla",
  "Ningún registro coincide con la búsqueda.": "Cap registre coincideix amb la cerca.",
  "Quitar de favoritos": "Treure de preferits",
  "(imagen no disponible)": "(imatge no disponible)",
  página: "pàgina",
  "(opcional — aparte)": "(opcional — a part)",
  "(fuera de presupuesto)": "(fora de pressupost)",
  "(extras)": "(extres)",
  "Base imposable": "Base imposable",
  "Partida añadida": "Partida afegita",
  Revisión: "Revisió",
  "Sin incidencias": "Sense incidències",
  "partida seleccionada": "partida seleccionata",
  "Subpartida añadida": "Subpartida afegida",
  "Subpartida eliminada": "Subpartida eliminada",
  "página(s)": "pàgina(es)",
  "% de confianza": "% de confiança",
  "Ruta completa": "Ruta completa",
  "Validación ·": "Validació ·",
  catálogo: "catàleg",
  cámara: "càmera",
  "el anexo está activo": "l'annex està actiu",
  "el anexo está desactivado": "l'annex està desactivat",
  "Pie actualizado": "Peu actualitzat",
  "Imagen marcada como interna": "Imatge marcada com a interna",
  "Imagen incluida en el anexo": "Imatge inclosa a l'annex",
  "Orden actualizado": "Ordre actualitzat",
  "Imagen eliminada": "Imatge eliminada",
  "Imagen del catálogo añadida": "Imatge del catàleg afegida",
  "Foto de la visita añadida": "Foto de la visita afegida",
  "imagen añadida": "imatge afegida",
  "imágenes añadidas": "imatges afegides",
  "en uso": "en ús",
  Símbolo: "Símbol",
  "Feria local": "Fira local",
  "forma de pago": "forma de pagament",
  "Transferencia 15 días": "Transferència 15 dies",
  "facturas de proveedores": "factures de proveïdors",
  "Lista actualizada": "Llista actualitzada",
  "Entrada reactivada": "Entrada reactivada",
  "Entrada retirada": "Entrada retirada",
  "Registro de clientes. Los datos obligatorios se validan antes de facturar o contratar.":
    "Registre de clients. Les dades obligatòries es validen abans de facturar o contractar.",
  "Ningún cliente coincide con la búsqueda.": "Cap client coincideix amb la cerca.",
  "Buscar proveedor por nombre o NIF": "Cercar proveïdor per nom o NIF",
  "Ningún proveedor coincide con la búsqueda.": "Cap proveïdor coincideix amb la cerca.",
  "Buscar industrial por nombre o NIF": "Cercar industrial per nom o NIF",
  "Ninguna subcontrata coincide con la búsqueda.": "Cap subcontracta coincideix amb la cerca.",
  "Buscar por nombre": "Cercar per nom",
  "Ningún trabajador coincide con la búsqueda.": "Cap treballador coincideix amb la cerca.",
  "Orden de partidas actualizado": "Ordre de partides actualitzat",
  "Pago registrado:": "Pagament registrat:",
  "todavía sin ningún precio": "encara sense cap preu",
  "Tres pasos: qué contiene el trimestre, qué excepciones lo bloquean, y las cifras de IVA e IRPF antes de enviar.":
    "Tres passos: què conté el trimestre, quines excepcions el bloquegen, i les xifres d'IVA i IRPF abans d'enviar.",
  "sin excepciones": "sense excepcions",
  "¿Por qué puede enviarse el paquete con esta excepción?":
    "Per què es pot enviar el paquet amb aquesta excepció?",
  "Excepción justificada": "Excepció justificada",
  "Respuesta de la gestoría:": "Resposta de la gestoria:",
  "Consulta resuelta": "Consulta resolta",
  "¿Qué se pregunta a la gestoría sobre": "Què es pregunta a la gestoria sobre",
  "Consulta registrada": "Consulta registrada",
  "Motivo para reabrir": "Motiu per reobrir",
  "(queda registrado):": "(queda registrat):",
  "¿A quién se envía el paquete": "A qui s'envia el paquet",
  "generado y registrado": "generat i registrat",
  "Todo, incluida esta pantalla.": "Tot, inclosa aquesta pantalla.",
  "Presupuestos, facturas, cobros y pagos. No gestiona usuarios.":
    "Pressupostos, factures, cobraments i pagaments. No gestiona usuaris.",
  "Registrar avance, partes y fotos. Sin importes ni márgenes.":
    "Registrar avanç, parts i fotos. Sense imports ni marges.",
  "Sólo lectura y exportación. Nunca ve márgenes ni precios de venta.":
    "Només lectura i exportació. Mai no veu marges ni preus de venda.",
  "Altas, bajas y permisos. Al crear una cuenta, la persona elige su propia contraseña: nadie más la conoce.":
    "Altes, baixes i permisos. En crear un compte, la persona tria la seva pròpia contrasenya: ningú més no la coneix.",
  "Reenviar invitación": "Reenviar invitació",
  "Restablecer contraseña": "Restablir contrasenya",
  "Invitación para": "Invitació per a",
  "Cliente desactivado": "Client desactivat",
  "Guardar cliente": "Desar client",
  "Cliente creado": "Client creat",
  "Guardar proveedor": "Desar proveïdor",
  "Proveedor creado": "Proveïdor creat",
  "Guardar industrial": "Desar industrial",
  "Industrial creado": "Industrial creat",
  "un registro existente": "un registre existent",
  "Programar visita": "Programar visita",
  "Motivo de la pérdida (código:": "Motiu de la pèrdua (codi:",
  "(aceptada)": "(acceptada)",
  "Proyección ajustada": "Projecció ajustada",
  "Ajuste retirado": "Ajust retirat",
  "Repartir ·": "Repartir ·",
  Mínimo: "Mínim",
  "toda la empresa": "tota l'empresa",
  "1 anexo": "1 annex",
  "Cobros previstos ·": "Cobraments previstos ·",
  "No se ha modificado nada. Actualiza la aplicación para volver a abrir estos datos.":
    "No s'ha modificat res. Actualitza l'aplicació per tornar a obrir aquestes dades.",
  "Esta dirección no corresponde a ninguna pantalla. Puede venir de un enlace antiguo.":
    "Aquesta adreça no correspon a cap pantalla. Pot venir d'un enllaç antic.",
  "esta versión incluye dos años de histórico (2024-2025) y más información por cliente.":
    "aquesta versió inclou dos anys d'històric (2024-2025) i més informació per client.",
  "por porcentaje o por cantidad ejecutada — lo que sepa la obra":
    "per percentatge o per quantitat executada — el que sàpiga l'obra",
  "Sin partidas aceptadas: el avance se registra sobre las tareas del gráfico.":
    "Sense partides acceptates: l'avanç es registra sobre les tasques del gràfic.",
  "Todavía no hay proyectos.": "Encara no hi ha projectes.",
  "Sin subcontratas en este proyecto.": "Sense subcontractes en aquest projecte.",
  "· Certificado": "· Certificat",
  "Sin certificaciones.": "Sense certificacions.",
  "Efecto en plazo": "Efecte en termini",
  "Nadie tiene horas ni asignación para este día.":
    "Ningú no té hores ni assignació per a aquest dia.",
  "Sin horas registradas en el periodo.": "Sense hores registrades al període.",
  "reasigna o corrige una entrada": "reassigna o corregeix una entrada",
  "Sin horas en el periodo seleccionado.": "Sense hores al període seleccionat.",
  "Sin documentación registrada.": "Sense documentació registrada.",
  "Cómo se le dice al cliente": "Com se li diu al client",
  "(si difiere)": "(si difereix)",
  "sin documento": "sense document",
  "Pedido para la obra": "Comanda per a l'obra",
  "Sin tarifas registradas.": "Sense tarifes registrades.",
  "movimientos sin conciliar en el periodo": "moviments sense conciliar al període",
  "El emparejamiento asistido necesita el módulo de capacidades, que no se ha cargado. Recarga la página.":
    "L'aparellament assistit necessita el mòdul de capacitats, que no s'ha carregat. Torna a carregar la pàgina.",
  "sin conciliar": "sense conciliar",
  "sin respaldo": "sense justificant",
  "Nada pendiente de conciliar en este periodo.": "Res pendent de conciliar en aquest període.",
  "El periodo está cerrado. Reábrelo para conciliar.":
    "El període està tancat. Reobre'l per conciliar.",
  "Ningún documento explica este movimiento. Asígnalo a un proyecto o clasifícalo abajo.":
    "Cap document explica aquest moviment. Assigna'l a un projecte o classifica'l a sota.",
  "Sin documentos abiertos en este sentido.": "Sense documents oberts en aquest sentit.",
  "Ningún periodo cerrado todavía.": "Cap període tancat encara.",
  "nada pendiente de encolar": "res pendent d'encuar",
  "Las comunicaciones necesitan el módulo de capacidades, que no se ha cargado. Recarga la página.":
    "Les comunicacions necessiten el mòdul de capacitats, que no s'ha carregat. Torna a carregar la pàgina.",
  "Sin plantillas.": "Sense plantilles.",
  "con datos reales del primer registro que encaja":
    "amb dades reals del primer registre que encaixa",
  "Selecciona o crea una plantilla.": "Selecciona o crea una plantilla.",
  "Sin reglas. Una regla dice cuándo se prepara un mensaje — nunca que se envíe solo.":
    "Sense regles. Una regla diu quan es prepara un missatge — mai que s'enviï sol.",
  "simulación — no se encola ni se envía nada": "simulació — no s'encua ni s'envia res",
  "sin destinatario": "sense destinatari",
  "Esta regla no produciría ningún mensaje con los datos de hoy.":
    "Aquesta regla no produiria cap missatge amb les dades d'avui.",
  "nada sale sin aprobación — el envío real no está conectado":
    "res no surt sense aprovació — l'enviament real no està connectat",
  "La cola está vacía.": "La cua és buida.",
  "Sin proyectos activos.": "Sense projectes actius.",
  "Sin partidas.": "Sense partides.",
  "Incluir anexo en el documento": "Incloure l'annex al document",
  "Imágenes por página": "Imatges per pàgina",
  "Nada que corregir. El presupuesto puede emitirse.":
    "Res a corregir. El pressupost es pot emetre.",
  "Sólo uso interno": "Només ús intern",
  "Esta subpartida no tiene imágenes.": "Aquesta subpartida no té imatges.",
  "La versión está congelada: su anexo se conserva tal como se emitió.":
    "La versió està congelada: el seu annex es conserva tal com es va emetre.",
  "Desde el ordenador": "Des de l'ordinador",
  "Sin tarifa": "Sense tarifa",
  "Sin docs": "Sense docs",
  "Real registrado": "Real registrat",
  "Supera el umbral": "Supera el llindar",
  "Retraso sobre línea base": "Retard sobre la línia base",
  "Tareas críticas en riesgo": "Tasques crítiques en risc",
  "Sin línea base congelada: no hay promesa contra la que medir.":
    "Sense línia base congelada: no hi ha promesa contra la qual mesurar.",
  "Sin planificación.": "Sense planificació.",
  crítica: "crítica",
  "Nada fuera de plazo.": "Res fora de termini.",
  "Ninguna subpartida coincide.": "Cap subpartida coincideix.",
  "rendimiento · merma · compra mínima · envase": "rendiment · minva · compra mínima · envàs",
  "Sin paquetes definidos.": "Sense paquets definits.",
  "En el contrato y en cada factura emitida, y es lo que se imprime como condición de pago.":
    "Al contracte i a cada factura emesa, i és el que s'imprimeix com a condició de pagament.",
  "El código es lo que queda guardado en los documentos ya emitidos, así que no se edita. El nombre sí —":
    "El codi és el que queda desat als documents ja emesos, així que no s'edita. El nom sí —",
  "nunca ha cotizado esta subpartida": "mai no ha cotitzat aquesta subpartida",
  "No hay proveedores dados de alta todavía.": "No hi ha proveïdors donats d'alta encara.",
  "se añade, nunca se sobrescribe": "s'afegeix, mai no se sobreescriu",
  "Sin precios registrados para este filtro.": "Sense preus registrats per a aquest filtre.",
  "llegaron después de cerrar el trimestre": "van arribar després de tancar el trimestre",
  "Ya declarado": "Ja declarat",
  "justifica cada una, o corrígela en su pantalla":
    "justifica'n cadascuna, o corregeix-la a la seva pantalla",
  "Sin excepciones: el paquete puede enviarse tal cual.":
    "Sense excepcions: el paquet es pot enviar tal qual.",
  "Sin consultas.": "Sense consultes.",
  Extemporáneos: "Extemporanis",
  "Consultas abiertas": "Consultes obertes",
  "fuera del coste de obra": "fora del cost d'obra",
  "Nada en este trimestre.": "Res en aquest trimestre.",
  "invitación pendiente": "invitació pendent",
  "en el servidor": "al servidor",
  "Sin usuarios todavía.": "Sense usuaris encara.",
  "No se ha enviado ningún correo": "No s'ha enviat cap correu",
  "Se creará la cuenta y se generará un enlace de activación. La persona elige su propia contraseña.":
    "Es crearà el compte i es generarà un enllaç d'activació. La persona tria la seva pròpia contrasenya.",
  "Configurado en el servidor": "Configurat al servidor",
  "Persona de contacto:": "Persona de contacte:",
  "Oculto — su perfil no tiene permiso para ver datos bancarios":
    "Ocult — el vostre perfil no té permís per veure dades bancàries",
  "Sin visitas todavía.": "Sense visites encara.",
  "Sin mediciones todavía.": "Sense mesuraments encara.",
  "Sin fotos.": "Sense fotos.",
  "Todavía no se ha creado un presupuesto desde esta visita.":
    "Encara no s'ha creat cap pressupost des d'aquesta visita.",
  "Se imprime después de los totales y antes de las condiciones. Al emitir la versión, el anexo queda congelado con ella.":
    "S'imprimeix després dels totals i abans de les condicions. En emetre la versió, l'annex queda congelat amb ella.",
  "El anexo gráfico está desactivado para este presupuesto.":
    "L'annex gràfic està desactivat per a aquest pressupost.",
  "· real": "· real",
  "· avance": "· avanç",
  "Proyección calculada": "Projecció calculada",
  "provisional — muy poco avance para extrapolar": "provisional — massa poc avanç per extrapolar",
  "Ajuste vigente": "Ajust vigent",
  "Sin partidas aceptadas": "Sense partides acceptates",
  "Entrada eliminada": "Entrada eliminada",
  "Asignar trabajador": "Assignar treballador",
  "Trabajador asignado": "Treballador assignat",
  "Documentación ·": "Documentació ·",
  "Mano de obra propia": "Mà d'obra pròpia",
  "Gestión de residuos": "Gestió de residus",
  "Crear subpartida": "Crear subpartida",
  "Esta subpartida no tiene imágenes. Las que se añadan salen en el anexo gráfico del presupuesto, nunca dentro de la fila.":
    "Aquesta subpartida no té imatges. Les que s'afegeixin surten a l'annex gràfic del pressupost, mai dins la fila.",
  "Ya existe una subpartida con el código": "Ja existeix una subpartida amb el codi",
  "Subpartida actualizada": "Subpartida actualitzada",
  "Subpartida creada": "Subpartida creada",
  "Subpartida retirada del catálogo": "Subpartida retirada del catàleg",
  "Portal del proveedor": "Portal del proveïdor",
  "Factura de compra": "Factura de compra",
  "Confirmación manual": "Confirmació manual",
  "Precio registrado": "Preu registrat",
  "Trabajador creado": "Treballador creat",
  "Trabajador reactivado": "Treballador reactivat",
  "Trabajador desactivado": "Treballador desactivat",
  "Tarifa añadida": "Tarifa afegida",
  "fecha próxima": "data propera",
  "Movimiento conciliado": "Moviment conciliat",
  "Marcado sin respaldo — tarea creada para reclamarlo":
    "Marcat sense justificant — s'ha creat una tasca per reclamar-lo",
  "traspaso(s) marcados — no cuentan como ingreso ni gasto":
    "traspàs(os) marcats — no compten com a ingrés ni despesa",
  "movimiento(s) identificados como gasto general":
    "moviment(s) identificats com a despesa general",
  "movimiento(s) marcados sin respaldo — una tarea por cada uno":
    "moviment(s) marcats sense justificant — una tasca per a cada un",
  "Periodo cerrado": "Període tancat",
  "Periodo reabierto": "Període reobert",
  "Presupuesto enviado": "Pressupost enviat",
  "Factura vencida": "Factura vençuda",
  "Ese cliente no paga esta obra": "Aquest client no paga aquesta obra",
  "Añádalo como pagador del proyecto antes de emitirle una factura.":
    "Afegiu-lo com a pagador del projecte abans d'emetre-li una factura.",
  "Datos fiscales del cliente incompletos": "Dades fiscals del client incompletes",
  "Faltan los datos de la empresa": "Falten les dades de l'empresa",
  "No cuadra": "No quadra",
  "La tabla de arriba no cuadra con el coste del proyecto.":
    "La taula de dalt no quadra amb el cost del projecte.",
  "— sin obra asociada —": "— sense obra associada —",
  "Un contrato externo no nombra ningún presupuesto, así que la obra a la que pertenece hay que decirla aquí — o dejarlo sin asociar.":
    "Un contracte extern no anomena cap pressupost, així que l'obra a la qual pertany s'ha de dir aquí — o deixar-ho sense associar.",
  "⚠ El logotipo tiene que ser una imagen (PNG o JPG)":
    "⚠ El logotip ha de ser una imatge (PNG o JPG)",
  "Estos valores eran fijos dentro del programa y ahora se leen de aquí. Un presupuesto o un contrato que ya existe conserva los suyos.":
    "Aquests valors eren fixos dins del programa i ara es llegeixen d'aquí. Un pressupost o un contracte que ja existeix conserva els seus.",
  "Quién emite los documentos de esta empresa, dónde está, cómo se le paga y con qué valores por defecto trabaja. Todo lo que sale impreso — presupuestos, contratos y facturas — se lee de aquí.":
    "Qui emet els documents d'aquesta empresa, on és, com se li paga i amb quins valors per defecte treballa. Tot el que surt imprès — pressupostos, contractes i factures — es llegeix d'aquí.",
  Logotipo: "Logotip",
  Tomo: "Tom",
  Folio: "Foli",
  Hoja: "Full",
  "Texto que se imprime al pie de cada documento": "Text que s'imprimeix al peu de cada document",
  "Dirección de trabajo": "Adreça de treball",
  "Valores por defecto": "Valors per defecte",
  "Pie legal": "Peu legal",
  "Arrastra aquí el logotipo (PNG o JPG), o búscalo en el dispositivo.":
    "Arrossega aquí el logotip (PNG o JPG), o cerca'l al dispositiu.",
  "la denominación social": "la denominació social",
  "el NIF": "el NIF",
  "la dirección": "l'adreça",
  "el IBAN": "l'IBAN",
  "Denominación social *": "Denominació social *",
  "Domicilio social *": "Domicili social *",
  "BIC / SWIFT": "BIC / SWIFT",
  "IVA por defecto (%)": "IVA per defecte (%)",
  "IRPF por defecto (%)": "IRPF per defecte (%)",
  "Interés de demora (% anual)": "Interès de demora (% anual)",
  "Días de cortesía": "Dies de cortesia",
  "Planificar en (días)": "Planificar en (dies)",
  "Empezar en (días)": "Començar en (dies)",
  "Margen mínimo, alerta (%)": "Marge mínim, alerta (%)",
  "Datos de la empresa guardados": "Dades de l'empresa desades",
  "sin ellos no se puede emitir ninguna factura.": "sense elles no es pot emetre cap factura.",
  "Sin estos datos no se puede emitir ninguna factura.":
    "Sense aquestes dades no es pot emetre cap factura.",
  "La empresa puede emitir presupuestos, contratos y facturas.":
    "L'empresa pot emetre pressupostos, contractes i factures.",
  "el que consta en el Registro": "el que consta al Registre",
  "la que va en la cabecera": "la que va a la capçalera",
  "Igual que el domicilio social": "Igual que el domicili social",
  "va al pie de los documentos": "va al peu dels documents",
  "se aplican a lo nuevo, no a lo ya emitido": "s'apliquen al que és nou, no al que ja s'ha emès",
  "Complete el nombre fiscal, el NIF, la dirección y el IBAN en Configuración › Empresa. La factura lleva la identidad del emisor y no puede emitirse sin ella.":
    "Ompliu el nom fiscal, el NIF, l'adreça i l'IBAN a Configuració › Empresa. " +
    "La factura porta la identitat de l'emissor i no es pot emetre sense ella.",
  "Obra finalizada": "Obra finalitzada",
  "Documentación de subcontrata caducada": "Documentació de subcontracta caducada",
  "Cola y historial": "Cua i historial",
  "mensaje(s) en cola — ninguno se ha enviado": "missatge(s) a la cua — cap no s'ha enviat",
  "previsualización con datos reales": "previsualització amb dades reals",
  "Plantilla creada": "Plantilla creada",
  "Regla actualizada": "Regla actualitzada",
  "Regla creada": "Regla creada",
  "probar una regla en modo simulación": "provar una regla en mode simulació",
  "Simulación ·": "Simulació ·",
  "sin dirección": "sense adreça",
  "Registrado como enviado": "Registrat com a enviat",
  "Sin destinatario": "Sense destinatari",
  "Aprobado — sigue sin enviarse": "Aprovat — encara sense enviar",
  "el umbral de cada indicador": "el llindar de cada indicador",
  "Proyección 30 d": "Projecció 30 d",
  "Prioridad actualizada": "Prioritat actualitzada",
  "exportar la vista a PDF/Excel": "exportar la vista a PDF/Excel",
  "· facturado": "· facturat",
  "fin → inicio": "fi → inici",
  "inicio → inicio": "inici → inici",
  "fin → fin": "fi → fi",
  "evento → plantilla → destinatario → momento → canal":
    "esdeveniment → plantilla → destinatari → moment → canal",
  "mensaje(s) que las reglas producirían y todavía no están en la cola":
    "missatge(s) que les regles produirien i encara no són a la cua",
  "Programar visita —": "Programar visita —",
  "Visita —": "Visita —",
  "Persona de contacto": "Persona de contacte",
  "🚚 Nuevo proveedor": "🚚 Nou proveïdor",
  "👥 Nuevo tercero": "👥 Nou tercer",
  "Nueva tarea": "Nova tasca",
  "Dependencia creada": "Dependència creada",
  "Tarea movida — la planificación se ha recalculado":
    "Tasca moguda — la planificació s'ha recalculat",
  "Nuevo hito": "Nova fita",
  "Nueva tarea de obra": "Nova tasca d'obra",
  "Hito añadido": "Fita afegida",
  "Tarea añadida": "Tasca afegida",
  "Congelar línea base": "Congelar línia base",
  "Línea base congelada": "Línia base congelada",
  "Tarea actualizada": "Tasca actualitzada",
  "La tarea vuelve a su fecha más temprana": "La tasca torna a la seva data més primerenca",
  "Dependencia eliminada": "Dependència eliminada",
  "Tarea eliminada": "Tasca eliminada",
  "Datos actualizados": "Dades actualitzades",
  facturación: "facturació",
  Categoría: "Categoria",
  "Día no laborable añadido": "Dia no laborable afegit",
  "Día no laborable quitado": "Dia no laborable tret",
  "Tercero desactivado": "Tercer desactivat",
  "Tercero reactivado": "Tercer reactivat",
  "No aplica": "No s'aplica",
  "Nuevo cliente": "Nou client",
  "Nuevo proveedor": "Nou proveïdor",
  "Nueva oportunidad": "Nova oportunitat",
  "Oportunidad creada": "Oportunitat creada",
  "Nuevo proyecto": "Nou projecte",
  "Proyecto creado": "Projecte creat",
  "Tarea creada": "Tasca creada",
  "Sin resultados": "Sense resultats",
  "Sin alertas abiertas.": "Sense alertes obertes.",
  "◆ Hitos de cobro": "◆ Fites de cobrament",
  "Todavía no hay proyectos que planificar.": "Encara no hi ha projectes per planificar.",
  "Fin de obra": "Fi d'obra",
  "Hito de cobro del contrato": "Fita de cobrament del contracte",
  "Día no laborable": "Dia no laborable",
  "DÍAS NO LABORABLES": "DIES NO LABORABLES",
  "IRPF % (si aplica)": "IRPF % (si s'aplica)",
  "Para una obra con presupuesto formal, créala desde el presupuesto aceptado: así conserva su línea base.":
    "Per a una obra amb pressupost formal, crea-la des del pressupost acceptat: així conserva la seva línia base.",
  App: "App",
  "Caja no cubre los pagos previstos": "La caixa no cobreix els pagaments previstos",
  "Partida por encima de coste previsto": "Partida per sobre del cost previst",
  "Certificado por encima de lo adjudicado": "Certificat per sobre de l'adjudicat",
  "Diferencias en la conciliación de una orden": "Diferències en la conciliació d'una comanda",
  "Documentación caducada": "Documentació caducada",
  "Documentación caducada de un trabajador": "Documentació caducada d'un treballador",
  "Documentación caducada de una subcontrata": "Documentació caducada d'una subcontracta",
  "Duración contractual excedida": "Durada contractual excedida",
  "Subcontrata sin factura tras varios días": "Subcontracta sense factura després de diversos dies",
  "Extra sin aprobar con coste incurrido": "Extra sense aprovar amb cost incorregut",
  "Facturas de proveedor sin asignar": "Factures de proveïdor sense assignar",
  "Fecha de inicio comprometida en riesgo": "Data d'inici compromesa en risc",
  "Garantía próxima a vencer": "Garantia a punt de vèncer",
  "Hito de cobro vencido": "Fita de cobrament vençuda",
  "Jornadas sin registrar": "Jornades sense registrar",
  "Llegada de material retrasada": "Arribada de material endarrerida",
  "Margen bajo el umbral configurado": "Marge per sota del llindar configurat",
  "Margen negativo": "Marge negatiu",
  "Movimientos bancarios sin asignar": "Moviments bancaris sense assignar",
  "Obra iniciada sin contrato firmado": "Obra iniciada sense contracte signat",
  "Oportunidad sin avance": "Oportunitat sense avanç",
  "Paquete trimestral sin enviar a gestoría": "Paquet trimestral sense enviar a la gestoria",
  "Posible factura duplicada": "Possible factura duplicada",
  "Presupuesto con líneas pendientes": "Pressupost amb línies pendents",
  "Presupuesto por caducar": "Pressupost a punt de caducar",
  "Retención no liberada tras su plazo": "Retenció no alliberada després del seu termini",
  "Subcontrata sin factura": "Subcontracta sense factura",
  Ninguno: "Cap",
  Nº: "Núm.",
  Plantillas: "Plantilles",
  Posventa: "Postvenda",
  "Recientes:": "Recents:",
  Retirar: "Retirar",
  Ud: "Un",
  activa: "activa",
  Fiscal: "Fiscal",
  ERP: "ERP",
  "Canei Subirats": "Canei Subirats",
  "Canei Subirats — ERP": "Canei Subirats — ERP",
  "Fira local": "Fira local",
  Termini: "Termini",
  "Transferència 15 dies": "Transferència 15 dies",
  feriaLocal: "feriaLocal",
  plazo: "plazo",
  "{{cliente}}": "{{cliente}}",
  "{{fecha}}": "{{fecha}}",
  "{{importe}}": "{{importe}}",
  "{{number}}": "{{number}}",
  "Asignar, poner fecha, posponer con motivo, resolver con nota y evidencia, o convertir en tarea. Las reglas viven detrás de ⚙ Reglas.":
    "Assignar, posar data, ajornar amb motiu, resoldre amb nota i evidència, o convertir en tasca. Les regles viuen darrere de ⚙ Regles.",
  "Cada subpartida del catálogo y cada línea de presupuesto llevan una unidad. Es lo que se imprime junto a la cantidad en el presupuesto y en la factura.":
    "Cada subpartida del catàleg i cada línia de pressupost porten una unitat. És el que s'imprimeix al costat de la quantitat al pressupost i a la factura.",
  "De dónde llegan los clientes y por qué se pierden las oportunidades. Las dos listas alimentan los informes de origen y de conversión.":
    "D'on arriben els clients i per què es perden les oportunitats. Les dues llistes alimenten els informes d'origen i de conversió.",
  "El catálogo de subpartidas: partida, unidad, marca, modelo y calidad, con su coste y su precio de referencia. Arrastra una partida para cambiar el orden en que sale en los presupuestos.":
    "El catàleg de subpartides: partida, unitat, marca, model i qualitat, amb el seu cost i el seu preu de referència. Arrossega una partida per canviar l'ordre en què surt als pressupostos.",
  "Es lo que queda guardado en cada línea ya escrita: renombrarlo rompería documentos ya emitidos. Lo que sí se edita es el nombre, y por eso hay dos —":
    "És el que queda desat a cada línia ja escrita: reanomenar-lo trencaria documents ja emesos. El que sí que s'edita és el nom, i per això n'hi ha dos —",
  ". El documento sale en el idioma del cliente, así que una unidad sin nombre en catalán saldría en castellano dentro de un presupuesto en catalán.":
    ". El document surt en l'idioma del client, així que una unitat sense nom en català sortiria en castellà dins d'un pressupost en català.",
  "Esta copia funciona en modo local, sin servidor, así que no hay cuentas que gestionar. Las cuentas viven donde viven los datos.":
    "Aquesta còpia funciona en mode local, sense servidor, així que no hi ha comptes per gestionar. Els comptes viuen on viuen les dades.",
  "Estas listas estaban escritas dentro del programa: añadir una fuente era una entrega nueva. Ahora son datos de la empresa y se mantienen desde aquí.":
    "Aquestes llistes estaven escrites dins del programa: afegir una font era un lliurament nou. Ara són dades de l'empresa i es mantenen des d'aquí.",
  "Las condiciones de pago que se ofrecen en contratos y facturas. El vencimiento real lo calcula cada documento; aquí se mantiene la lista.":
    "Les condicions de pagament que s'ofereixen en contractes i factures. El venciment real el calcula cada document; aquí es manté la llista.",
  "Las unidades que usan las subpartidas, los presupuestos y los documentos impresos. El símbolo es lo que aparece en la tabla; el nombre, en el desplegable y en el documento.":
    "Les unitats que fan servir les subpartides, els pressupostos i els documents impresos. El símbol és el que apareix a la taula; el nom, al desplegable i al document.",
  "Plantillas por familia, reglas evento → plantilla → destinatario → momento, y la cola resultante. Por defecto nada sale sin que una persona lo apruebe.":
    "Plantilles per família, regles esdeveniment → plantilla → destinatari → moment, i la cua resultant. Per defecte res no surt sense que una persona ho aprovi.",
  "Precios por proveedor con su fecha, su documento de origen y su vigencia. Filtra por subpartida para comparar proveedores; un precio ausente se ve ausente y nunca cuenta como cero.":
    "Preus per proveïdor amb la seva data, el seu document d'origen i la seva vigència. Filtra per subpartida per comparar proveïdors; un preu absent es veu absent i mai no compta com a zero.",
  "Retirar una unidad la quita de los desplegables. Las líneas que ya la llevan la conservan y se siguen viendo con su nombre — por eso la columna «en uso» informa, pero no bloquea.":
    "Retirar una unitat la treu dels desplegables. Les línies que ja la porten la conserven i se segueixen veient amb el seu nom — per això la columna «en ús» informa, però no bloqueja.",
  "Una cuenta se desactiva, nunca se borra: el registro de actividad tiene que seguir diciendo quién hizo qué. Al desactivar, sus sesiones abiertas terminan de inmediato.":
    "Un compte es desactiva, mai no s'esborra: el registre d'activitat ha de continuar dient qui va fer què. En desactivar, les seves sessions obertes acaben immediatament.",
  "puede pasar de «Transferencia 30 días» a «Transferencia a 30 días fecha factura» sin tocar ninguna factura antigua.":
    "pot passar de «Transferència 30 dies» a «Transferència a 30 dies data factura» sense tocar cap factura antiga.",
  "— al marcar una oportunidad como perdida. Sin un motivo escrito en el momento, el porqué se pierde y sólo queda el número.":
    "— en marcar una oportunitat com a perduda. Sense un motiu escrit en el moment, el perquè es perd i només queda el número.",
  "— Presupuestos, facturas, cobros y pagos. No gestiona usuarios.":
    "— Pressupostos, factures, cobraments i pagaments. No gestiona usuaris.",
  "— Registrar avance, partes y fotos. Sin importes ni márgenes.":
    "— Registrar avanç, parts i fotos. Sense imports ni marges.",
  "— Sólo lectura y exportación. Nunca ve márgenes ni precios de venta.":
    "— Només lectura i exportació. Mai no veu marges ni preus de venda.",
  "— Todo, incluida esta pantalla.": "— Tot, inclosa aquesta pantalla.",
  Avance: "Avanç",
  "Buscar por código o cliente": "Cercar per codi o client",
  Cerrado: "Tancat",
  Cerrados: "Tancats",
  "Conectado al ERP.": "Connectat a l'ERP.",
  Libros: "Llibres",
  "Todos los estados": "Tots els estats",
  "Estas filas se leen de": "Aquestes files es llegeixen de",
  "el ERP entero": "l'ERP sencer",
  ": se corrigen en": ": es corregeixen a",
  ", no aquí. Un dato que el ERP ya conoce no se teclea dos veces.":
    ", no aquí. Una dada que l'ERP ja coneix no es teclegen dues vegades.",
  "Las fotos, los documentos, los PDF de presupuesto y factura y cada correo se archivan por paso — guardados en el servidor de la empresa.":
    "Les fotos, els documents, els PDF de pressupost i factura i cada correu s'arxiven per pas — desats al servidor de l'empresa.",
  "Oficial 1ª — Albañilería": "Oficial 1a — Paleteria",
  "PRL Oficial 1ª": "PRL Oficial 1a",
  Adicional: "Addicional",
  "Antes de emitir": "Abans d'emetre",
  "Emitir factura": "Emetre factura",
  "Factura final": "Factura final",
  Líneas: "Línies",
  Manual: "Manual",
  "La factura no tiene importe.": "La factura no té import.",
  "Elige un origen arriba, o añade una línea.": "Tria un origen a dalt, o afegeix una línia.",
  "Volver al registro de facturas": "Tornar al registre de factures",
  "— elige el hito —": "— tria la fita —",
  "＋ Nueva factura": "＋ Nova factura",
  "Se abre el generador con lo que ya se puede facturar de esa obra: los hitos del contrato pendientes, el avance certificable y las líneas sueltas.":
    "Obre el generador amb el que ja es pot facturar d'aquella obra: les fites del contracte pendents, l'avanç certificable i les línies soltes.",
  Planificado: "Planificat",
  "Volver a la lista de obras": "Tornar a la llista d'obres",
  "estado y porcentaje, por partida y por subpartida":
    "estat i percentatge, per partida i per subpartida",
  "Recalcula las fechas previstas de los hitos de cobro del contrato a partir de la planificación actual":
    "Recalcula les dates previstes de les fites de cobrament del contracte a partir de la planificació actual",
  caducado: "caducat",
  vigente: "vigent",
  "Los precios no se editan: cada cotización nueva se añade como una fila más, porque lo que se presupuestó el mes pasado tiene que seguir explicándose con el precio que había entonces.":
    "Els preus no s'editen: cada cotització nova s'afegeix com una fila més, perquè el que es va pressupostar el mes passat ha de continuar explicant-se amb el preu que hi havia llavors.",
  "Transporte — · residuos — · pedido mínimo —": "Transport — · residus — · comanda mínima —",
  "Factura sin documento": "Factura sense document",
  "Facturas soportadas": "Factures suportades",
  "Movimientos bancarios": "Moviments bancaris",
  "Resúmenes fiscales": "Resums fiscals",
  "Abrir facturas de proveedores": "Obrir factures de proveïdors",
  Castellano: "Castellà",
  Catalán: "Català",
  "Idioma de la interfaz": "Idioma de la interfície",
  "Qué NO cambia": "Què NO canvia",
  "La elección se guarda en este navegador y se aplica al recargar. Es personal: un compañero puede tener el programa en otro idioma sin que cambie el suyo.":
    "L'elecció es desa en aquest navegador i s'aplica en recarregar. És personal: un company pot tenir el programa en un altre idioma sense que canviï el seu.",
  "se emiten en el idioma del cliente, que se elige en el propio documento. Cambiar el idioma de aquí no reescribe un documento ya emitido ni el que está por emitir.":
    "s'emeten en l'idioma del client, que es tria en el propi document. Canviar l'idioma d'aquí no reescriu un document ja emès ni el que està per emetre.",
  "— nombres de subpartidas, descripciones, notas — se guardan tal y como se escribieron. Esto traduce la interfaz, no el contenido.":
    "— noms de subpartides, descripcions, notes — es desen tal com es van escriure. Això tradueix la interfície, no el contingut.",
  Maquinaria: "Maquinària",
  Profesional: "Professional",
  "Un precio de referencia en blanco se muestra como «sin precio», nunca como cero — la subpartida entra en el presupuesto marcada como pendiente de valorar.":
    "Un preu de referència en blanc es mostra com «sense preu», mai com a zero — la subpartida entra al pressupost marcada com a pendent de valorar.",
  "La retención de IRPF depende del perfil del tercero (profesional, empresarial o módulos) y no está confirmada. Se conserva tal cual hasta que la gestoría la revise — ver":
    "La retenció d'IRPF depèn del perfil del tercer (professional, empresarial o mòduls) i no està confirmada. Es conserva tal qual fins que la gestoria la revisi — veure",
  Comprobación: "Comprovació",
  "Enviar al cliente →": "Enviar al client →",
  "sin incidencias": "sense incidències",
  Recibida: "Rebuda",
  Activa: "Activa",
  Cancelado: "Cancel·lat",
  "Total entradas": "Total entrades",
  "Total salidas": "Total sortides",
  "Reclamar documentación a subcontrata": "Reclamar documentació a subcontracta",
  "Reclamar factura vencida a los 3 días": "Reclamar factura vençuda als 3 dies",
  "Seguir presupuesto a los 5 días": "Seguir pressupost als 5 dies",
  "40% a la firma · 60% a la entrega": "40% a la signatura · 60% a l'entrega",
  "Tarifa horaria": "Tarifa horària",
  "orden de compra": "ordre de compra",
  "órdenes de compra": "ordres de compra",
  factura: "factura",
  facturas: "factures",
  presupuesto: "pressupost",
  contrato: "contracte",
  tarea: "tasca",
  visita: "visita",
  oportunidad: "oportunitat",
  cliente: "client",
  proveedor: "proveïdor",
  trabajador: "treballador",
  subcontrata: "subcontracta",
  hito: "fita",
  adicional: "addicional",
  parte: "part",
  recibo: "rebut",
  albarán: "albarà",
  "Fichero de personal interno. La tarifa vigente y la documentación (PRL, alta S.S.) se gestionan desde la ficha de cada persona.":
    "Fitxer de personal intern. La tarifa vigent i la documentació (PRL, alta S.S.) es gestionen des de la fitxa de cada persona.",
  "Fichero de subcontratas (industriales). El ciclo de documentos y certificaciones sigue en el motor; esta pantalla es su ficha.":
    "Fitxer de subcontractes (industrials). El cicle de documents i certificacions continua al motor; aquesta pantalla és la seva fitxa.",
  "Fichero de proveedores. La categoría clasifica el proveedor; los precios y compras se gestionan en sus propias pantallas.":
    "Fitxer de proveïdors. La categoria classifica el proveïdor; els preus i les compres es gestionen a les seves pròpies pantalles.",
  "Elige una obra para abrir su carta Gantt: ahí se marca el avance, se mueven las barras y se recalculan los cobros previstos.":
    "Tria una obra per obrir la seva carta Gantt: allà es marca l'avanç, es mouen les barres i es recalculen els cobraments previstos.",
  "Se abre el generador con lo que ya se puede facturar de esa obra: los hitos del contrato pendientes, el avance certificable y los adicionales aprobados.":
    "Obre el generador amb el que ja es pot facturar d'aquella obra: les fites del contracte pendents, l'avanç certificable i els addicionals aprovats.",
  Alta: "Alta",
  Datos: "Dades",
  Ciudad: "Ciutat",
  Contacto: "Contacte",
  "Crear presupuesto": "Crear pressupost",
  "De un lead abierto": "D'un lead obert",
  "De una visita realizada": "D'una visita realitzada",
  "Ir a Presupuestos": "Anar a Pressupostos",
  "Hasta el": "Fins al",
  "Las mediciones y notas de la visita quedan enlazadas al presupuesto.":
    "Els mesuraments i les notes de la visita queden enllaçats al pressupost.",
  "Arrastra aquí el contrato firmado (PDF o imagen), o búscalo en el dispositivo.":
    "Arrossega aquí el contracte signat (PDF o imatge), o cerca'l al dispositiu.",
  "Esta obra no tiene un presupuesto aceptado del que derivar la planificación":
    "Aquesta obra no té un pressupost acceptat del qual derivar la planificació",
  "Esta obra todavía no tiene un presupuesto aceptado, así que no hay partidas ni subpartidas que planificar ni sobre los que registrar avance.":
    "Aquesta obra encara no té un pressupost acceptat, així que no hi ha partides ni subpartides per planificar ni sobre els quals registrar avanç.",
  "Extracto a la izquierda, documentos candidatos a la derecha y la propuesta de emparejamiento en medio, con el porqué. La cola de movimientos sin conciliar es el indicador de salud del periodo.":
    "Extracte a l'esquerra, documents candidats a la dreta i la proposta d'aparellament al mig, amb el perquè. La cua de moviments sense conciliar és l'indicador de salut del període.",
  "El idioma de la interfaz, para quien está usando el programa. No cambia el idioma de los presupuestos ni de los contratos: eso se elige por cliente, en cada documento.":
    "L'idioma de la interfície, per a qui està fent servir el programa. No canvia l'idioma dels pressupostos ni dels contractes: això es tria per client, a cada document.",
  "Visita realizada": "Visita realitzada",
  "¿De dónde sale este presupuesto?": "D'on surt aquest pressupost?",
  "No hace falta esperar a la visita para preparar el presupuesto.":
    "No cal esperar la visita per preparar el pressupost.",
  "Parte diario por trabajador, con la semana al lado; y el resumen por proyecto y partida con la conciliación de horas.":
    "Part diari per treballador, amb la setmana al costat; i el resum per projecte i partida amb la conciliació d'hores.",
  TQ1: "TQ1",
  TQ2: "TQ2",
  TQ3: "TQ3",
  TQ4: "TQ4",
  unidad: "unitat",
  "partida alzada": "partida alçada",
  metro: "metre",
  "metro lineal": "metre lineal",
  "metro cuadrado": "metre quadrat",
  "metro cúbico": "metre cúbic",
  hora: "hora",
  kilogramo: "quilogram",
  litro: "litre",
  porcentaje: "percentatge",
  Buscador: "Cercador",
  "Redes sociales": "Xarxes socials",
  "Cliente recurrente": "Client recurrent",
  Plazos: "Terminis",
  Alcance: "Abast",
  Competencia: "Competència",
  "Sin respuesta": "Sense resposta",
  Desistió: "Va desistir",
  Demoliciones: "Enderrocs",
  Albañilería: "Paleteria",
  Electricidad: "Electricitat",
  Climatización: "Climatització",
  Revestimientos: "Revestiments",
  Carpintería: "Fusteria",
  Pintura: "Pintura",
  "Sanitarios y grifería": "Sanitaris i aixetes",
  Varios: "Diversos",
  "Transferencia 30 días": "Transferència 30 dies",
  "Transferencia 60 días": "Transferència 60 dies",
  "Transferencia 90 días": "Transferència 90 dies",
  Domiciliación: "Domiciliació",
  Tarjeta: "Targeta",
  "A cuenta": "A compte",
  "Pago único": "Pagament únic",
  "40% a la firma · 40% a mitad de obra · 20% a la finalización":
    "40% a la signatura · 40% a mitja obra · 20% a la finalització",
  "50% a la firma · 50% a la entrega": "50% a la signatura · 50% a l'entrega",
  "30% a la firma · 70% a la entrega": "30% a la signatura · 70% a l'entrega",
  "100% a la entrega": "100% a l'entrega",
  "Enviar presupuesto": "Enviar pressupost",
  "Volver a llamar": "Tornar a trucar",
  "Esperar respuesta del cliente": "Esperar resposta del client",
  "Hacer seguimiento": "Fer seguiment",
  "Compras de material": "Compres de material",
  "Mano de obra de obra": "Mà d'obra d'obra",
  "Otros costes de obra": "Altres costos d'obra",
  Alquileres: "Lloguers",
  "Combustible y suministros": "Combustible i subministraments",
  "Servicios profesionales": "Serveis professionals",
  "Otros tributos": "Altres tributs",
  "Medios auxiliares y seguridad": "Mitjans auxiliars i seguretat",
  "Tabiquería y trasdosados": "Envans i trasdossats",
  "Aislamientos e impermeabilización": "Aïllaments i impermeabilització",
  Cubiertas: "Cobertes",
  Fachadas: "Façanes",
  Solados: "Paviments",
  "Ventanas y vidriería": "Finestres i vidrieria",
  "Cocina y mobiliario": "Cuina i mobiliari",
  "Limpieza y remates": "Neteja i acabats",

  /* ==========================================================================
     THE BACKLOG, CLEARED — 1.036 entries, 2026-08-17.
     ==========================================================================
     Until now a Catalan reader saw SPANISH wherever an entry was missing: the
     layer falls back to the hub language, so the gap was invisible to everyone
     except the person reading it. These are the strings that had no Catalan
     form, translated in one pass, which is why they sit together rather than
     scattered through the file above.

     THEY ARE MACHINE-AUTHORED. That is stated plainly so the review has a
     scope instead of a rumour: a native speaker checking this file needs to
     read THIS block, not the two thousand entries that precede it. The
     vocabulary used is the building trade's own — enderroc, envà, arrebossat,
     lampisteria, aixetes, paviment, trencadís of register avoided — and the
     formal plural the rest of the product uses when it addresses a customer.

     Where a string is genuinely identical in both languages it is written out
     in full rather than omitted, per the note at the top of this file: a line
     that reads the same in both is a decision somebody took, not a line
     somebody forgot.
     ========================================================================== */
  "Margen medio de presupuesto": "Marge mitjà de pressupost",
  "Cobros vencidos": "Cobraments vençuts",
  "Previsión de caja 30d": "Previsió de caixa 30d",
  "en ejecución / facturación": "en execució / facturació",
  "por debajo del objetivo del 15%": "per sota de l'objectiu del 15%",
  saludable: "saludable",
  "facturas de proveedor por pagar": "factures de proveïdor per pagar",
  "incl. nóminas y gastos generales": "incl. nòmines i despeses generals",
  "Visita a obra": "Visita a obra",
  Estimación: "Estimació",
  Compras: "Compres",
  Cobros: "Cobraments",
  "Pagos a proveedores": "Pagaments a proveïdors",
  "Cierre · beneficio": "Tancament · benefici",
  "Solicitud de reseña": "Sol·licitud de ressenya",
  Cierre: "Tancament",
  Reseña: "Ressenya",
  "No hay proyectos en «Contacto».": "No hi ha projectes a «Contacte».",
  "No hay proyectos en «Visita a obra».": "No hi ha projectes a «Visita a obra».",
  "No hay proyectos en «Estimación».": "No hi ha projectes a «Estimació».",
  "No hay proyectos en «Presupuesto».": "No hi ha projectes a «Pressupost».",
  "No hay proyectos en «Aceptación».": "No hi ha projectes a «Acceptació».",
  "No hay proyectos en «Proyecto».": "No hi ha projectes a «Projecte».",
  "No hay proyectos en «Compras».": "No hi ha projectes a «Compres».",
  "No hay proyectos en «Ejecución».": "No hi ha projectes a «Execució».",
  "No hay proyectos en «Facturación».": "No hi ha projectes a «Facturació».",
  "No hay proyectos en «Cobros».": "No hi ha projectes a «Cobraments».",
  "No hay proyectos en «Pagos a proveedores».": "No hi ha projectes a «Pagaments a proveïdors».",
  "No hay proyectos en «Cierre · beneficio».": "No hi ha projectes a «Tancament · benefici».",
  "No hay proyectos en «Solicitud de reseña».": "No hi ha projectes a «Sol·licitud de ressenya».",
  "Abrir →": "Obrir →",
  "⚠ en riesgo": "⚠ en risc",
  "● vigilar": "● vigilar",
  "✓ según lo previsto": "✓ segons el previst",
  "● vigilar margen": "● vigilar marge",
  Fase: "Fase",
  "Coste previsto": "Cost previst",
  Caja: "Caixa",
  "Ingresos (facturado)": "Ingressos (facturat)",
  "aún sin facturar": "encara sense facturar",
  "Beneficio previsto": "Benefici previst",
  "presupuesto − comprometido/real": "pressupost − compromès/real",
  "Comprometido vs coste previsto": "Compromès vs cost previst",
  "compromisos de compra": "compromisos de compra",
  "No vencido": "No vençut",
  "60d+ / vencido": "60d+ / vençut",
  "Contratista A": "Contractista A",
  "Contratista B": "Contractista B",
  "Alicatado de pared 20×60": "Enrajolat de paret 20×60",
  "Suelo porcelánico": "Paviment porcellànic",
  "Kit de fontanería": "Kit de lampisteria",
  Demolición: "Enderroc",
  S29: "S29",
  S30: "S30",
  S31: "S31",
  S32: "S32",
  S33: "S33",
  S34: "S34",
  Abr: "Abr",
  "Presupuesto — Aceptación": "Pressupost — Acceptació",
  "Aceptación del cliente": "Acceptació del client",
  "Recordatorio de pago": "Recordatori de pagament",
  "✓ enviado": "✓ enviat",
  "● borrador": "● esborrany",
  Para: "Per a",
  "✉ Enviar": "✉ Enviar",
  "⤓ Borrador de Outlook (.eml)": "⤓ Esborrany de l'Outlook (.eml)",
  "↻ Restablecer borrador": "↻ Restablir l'esborrany",
  "Redactado automáticamente · edite libremente · envíe o guarde en Outlook":
    "Redactat automàticament · editeu-lo lliurement · envieu-lo o deseu-lo a l'Outlook",
  "¿Qué tal lo hemos hecho? — un pequeño favor": "Què tal ho hem fet? — un petit favor",
  "Factura vencida — Baño completo — C/ Balmes 24": "Factura vençuda — Bany complet — C/ Balmes 24",
  "3.687,20 € · 4 día(s) de retraso": "3.687,20 € · 4 dia(es) de retard",
  "Presupuesto a la espera de respuesta — Interiorismo — Pg. de Gràcia 88":
    "Pressupost a l'espera de resposta — Interiorisme — Pg. de Gràcia 88",
  "enviado hace 3d, sin aceptación": "enviat fa 3d, sense acceptació",
  "Todo en orden": "Tot en ordre",
  "Sin facturas vencidas ni riesgos de margen.": "Sense factures vençudes ni riscos de marge.",
  "Enviar presupuesto a Estudi Marés (P-039)": "Enviar pressupost a Estudi Marés (P-039)",
  "borrador listo en Correos": "esborrany a punt al Correu",
  "Reclamar factura vencida — María García (P-024)":
    "Reclamar factura vençuda — María García (P-024)",
  "recordatorio redactado": "recordatori redactat",
  "Visita a obra — conversión de buhardilla, Cornellà (P-046)":
    "Visita a obra — conversió de golfes, Cornellà (P-046)",
  "nuevo contacto, agendar visita": "nou contacte, programar visita",
  "Emitir pedido de materiales de cocina (P-036)": "Emetre comanda de materials de cuina (P-036)",
  "proveedor seleccionado en la comparativa": "proveïdor seleccionat a la comparativa",
  "Confirmar aceptación y abrir proyecto (P-036)": "Confirmar acceptació i obrir projecte (P-036)",
  "el cliente aceptó verbalmente": "el client ho va acceptar verbalment",
  "Factura por hito — Grup Ferrer (P-031)": "Factura per fita — Grup Ferrer (P-031)",
  "fase 2 de 3 completada": "fase 2 de 3 completada",
  Operaciones: "Operacions",
  "Baño completo — C/ Balmes 24": "Bany complet — C/ Balmes 24",
  "Cocina + salón — Av. Diagonal 402": "Cuina + saló — Av. Diagonal 402",
  "Reforma de cocina — Sant Just, Creu 12": "Reforma de cuina — Sant Just, Creu 12",
  "Interiorismo — Pg. de Gràcia 88": "Interiorisme — Pg. de Gràcia 88",
  "Trabajos verticales — fachada C/ Major": "Treballs verticals — façana C/ Major",
  "Baño + aerotermia — Esplugues": "Bany + aerotèrmia — Esplugues",
  "Contacto — conversión de buhardilla, Cornellà": "Contacte — conversió de golfes, Cornellà",
  "La identidad legal, fiscal y bancaria de la propia empresa — la raíz desde la que se emite cada documento.":
    "La identitat legal, fiscal i bancària de la mateixa empresa — l'arrel des de la qual s'emet cada document.",
  "Oficinas, patios y almacenes físicos de la empresa.":
    "Oficines, patis i magatzems físics de l'empresa.",
  "Cuentas bancarias de la empresa utilizadas para cobros y pagos.":
    "Comptes bancaris de l'empresa utilitzats per a cobraments i pagaments.",
  "Series documentales que la automatización incrementa — presupuestos, facturas, pedidos de compra, rectificativas.":
    "Sèries documentals que l'automatització incrementa — pressupostos, factures, comandes de compra, rectificatives.",
  "Personas con nombre en clientes o colaboradores, con rol y canal.":
    "Persones amb nom a clients o col·laboradors, amb rol i canal.",
  "Oportunidades del pipeline de ventas con etapa, valor y próxima acción para la automatización del seguimiento.":
    "Oportunitats del pipeline de vendes amb etapa, valor i acció següent per a l'automatització del seguiment.",
  "Todos a quienes compra — proveedores de materiales y subcontratistas autónomos — con condiciones y perfil de retención de IRPF.":
    "Tothom a qui compra — proveïdors de materials i subcontractistes autònoms — amb condicions i perfil de retenció d'IRPF.",
  "Definiciones reutilizables de condiciones de pago referenciadas por clientes, proveedores y documentos.":
    "Definicions reutilitzables de condicions de pagament referenciades per clients, proveïdors i documents.",
  "El libro de precios. Cada material, tarifa de mano de obra, equipo y línea de subcontrata de los que se nutren presupuestos y compras.":
    "El llibre de preus. Cada material, tarifa de mà d'obra, equip i línia de subcontracta dels quals s'alimenten pressupostos i compres.",
  "La estructura de costes de una reforma — las partidas en los que se consolidan presupuestos, ofertas y el margen por partida.":
    "L'estructura de costos d'una reforma — les partides en què es consoliden pressupostos, ofertes i el marge per partida.",
  "Conjuntos de precios con nombre y fecha de vigencia asignados a grupos de clientes; el sistema elige el adecuado para cada presupuesto.":
    "Conjunts de preus amb nom i data de vigència assignats a grups de clients; el sistema tria l'adequat per a cada pressupost.",
  "Códigos de IVA y retención con fecha de vigencia; la regla de jurisdicción decide cuál se aplica en cada línea.":
    "Codis d'IVA i retenció amb data de vigència; la regla de jurisdicció decideix quin s'aplica a cada línia.",
  "Unidades canónicas utilizadas en artículos, stock y documentos.":
    "Unitats canòniques utilitzades en articles, estoc i documents.",
  "Ubicaciones de almacenaje para la valoración y disponibilidad de materiales.":
    "Ubicacions d'emmagatzematge per a la valoració i disponibilitat de materials.",
  "Cantidades disponibles y valoración por artículo y ubicación — alimenta las compras y la salida de material a obra.":
    "Quantitats disponibles i valoració per article i ubicació — alimenta les compres i la sortida de material a obra.",
  "Personas con sus tarifas de coste/venta, habilidades y certificación PRL para la planificación y el coste de obra.":
    "Persones amb les seves tarifes de cost/venda, habilitats i certificació PRL per a la planificació i el cost d'obra.",
  "Activos en propiedad — maquinaria, herramientas y furgonetas — con valor y mantenimiento para amortización y disponibilidad.":
    "Actius en propietat — maquinària, eines i furgonetes — amb valor i manteniment per a amortització i disponibilitat.",
  "El registro de obras — vincula cliente, obra y estado; el recorrido y las finanzas se asocian aquí.":
    "El registre d'obres — vincula client, obra i estat; el recorregut i les finances s'hi associen.",
  "Registro de certificados, licencias y seguros con vencimiento — la automatización avisa antes de que caduquen.":
    "Registre de certificats, llicències i assegurances amb venciment — l'automatització avisa abans que caduquin.",
  "Los disparadores y cadencias que el ERP ejecuta de forma desatendida — recordatorios, solicitudes de reseña, alertas de reposición, escalados.":
    "Els disparadors i cadències que l'ERP executa de manera desatesa — recordatoris, sol·licituds de ressenya, alertes de reposició, escalats.",
  "Razón social": "Raó social",
  "Nombre comercial": "Nom comercial",
  "NIF-IVA (UE)": "NIF-IVA (UE)",
  "Código de actividad CNAE": "Codi d'activitat CNAE",
  "Registro Mercantil": "Registre Mercantil",
  "Capital social": "Capital social",
  "Domicilio social": "Domicili social",
  "Código postal": "Codi postal",
  Provincia: "Província",
  País: "País",
  "Sitio web": "Lloc web",
  "IBAN por defecto": "IBAN per defecte",
  "Régimen fiscal": "Règim fiscal",
  "% IRPF por defecto": "% IRPF per defecte",
  "Cierre del ejercicio fiscal": "Tancament de l'exercici fiscal",
  "Moneda base": "Moneda base",
  "Aseguradora de responsabilidad civil": "Asseguradora de responsabilitat civil",
  "Número de póliza": "Número de pòlissa",
  "Vencimiento de la póliza": "Venciment de la pòlissa",
  Banco: "Banc",
  Moneda: "Moneda",
  Finalidad: "Finalitat",
  "Tipo de documento": "Tipus de document",
  Prefijo: "Prefix",
  "Siguiente número": "Número següent",
  "Dígitos (relleno)": "Dígits (farciment)",
  Reinicio: "Reinici",
  "Contacto principal": "Contacte principal",
  "Dirección de facturación": "Adreça de facturació",
  "Dirección de obra por defecto": "Adreça d'obra per defecte",
  "Método de pago": "Mètode de pagament",
  "IBAN (para SEPA)": "IBAN (per a SEPA)",
  "Ref. mandato SEPA": "Ref. mandat SEPA",
  "Descuento habitual %": "Descompte habitual %",
  "Límite de crédito": "Límit de crèdit",
  "Consentimiento RGPD": "Consentiment RGPD",
  Etiquetas: "Etiquetes",
  "Organización / código de cliente": "Organització / codi de client",
  Principal: "Principal",
  "Cliente / prospecto": "Client / prospecte",
  Etapa: "Etapa",
  Propietario: "Propietari",
  "Próxima fecha": "Data següent",
  "Oficios / mercancías": "Oficis / mercaderies",
  "Perfil de IRPF": "Perfil d'IRPF",
  "Retención %": "Retenció %",
  "Plazo de entrega (días)": "Termini de lliurament (dies)",
  Calificación: "Qualificació",
  "Vencimiento seguro / RC": "Venciment assegurança / RC",
  "Días netos": "Dies nets",
  "Dto. pronto pago %": "Dte. pagament immediat %",
  "Dto. dentro de (días)": "Dte. dins de (dies)",
  "Precio de venta / tarifa": "Preu de venda / tarifa",
  "Margen %": "Marge %",
  "Clase de IVA": "Classe d'IVA",
  "Proveedor preferente": "Proveïdor preferent",
  "Código de barras / EAN": "Codi de barres / EAN",
  "Punto de pedido": "Punt de comanda",
  "Margen % por defecto": "Marge % per defecte",
  Orden: "Ordre",
  "Válido desde": "Vàlid des de",
  "Válido hasta": "Vàlid fins a",
  "Recargo %": "Recàrrec %",
  "Grupo de clientes": "Grup de clients",
  "Tipo %": "Tipus %",
  "Inversión del sujeto pasivo (ISP)": "Inversió del subjecte passiu (ISP)",
  Almacén: "Magatzem",
  "SKU del artículo": "SKU de l'article",
  Existencias: "Existències",
  Reservado: "Reservat",
  "Máx.": "Màx.",
  "Estante / ubicación": "Prestatge / ubicació",
  "Coste unitario (valoración)": "Cost unitari (valoració)",
  "Último recuento": "Últim recompte",
  "Tarifa de coste €/h": "Tarifa de cost €/h",
  "Tarifa de venta €/h": "Tarifa de venda €/h",
  Habilidades: "Habilitats",
  "Certificado PRL": "Certificat PRL",
  "Vencimiento PRL": "Venciment PRL",
  "Color de calendario": "Color de calendari",
  Encargado: "Encarregat",
  "Matrícula / nº de serie": "Matrícula / núm. de sèrie",
  "Fecha de compra": "Data de compra",
  "Próximo mantenimiento / ITV": "Manteniment següent / ITV",
  "Dirección de la obra": "Adreça de l'obra",
  "Jefe de obra": "Cap d'obra",
  "Importe del contrato": "Import del contracte",
  "Se aplica a": "S'aplica a",
  Clave: "Clau",
  Acción: "Acció",
  "Plantilla / mensaje": "Plantilla / missatge",
  Cadencia: "Cadència",
  Activada: "Activada",
  Identidad: "Identitat",
  "Banca y fiscalidad": "Banca i fiscalitat",
  Cumplimiento: "Compliment",
  Direcciones: "Adreces",
  Gobernanza: "Governança",
  Capacidades: "Capacitats",
  Artículo: "Article",
  Precios: "Preus",
  Aprovisionamiento: "Aprovisionament",
  "Razón social *": "Raó social *",
  "Banco *": "Banc *",
  "Tipo de documento *": "Tipus de document *",
  "Razón social / nombre completo *": "Raó social / nom complet *",
  "Cliente / prospecto *": "Client / prospecte *",
  "Tipo % *": "Tipus % *",
  "Almacén *": "Magatzem *",
  "SKU del artículo *": "SKU de l'article *",
  "Documento *": "Document *",
  "Disparador *": "Disparador *",
  "Recargo de equivalencia": "Recàrrec d'equivalència",
  Módulos: "Mòduls",
  "Grupo IVA": "Grup d'IVA",
  Patio: "Pati",
  "Sala de exposición": "Sala d'exposició",
  Operativa: "Operativa",
  Ahorro: "Estalvi",
  Nunca: "Mai",
  Anual: "Anual",
  Mensual: "Mensual",
  "Sector público": "Sector públic",
  Inmediato: "Immediat",
  "15 días": "15 dies",
  "30 días": "30 dies",
  "60 días": "60 dies",
  "50% por adelantado": "50% per avançat",
  "Domiciliación SEPA": "Domiciliació SEPA",
  Financiación: "Finançament",
  Recomendación: "Recomanació",
  Arquitecto: "Arquitecte",
  Inactivo: "Inactiu",
  Bloqueado: "Bloquejat",
  Nuevo: "Nou",
  Cualificado: "Qualificat",
  Ganado: "Guanyat",
  Perdido: "Perdut",
  "Proveedor de materiales": "Proveïdor de materials",
  "Subcontratista (autónomo)": "Subcontractista (autònom)",
  "Subcontratista (empresa)": "Subcontractista (empresa)",
  Servicio: "Servei",
  "Alquiler de equipos": "Lloguer d'equips",
  "Sin retención (empresarial)": "Sense retenció (empresarial)",
  "Profesional 15%": "Professional 15%",
  "Nuevo profesional 7%": "Nou professional 7%",
  "A — preferente": "A — preferent",
  "B — aprobado": "B — aprovat",
  "C — ocasional": "C — ocasional",
  "En espera": "En espera",
  Equipo: "Equip",
  día: "dia",
  "tanto alzado": "a preu fet",
  "Reducido 10%": "Reduït 10%",
  "Superreducido 4%": "Superreduït 4%",
  Exento: "Exempt",
  "IVA general": "IVA general",
  "IVA reducido": "IVA reduït",
  "IVA superreducido": "IVA superreduït",
  "IRPF retención": "IRPF retenció",
  "Recargo equivalencia": "Recàrrec d'equivalència",
  Descatalogado: "Descatalogat",
  Recuento: "Recompte",
  Longitud: "Longitud",
  Superficie: "Superfície",
  Volumen: "Volum",
  Peso: "Pes",
  Tiempo: "Temps",
  "Tanto alzado": "A preu fet",
  "Almacén principal": "Magatzem principal",
  "Stock de furgoneta": "Estoc de furgoneta",
  "Almacén de obra": "Magatzem d'obra",
  "Encargado de obra": "Encarregat d'obra",
  "Oficial 1ª": "Oficial 1a",
  Peón: "Peó",
  Ventas: "Vendes",
  Válido: "Vàlid",
  "Por vencer": "A punt de vèncer",
  "N/D": "N/D",
  "De baja": "De baixa",
  Vehículo: "Vehicle",
  Máquina: "Màquina",
  Herramienta: "Eina",
  "En servicio": "En servei",
  "En reparación": "En reparació",
  Retirado: "Retirat",
  Cocina: "Cuina",
  Baño: "Bany",
  Fachada: "Façana",
  Ampliación: "Ampliació",
  Seguro: "Assegurança",
  Licencia: "Llicència",
  Garantía: "Garantia",
  "Enviar correo": "Enviar correu",
  "Notificar al responsable": "Notificar al responsable",
  "Generar pedido de compra": "Generar comanda de compra",
  Marcar: "Marcar",
  "p. ej. 31/12": "p. ex. 31/12",
  "p. ej. FRA-2026-": "p. ex. FRA-2026-",
  "código de Listas de precios": "codi de Llistes de preus",
  "código de Partidas": "codi de Partides",
  "código de proveedor": "codi de proveïdor",
  "código de cliente": "codi de client",
  "código de entidad": "codi d'entitat",
  "hex, p. ej. #48733c": "hex, p. ex. #48733c",
  "p. ej. invoice.overdue": "p. ex. invoice.overdue",
  "p. ej. +7d, luego +14d": "p. ex. +7d, després +14d",
  "separadas, por comas": "separades per comes",
  "Aún no hay registros.": "Encara no hi ha registres.",
  "Nada coincide con su búsqueda.": "No hi ha res que coincideixi amb la vostra cerca.",
  "Haga clic en": "Feu clic a",
  "para crear el primero.": "per crear-ne el primer.",
  "Guardado ✓": "Desat ✓",
  Eliminado: "Eliminat",
  "Exportado canei-master-data.json": "Exportat canei-master-data.json",
  "Importado ✓": "Importat ✓",
  "Archivo JSON no válido": "Fitxer JSON no vàlid",
  "¿Eliminar este registro? Esta acción no se puede deshacer.":
    "Voleu eliminar aquest registre? Aquesta acció no es pot desfer.",
  "La importación sustituye los registros actuales por el contenido del archivo. Exporte primero una copia de seguridad si tiene dudas. ¿Continuar?":
    "La importació substitueix els registres actuals pel contingut del fitxer. Exporteu primer una còpia de seguretat si teniu dubtes. Voleu continuar?",
  "¿Cargar el conjunto de datos inicial? Esto sustituye todo lo introducido actualmente.":
    "Voleu carregar el conjunt de dades inicial? Això substitueix tot el que s'ha introduït fins ara.",
  "Editar empresa / entidad legal": "Editar empresa / entitat legal",
  "Nueva empresa / entidad legal": "Nova empresa / entitat legal",
  "Editar sucursal / ubicación": "Editar sucursal / ubicació",
  "Nueva sucursal / ubicación": "Nova sucursal / ubicació",
  "Editar cuenta bancaria": "Editar compte bancari",
  "Nueva cuenta bancaria": "Nou compte bancari",
  "Editar serie de numeración": "Editar sèrie de numeració",
  "Nueva serie de numeración": "Nova sèrie de numeració",
  "Editar cliente": "Editar client",
  "Editar contacto": "Editar contacte",
  "Nuevo contacto": "Nou contacte",
  "Editar oportunidad / pipeline": "Editar oportunitat / pipeline",
  "Nueva oportunidad / pipeline": "Nova oportunitat / pipeline",
  "Editar proveedor / subcontratista": "Editar proveïdor / subcontractista",
  "Nuevo proveedor / subcontratista": "Nou proveïdor / subcontractista",
  "Editar condición de pago": "Editar condició de pagament",
  "Nueva condición de pago": "Nova condició de pagament",
  "Editar artículo — materiales y trabajos": "Editar article — materials i treballs",
  "Nuevo artículo — materiales y trabajos": "Nou article — materials i treballs",
  "Editar partida del catálogo": "Editar partida del catàleg",
  "Nueva partida del catálogo": "Nova partida del catàleg",
  "Editar lista de precios / tarifa": "Editar llista de preus / tarifa",
  "Nueva lista de precios / tarifa": "Nova llista de preus / tarifa",
  "Editar código de IVA / impuesto": "Editar codi d'IVA / impost",
  "Nuevo código de IVA / impuesto": "Nou codi d'IVA / impost",
  "Editar unidad de medida": "Editar unitat de mesura",
  "Nueva unidad de medida": "Nova unitat de mesura",
  "Editar almacén / depósito": "Editar magatzem / dipòsit",
  "Nuevo almacén / depósito": "Nou magatzem / dipòsit",
  "Editar stock / inventario": "Editar estoc / inventari",
  "Nuevo stock / inventario": "Nou estoc / inventari",
  "Editar recurso del equipo": "Editar recurs de l'equip",
  "Nuevo recurso del equipo": "Nou recurs de l'equip",
  "Editar equipo / vehículo": "Editar equip / vehicle",
  "Nuevo equipo / vehículo": "Nou equip / vehicle",
  "Editar proyecto / obra": "Editar projecte / obra",
  "Nuevo proyecto / obra": "Nou projecte / obra",
  "Editar documento de cumplimiento": "Editar document de compliment",
  "Nuevo documento de cumplimiento": "Nou document de compliment",
  "Editar regla de automatización": "Editar regla d'automatització",
  "Nueva regla de automatización": "Nova regla d'automatització",
  "Fraccionado 30/60": "Fraccionat 30/60",
  "50% a 30, 50% a 60": "50% a 30, 50% a 60",
  "Estándar 2026": "Estàndard 2026",
  "Minorista (B2C)": "Detall (B2C)",
  "Reforma vivienda (art.91)": "Reforma d'habitatge (art.91)",
  "Profesional general": "Professional general",
  Metro: "Metre",
  "Metro cuadrado": "Metre quadrat",
  "Metro cúbico": "Metre cúbic",
  Kilogramo: "Quilogram",
  "Almacén central": "Magatzem central",
  "Furgoneta 1": "Furgoneta 1",
  "Generales / mano de obra": "Generals / mà d'obra",
  "Solados y alicatados": "Paviments i enrajolats",
  "Demolición de tabique de ladrillo": "Enderroc d'envà de maó",
  "Punto de agua empotrado (alta/baja)": "Punt d'aigua encastat (alta/baixa)",
  "Punto de luz / enchufe empotrado": "Punt de llum / endoll encastat",
  "Alicatado gres porcelánico (material)": "Enrajolat de gres porcellànic (material)",
  "Mano de obra oficial 1ª": "Mà d'obra oficial 1a",
  cemento: "ciment",
  yeso: "guix",
  fontanería: "lampisteria",
  electricidad: "electricitat",
  gestión: "gestió",
  ventas: "vendes",
  albañilería: "paleteria",
  alicatado: "enrajolat",
  ayuda: "ajuda",
  demolición: "enderroc",
  "reforma integral": "reforma integral",
  comunidad: "comunitat",
  fachada: "façana",
  "local comercial": "local comercial",
  "Cocina + 2 baños.": "Cuina + 2 banys.",
  Administrador: "Administrador",
  "Hacer seguimiento del presupuesto": "Fer seguiment del pressupost",
  "Concertar visita": "Concertar visita",
  "Furgoneta Renault Kangoo": "Furgoneta Renault Kangoo",
  "Martillo demoledor": "Martell demolidor",
  "Seguro Responsabilidad Civil": "Assegurança de responsabilitat civil",
  "Recordatorio — amistoso": "Recordatori — amistós",
  "Pedir reseña de 5★": "Demanar ressenya de 5★",
  "Renovar antes del vencimiento": "Renovar abans del venciment",
  inmediato: "immediat",
  "Depreciación y amortización": "Depreciació i amortització",
  "Intereses netos": "Interessos nets",
  "Impuesto de sociedades": "Impost de societats",
  "Activo corriente": "Actiu corrent",
  "Activo no corriente": "Actiu no corrent",
  "Pasivo corriente": "Passiu corrent",
  "Pasivo no corriente": "Passiu no corrent",
  "Patrimonio neto": "Patrimoni net",
  "Libro de PyG (mensual)": "Llibre de PiG (mensual)",
  "Balance (instantáneas)": "Balanç (instantànies)",
  "Cuentas a cobrar (abiertas)": "Comptes a cobrar (oberts)",
  "Cuentas a pagar (abiertas)": "Comptes a pagar (oberts)",
  "Libro de IVA e impuestos": "Llibre d'IVA i impostos",
  "Préstamos y financiación": "Préstecs i finançament",
  "Cuentas bancarias y de caja": "Comptes bancaris i de caixa",
  "Variables e hipótesis": "Variables i hipòtesis",
  "Plan de cuentas.": "Pla de comptes.",
  "Libro de PyG (mensual).": "Llibre de PiG (mensual).",
  "Balance (instantáneas).": "Balanç (instantànies).",
  "Cuentas a cobrar (abiertas).": "Comptes a cobrar (oberts).",
  "Cuentas a pagar (abiertas).": "Comptes a pagar (oberts).",
  "Libro de IVA e impuestos.": "Llibre d'IVA i impostos.",
  "Préstamos y financiación.": "Préstecs i finançament.",
  "Cuentas bancarias y de caja.": "Comptes bancaris i de caixa.",
  "Variables e hipótesis.": "Variables i hipòtesis.",
  "Nombre de la cuenta": "Nom del compte",
  "Estado financiero": "Estat financer",
  "Línea de agregación": "Línia d'agregació",
  "Presupuesto anual (PyG)": "Pressupost anual (PiG)",
  Cuenta: "Compte",
  "A fecha de": "A data de",
  "Factura recibida": "Factura rebuda",
  "Repercutido / retenido": "Repercutit / retingut",
  "Soportado / deducible": "Suportat / deduïble",
  "Línea de financiación": "Línia de finançament",
  "Principal original": "Principal original",
  "Cuota mensual": "Quota mensual",
  "Fecha de vencimiento": "Data de venciment",
  "Plantilla (ETC)": "Plantilla (ETC)",
  "Cartera de pedidos (ganada, no facturada)": "Cartera de comandes (guanyada, no facturada)",
  "Valor del pipeline abierto": "Valor del pipeline obert",
  "% de margen bruto objetivo": "% de marge brut objectiu",
  "Nombre de la cuenta *": "Nom del compte *",
  "Estado financiero *": "Estat financer *",
  "Línea de agregación *": "Línia d'agregació *",
  "Mes *": "Mes *",
  "Cuenta *": "Compte *",
  "Real *": "Real *",
  "A fecha de *": "A data de *",
  "Saldo *": "Saldo *",
  "Factura *": "Factura *",
  "Vencimiento *": "Venciment *",
  "Importe *": "Import *",
  "Factura recibida *": "Factura rebuda *",
  "Periodo *": "Període *",
  "Tipo *": "Tipus *",
  "Línea de financiación *": "Línia de finançament *",
  "Pendiente *": "Pendent *",
  "código de cuenta": "codi de compte",
  "Cada cuenta, su estado financiero y la línea en la que se agrupa. Los presupuestos son anuales; el presupuesto del periodo se distribuye uniformemente.":
    "Cada compte, el seu estat financer i la línia en què s'agrupa. Els pressupostos són anuals; el pressupost del període es distribueix uniformement.",
  "Ingresos y gastos reales por mes y cuenta. La cuenta de resultados los suma en el periodo seleccionado.":
    "Ingressos i despeses reals per mes i compte. El compte de resultats els suma en el període seleccionat.",
  "Saldos de cierre a una fecha, por cuenta. El de apertura (2025-12) y el actual (2026-07) alimentan el balance y las variaciones del flujo de caja.":
    "Saldos de tancament a una data, per compte. El d'obertura (2025-12) i l'actual (2026-07) alimenten el balanç i les variacions del flux de caixa.",
  "Facturas de clientes pendientes de cobro con su vencimiento — determina el DSO y la antigüedad de cobros.":
    "Factures de clients pendents de cobrament amb el seu venciment — determina el DSO i l'antiguitat de cobraments.",
  "Facturas de proveedores pendientes de pago con su vencimiento — determina el DPO y la antigüedad de pagos.":
    "Factures de proveïdors pendents de pagament amb el seu venciment — determina el DPO i l'antiguitat de pagaments.",
  "IVA repercutido frente a soportado por periodo (Modelo 303) y retenciones (Modelo 111) — el neto es lo que se liquida.":
    "IVA repercutit enfront de suportat per període (Model 303) i retencions (Model 111) — el net és el que es liquida.",
  "Deuda pendiente con tipo y calendario — alimenta los intereses, la deuda neta y el flujo de caja de financiación.":
    "Deute pendent amb tipus i calendari — alimenta els interessos, el deute net i el flux de caixa de finançament.",
  "Saldos en vivo por cuenta — juntos forman la línea de caja del balance.":
    "Saldos en viu per compte — junts formen la línia de caixa del balanç.",
  "Datos no contables usados por los KPI — plantilla, cartera de pedidos y pipeline abierto.":
    "Dades no comptables utilitzades pels KPI — plantilla, cartera de comandes i pipeline obert.",
  "Cada cuenta, su estado financiero y la línea en la que se agrupa. Los presupuestos son anuales; el presupuesto del periodo se distribuye uniformemente. Editar cualquier cifra recalcula al instante todos los estados y KPI.":
    "Cada compte, el seu estat financer i la línia en què s'agrupa. Els pressupostos són anuals; el pressupost del període es distribueix uniformement. Editar qualsevol xifra recalcula a l'instant tots els estats i KPI.",
  "Ingresos y gastos reales por mes y cuenta. La cuenta de resultados los suma en el periodo seleccionado. Editar cualquier cifra recalcula al instante todos los estados y KPI.":
    "Ingressos i despeses reals per mes i compte. El compte de resultats els suma en el període seleccionat. Editar qualsevol xifra recalcula a l'instant tots els estats i KPI.",
  "Saldos de cierre a una fecha, por cuenta. El de apertura (2025-12) y el actual (2026-07) alimentan el balance y las variaciones del flujo de caja. Editar cualquier cifra recalcula al instante todos los estados y KPI.":
    "Saldos de tancament a una data, per compte. El d'obertura (2025-12) i l'actual (2026-07) alimenten el balanç i les variacions del flux de caixa. Editar qualsevol xifra recalcula a l'instant tots els estats i KPI.",
  "Facturas de clientes pendientes de cobro con su vencimiento — determina el DSO y la antigüedad de cobros. Editar cualquier cifra recalcula al instante todos los estados y KPI.":
    "Factures de clients pendents de cobrament amb el seu venciment — determina el DSO i l'antiguitat de cobraments. Editar qualsevol xifra recalcula a l'instant tots els estats i KPI.",
  "Facturas de proveedores pendientes de pago con su vencimiento — determina el DPO y la antigüedad de pagos. Editar cualquier cifra recalcula al instante todos los estados y KPI.":
    "Factures de proveïdors pendents de pagament amb el seu venciment — determina el DPO i l'antiguitat de pagaments. Editar qualsevol xifra recalcula a l'instant tots els estats i KPI.",
  "IVA repercutido frente a soportado por periodo (Modelo 303) y retenciones (Modelo 111) — el neto es lo que se liquida. Editar cualquier cifra recalcula al instante todos los estados y KPI.":
    "IVA repercutit enfront de suportat per període (Model 303) i retencions (Model 111) — el net és el que es liquida. Editar qualsevol xifra recalcula a l'instant tots els estats i KPI.",
  "Deuda pendiente con tipo y calendario — alimenta los intereses, la deuda neta y el flujo de caja de financiación. Editar cualquier cifra recalcula al instante todos los estados y KPI.":
    "Deute pendent amb tipus i calendari — alimenta els interessos, el deute net i el flux de caixa de finançament. Editar qualsevol xifra recalcula a l'instant tots els estats i KPI.",
  "Saldos en vivo por cuenta — juntos forman la línea de caja del balance. Editar cualquier cifra recalcula al instante todos los estados y KPI.":
    "Saldos en viu per compte — junts formen la línia de caixa del balanç. Editar qualsevol xifra recalcula a l'instant tots els estats i KPI.",
  "Datos no contables usados por los KPI — plantilla, cartera de pedidos y pipeline abierto. Editar cualquier cifra recalcula al instante todos los estados y KPI.":
    "Dades no comptables utilitzades pels KPI — plantilla, cartera de comandes i pipeline obert. Editar qualsevol xifra recalcula a l'instant tots els estats i KPI.",
  "p. ej. 2026-Q2": "p. ex. 2026-Q2",
  "Pago parcial": "Pagament parcial",
  "En consulta": "En consulta",
  "Libros (entradas)": "Llibres (entrades)",
  "Resumen — Panel de KPI": "Resum — Tauler de KPI",
  "Estados financieros — Cuenta de resultados": "Estats financers — Compte de resultats",
  "Estados financieros — Balance": "Estats financers — Balanç",
  "Estados financieros — Flujo de caja": "Estats financers — Flux de caixa",
  "Capital circulante — Antigüedad cobros / pagos":
    "Capital circulant — Antiguitat cobraments / pagaments",
  "Capital circulante — Cuentas a cobrar": "Capital circulant — Comptes a cobrar",
  "Capital circulante — Cuentas a pagar": "Capital circulant — Comptes a pagar",
  "Capital circulante — IVA e impuestos": "Capital circulant — IVA i impostos",
  "Libros (entradas) — Plan de cuentas": "Llibres (entrades) — Pla de comptes",
  "Libros (entradas) — Libro de PyG": "Llibres (entrades) — Llibre de PiG",
  "Libros (entradas) — Entrada de balance": "Llibres (entrades) — Entrada de balanç",
  "Libros (entradas) — Bancos y caja": "Llibres (entrades) — Bancs i caixa",
  "Libros (entradas) — Préstamos": "Llibres (entrades) — Préstecs",
  "Libros (entradas) — Variables": "Llibres (entrades) — Variables",
  "Ingresos — reformas": "Ingressos — reformes",
  Materiales: "Materials",
  "Mano de obra en obra": "Mà d'obra a l'obra",
  "Salarios — administración y dirección": "Salaris — administració i direcció",
  Suministros: "Subministraments",
  "Vehículos y combustible": "Vehicles i combustible",
  "Honorarios profesionales (gestor)": "Honoraris professionals (gestor)",
  "Oficina y otros": "Oficina i altres",
  Amortización: "Amortització",
  "Deudores comerciales (AR)": "Deutors comercials (AR)",
  "Existencias / materiales": "Existències / materials",
  "Obra en curso (WIP)": "Obra en curs (WIP)",
  "Equipos y vehículos (neto)": "Equips i vehicles (net)",
  "Acreedores comerciales (AP)": "Creditors comercials (AP)",
  "IVA e impuestos a pagar": "IVA i impostos a pagar",
  "Préstamo bancario": "Préstec bancari",
  "Resultados acumulados": "Resultats acumulats",
  "Préstamo bancario — expansión": "Préstec bancari — expansió",
  "Cuenta operativa": "Compte operatiu",
  "Reserva para impuestos": "Reserva per a impostos",
  "700 · Ingresos — reformas": "700 · Ingressos — reformes",
  "600 · Materiales": "600 · Materials",
  "601 · Subcontratas": "601 · Subcontractes",
  "602 · Mano de obra en obra": "602 · Mà d'obra a l'obra",
  "640 · Salarios — administración y dirección": "640 · Salaris — administració i direcció",
  "621 · Alquiler": "621 · Lloguer",
  "628 · Suministros": "628 · Subministraments",
  "625 · Seguros": "625 · Assegurances",
  "629 · Vehículos y combustible": "629 · Vehicles i combustible",
  "623 · Honorarios profesionales (gestor)": "623 · Honoraris professionals (gestor)",
  "626 · Oficina y otros": "626 · Oficina i altres",
  "Panel en vivo para T1 2026. Todo se calcula a partir de los libros — edita una entrada y todas las cifras se actualizan.":
    "Tauler en viu per al T1 2026. Tot es calcula a partir dels llibres — editeu una entrada i totes les xifres s'actualitzen.",
  "Panel en vivo para T2 2026. Todo se calcula a partir de los libros — edita una entrada y todas las cifras se actualizan.":
    "Tauler en viu per al T2 2026. Tot es calcula a partir dels llibres — editeu una entrada i totes les xifres s'actualitzen.",
  "Panel en vivo para Julio 2026. Todo se calcula a partir de los libros — edita una entrada y todas las cifras se actualizan.":
    "Tauler en viu per al Juliol 2026. Tot es calcula a partir dels llibres — editeu una entrada i totes les xifres s'actualitzen.",
  "Panel en vivo para Plan anual (presupuesto). Todo se calcula a partir de los libros — edita una entrada y todas las cifras se actualizan.":
    "Tauler en viu per al Pla anual (pressupost). Tot es calcula a partir dels llibres — editeu una entrada i totes les xifres s'actualitzen.",
  "Posición de tesorería": "Posició de tresoreria",
  "en objetivo": "en objectiu",
  "Listo para automatizar.": "A punt per automatitzar.",
  "Estos KPI, estados y antigüedades se derivan, no se teclean — el mismo motor puede enviar alertas de cierre de mes, comprobaciones de covenants y avisos de recorrido de caja.":
    "Aquests KPI, estats i antiguitats es deriven, no es teclegen — el mateix motor pot enviar alertes de tancament de mes, comprovacions de covenants i avisos de recorregut de caixa.",
  "envía el conjunto de datos completo como JSON.": "envia el conjunt de dades complet com a JSON.",
  "Cuenta de pérdidas y ganancias": "Compte de pèrdues i guanys",
  "Acumulado 2026 (ene–jul) · Real vs presupuesto": "Acumulat 2026 (gen–jul) · Real vs pressupost",
  "T1 2026 · Real vs presupuesto": "T1 2026 · Real vs pressupost",
  "T2 2026 · Real vs presupuesto": "T2 2026 · Real vs pressupost",
  "Julio 2026 · Real vs presupuesto": "Juliol 2026 · Real vs pressupost",
  "Plan anual (presupuesto) · Real vs presupuesto": "Pla anual (pressupost) · Real vs pressupost",
  Variación: "Variació",
  "Beneficio antes de impuestos": "Benefici abans d'impostos",
  "a 21/07/2026 vs apertura": "a 21/07/2026 vs obertura",
  Actual: "Actual",
  Apertura: "Obertura",
  ACTIVO: "ACTIU",
  "Total activo": "Total actiu",
  "PASIVO Y PATRIMONIO NETO": "PASSIU I PATRIMONI NET",
  "Total pasivo y patrimonio neto": "Total passiu i patrimoni net",
  "✓ El balance cuadra — Activo = Pasivo + Patrimonio neto.":
    "✓ El balanç quadra — Actiu = Passiu + Patrimoni net.",
  "Estado de flujos de efectivo": "Estat de fluxos d'efectiu",
  "Acumulado · método indirecto": "Acumulat · mètode indirecte",
  "Actividades de explotación": "Activitats d'explotació",
  "Más: amortización": "Més: amortització",
  "Δ Cuentas a cobrar": "Δ Comptes a cobrar",
  "Δ Existencias": "Δ Existències",
  "Δ Obra en curso": "Δ Obra en curs",
  "Δ Cuentas a pagar": "Δ Comptes a pagar",
  "Δ IVA e impuestos a pagar": "Δ IVA i impostos a pagar",
  "Caja neta de explotación": "Caixa neta d'explotació",
  "Actividades de inversión": "Activitats d'inversió",
  "Inversiones (capex)": "Inversions (capex)",
  "Caja neta de inversión": "Caixa neta d'inversió",
  "Actividades de financiación": "Activitats de finançament",
  "Δ Préstamos": "Δ Préstecs",
  "Δ Capital social": "Δ Capital social",
  "Caja neta de financiación": "Caixa neta de finançament",
  "Variación neta de caja": "Variació neta de caixa",
  "Caja inicial (31/12/2025)": "Caixa inicial (31/12/2025)",
  "Caja final (21/07/2026)": "Caixa final (21/07/2026)",
  "✓ Concilia: la apertura más la variación neta es igual a la línea de caja del balance.":
    "✓ Concilia: l'obertura més la variació neta és igual a la línia de caixa del balanç.",
  "Radar de capital circulante.": "Radar de capital circulant.",
  "Antigüedad de cobros (AR)": "Antiguitat de cobraments (AR)",
  "Antigüedad de pagos (AP)": "Antiguitat de pagaments (AP)",
  "1–30 días": "1–30 dies",
  "31–60 días": "31–60 dies",
  "61–90 días": "61–90 dies",
  "90+ días": "90+ dies",
  "no vencido": "no vençut",
  "Sin filas.": "Sense files.",
  "Haz clic en": "Fes clic a",
  "para crear una.": "per crear-ne una.",
  "1 fila": "1 fila",
  "Editar Plan de cuentas": "Editar Pla de comptes",
  "Editar Libro de PyG (mensual)": "Editar Llibre de PiG (mensual)",
  "Editar Balance (instantáneas)": "Editar Balanç (instantànies)",
  "Editar Cuentas a cobrar (abiertas)": "Editar Comptes a cobrar (oberts)",
  "Editar Cuentas a pagar (abiertas)": "Editar Comptes a pagar (oberts)",
  "Editar Libro de IVA e impuestos": "Editar Llibre d'IVA i impostos",
  "Editar Préstamos y financiación": "Editar Préstecs i finançament",
  "Editar Cuentas bancarias y de caja": "Editar Comptes bancaris i de caixa",
  "Editar Variables e hipótesis": "Editar Variables i hipòtesis",
  "Nuevo — Plan de cuentas": "Nou — Pla de comptes",
  "Nuevo — Libro de PyG (mensual)": "Nou — Llibre de PiG (mensual)",
  "Nuevo — Balance (instantáneas)": "Nou — Balanç (instantànies)",
  "Nuevo — Cuentas a cobrar (abiertas)": "Nou — Comptes a cobrar (oberts)",
  "Nuevo — Cuentas a pagar (abiertas)": "Nou — Comptes a pagar (oberts)",
  "Nuevo — Libro de IVA e impuestos": "Nou — Llibre d'IVA i impostos",
  "Nuevo — Préstamos y financiación": "Nou — Préstecs i finançament",
  "Nuevo — Cuentas bancarias y de caja": "Nou — Comptes bancaris i de caixa",
  "Nuevo — Variables e hipótesis": "Nou — Variables i hipòtesis",
  "¿Eliminar esta fila?": "Voleu eliminar aquesta fila?",
  Exportado: "Exportat",
  "JSON no válido": "JSON no vàlid",
  "La importación sustituye los libros actuales por el contenido del archivo. Si tienes dudas, exporta antes una copia de seguridad. ¿Continuar?":
    "La importació substitueix els llibres actuals pel contingut del fitxer. Si tens dubtes, exporta abans una còpia de seguretat. Vols continuar?",
  "Canei Subirats ERP — Recorrido de principio a fin":
    "Canei Subirats ERP — Recorregut de principi a fi",
  "ERP · Recorrido de principio a fin": "ERP · Recorregut de principi a fi",
  "Torre de control ↗": "Torre de control ↗",
  "Constructor de presupuestos ↗": "Constructor de pressupostos ↗",
  "Libro del proyecto — en directo": "Llibre del projecte — en directe",
  "Presupuesto / línea base": "Pressupost / línia base",
  "Órdenes de cambio": "Ordres de canvi",
  "Comprometido (pedidos)": "Compromès (comandes)",
  "Ingresos (sin IVA)": "Ingressos (sense IVA)",
  "Cobrado (entradas de caja)": "Cobrat (entrades de caixa)",
  "Pagado a proveedores (salidas)": "Pagat a proveïdors (sortides)",
  "Avance por el ciclo de vida de 13 etapas": "Avanç pel cicle de vida de 13 etapes",
  "Carpeta del proyecto": "Carpeta del projecte",
  vacía: "buida",
  "⤓ Descargar carpeta (.zip)": "⤓ Descarregar carpeta (.zip)",
  "✎ Editar datos del proyecto": "✎ Editar dades del projecte",
  "Empezar de nuevo": "Començar de nou",
  "Fotos, documentos, los PDF de presupuesto y factura y cada correo se archivan por paso — guardados en este dispositivo.":
    "Fotos, documents, els PDF de pressupost i factura i cada correu s'arxiven per pas — desats en aquest dispositiu.",
  "Registro de actividad": "Registre d'activitat",
  "Listo — introduzca su proyecto para comenzar.":
    "A punt — introduïu el vostre projecte per començar.",
  "Qué demuestra esto": "Què demostra això",
  "Sus datos, un único hilo limpio: captación → beneficio → reseña.":
    "Les vostres dades, un únic fil net: captació → benefici → ressenya.",
  "Introduzca el proyecto una vez; el libro, los PDF de presupuesto y factura y cada correo al cliente se generan automáticamente con la identidad corporativa completa. Las fotos y los documentos se archivan por paso en una carpeta de proyecto descargable. Los correos se redactan para su revisión — nada se envía sin usted.":
    "Introduïu el projecte una vegada; el llibre, els PDF de pressupost i factura i cada correu al client es generen automàticament amb la identitat corporativa completa. Les fotos i els documents s'arxiven per pas en una carpeta de projecte descarregable. Els correus es redacten perquè els reviseu — no s'envia res sense vosaltres.",
  Captación: "Captació",
  "Visita de obra": "Visita d'obra",
  "01 Captación": "01 Captació",
  "02 Visita de obra": "02 Visita d'obra",
  "03 Estimación": "03 Estimació",
  "04 Presupuesto": "04 Pressupost",
  "05 Aceptación": "05 Acceptació",
  "06 Proyecto": "06 Projecte",
  "07 Compras": "07 Compres",
  "08 Ejecución": "08 Execució",
  "09 Facturación": "09 Facturació",
  "10 Cobros": "10 Cobraments",
  "11 Pagos a proveedores": "11 Pagaments a proveïdors",
  "12 Cierre": "12 Tancament",
  "13 Solicitud de reseña": "13 Sol·licitud de ressenya",
  "“02 Visita de obra”": "“02 Visita d'obra”",
  "Aún no hay archivos — avance el recorrido, añada fotos y documentos.":
    "Encara no hi ha fitxers — avanceu el recorregut, afegiu-hi fotos i documents.",
  "Llega una consulta. Registramos al cliente y abrimos un":
    "Arriba una consulta. Registrem el client i obrim un",
  "en el pipeline con una próxima acción — ningún dato perdido en un papel suelto.":
    "al pipeline amb una acció següent — cap dada perduda en un paper solt.",
  "Cliente:": "Client:",
  "Lead abierto:": "Lead obert:",
  nuevo: "nou",
  "· próxima acción: concertar una visita de obra": "· acció següent: concertar una visita d'obra",
  "La visita de obra se registra en el móvil: notas, mediciones y":
    "La visita d'obra es registra al mòbil: notes, mesuraments i",
  fotos: "fotos",
  ". Todo se archiva en la carpeta": ". Tot s'arxiva a la carpeta",
  "y alimenta la estimación sin volver a teclear.":
    "i alimenta l'estimació sense tornar a teclejar.",
  "Notas de la visita:": "Notes de la visita:",
  "— añada notas en los datos del proyecto —": "— afegiu notes a les dades del projecte —",
  "Una rápida": "Una ràpida",
  estimación: "estimació",
  "a partir de la visita — una cifra de orden de magnitud para alinearse con el cliente antes del presupuesto detallado.":
    "a partir de la visita — una xifra d'ordre de magnitud per alinear-se amb el client abans del pressupost detallat.",
  "Estimación:": "Estimació:",
  "El detallado": "El detallat",
  "se construye desde el catálogo (elegir, no teclear). Los trabajos opcionales se listan aparte para que nunca inflen el precio principal.":
    "es construeix des del catàleg (triar, no teclejar). Els treballs opcionals es llisten a part perquè mai no inflin el preu principal.",
  "Total base": "Total base",
  "El cliente": "El client",
  acepta: "accepta",
  ". La versión aceptada queda congelada; se redacta automáticamente un correo de aceptación con imagen corporativa y el presupuesto aceptado adjunto.":
    ". La versió acceptada queda congelada; es redacta automàticament un correu d'acceptació amb imatge corporativa i el pressupost acceptat adjunt.",
  "Aceptado:": "Acceptat:",
  "El presupuesto aceptado se convierte en un": "El pressupost acceptat es converteix en un",
  proyecto: "projecte",
  "con una": "amb una",
  "línea base inmutable": "línia base immutable",
  "— sin volver a introducir datos. Los presupuestos por partida se copian una vez y se congelan; todo lo que sigue se mide contra ellos.":
    "— sense tornar a introduir dades. Els pressupostos per partida es copien una vegada i es congelen; tot el que segueix es mesura contra ells.",
  "Proyecto abierto:": "Projecte obert:",
  "Órdenes de compra": "Comandes de compra",
  "se emiten a los proveedores seleccionados — esto": "s'emeten als proveïdors seleccionats — això",
  compromete: "compromet",
  "coste contra el presupuesto de cada partida, visible de inmediato.":
    "cost contra el pressupost de cada partida, visible de seguida.",
  "vs presupuesto": "vs pressupost",
  "✓ dentro": "✓ dins",
  "⚠ excedido": "⚠ excedit",
  "La obra avanza. Las": "L'obra avança. Les",
  "facturas de proveedor": "factures de proveïdor",
  "se contabilizan como": "es comptabilitzen com a",
  "coste real": "cost real",
  "(los duplicados se rechazan). El presupuestado-vs-real por partida se actualiza en directo — la alerta temprana de fugas de margen.":
    "(els duplicats es rebutgen). El pressupostat-vs-real per partida s'actualitza en directe — l'alerta primerenca de fuites de marge.",
  La: "La",
  "se emite a partir del presupuesto aceptado. El tipo de IVA reducido se aplica cuando se cumplen las condiciones de la reforma de vivienda (art. 91.Uno.2.10º), y el motivo queda registrado en el documento. La numeración no tiene huecos y cada factura queda sellada contra manipulaciones.":
    "s'emet a partir del pressupost acceptat. El tipus d'IVA reduït s'aplica quan es compleixen les condicions de la reforma d'habitatge (art. 91.U.2.10è), i el motiu queda registrat al document. La numeració no té buits i cada factura queda segellada contra manipulacions.",
  "Facturar a": "Facturar a",
  "PDF de factura completa archivado en “09 Facturación”":
    "PDF de factura completa arxivat a “09 Facturació”",
  "— partes (NIF/CIF), fechas, subpartidas, desglose de IVA, IBAN y condiciones de pago, y las notas legales.":
    "— parts (NIF/CIF), dates, subpartides, desglossament d'IVA, IBAN i condicions de pagament, i les notes legals.",
  "Emitida en español (es-ES).": "Emesa en espanyol (es-ES).",
  "Total a pagar": "Total a pagar",
  "Cobros:": "Cobraments:",
  "se registra un cobro y se asigna a la factura (los pagos parciales son de primera). Las facturas vencidas generan un recordatorio — redactado automáticamente abajo.":
    "es registra un cobrament i s'assigna a la factura (els pagaments parcials són de primera). Les factures vençudes generen un recordatori — redactat automàticament a sota.",
  "Cobro:": "Cobrament:",
  pagada: "pagada",
  "Pagos a proveedores:": "Pagaments a proveïdors:",
  "las facturas contabilizadas se pagan a su vencimiento. La salida de caja ya es visible frente a la entrada.":
    "les factures comptabilitzades es paguen al seu venciment. La sortida de caixa ja és visible enfront de l'entrada.",
  "Pagado:": "Pagat:",
  "Cierre del proyecto": "Tancament del projecte",
  "con la verdad que pide el BRD: no facturación —":
    "amb la veritat que demana el BRD: no facturació —",
  rentabilidad: "rendibilitat",
  ". Ingresos, coste real, margen y presupuestado-vs-real por partida.":
    ". Ingressos, cost real, marge i pressupostat-vs-real per partida.",
  "Cerrado con beneficio.": "Tancat amb benefici.",
  "El paso final, a menudo olvidado:": "El pas final, sovint oblidat:",
  "pedir una reseña con la buena impresión aún fresca":
    "demanar una ressenya amb la bona impressió encara fresca",
  ". En cuanto se cierra el proyecto, se redacta automáticamente una solicitud con imagen corporativa — un clic la envía, o descárguela como borrador de Outlook. Esto cierra el círculo desde la primera consulta hasta la reputación.":
    ". Tan bon punt es tanca el projecte, es redacta automàticament una sol·licitud amb imatge corporativa — un clic l'envia, o descarregueu-la com a esborrany de l'Outlook. Això tanca el cercle des de la primera consulta fins a la reputació.",
  "Solicitud de reseña redactada": "Sol·licitud de ressenya redactada",
  "capacidad:": "capacitat:",
  "· carpeta:": "· carpeta:",
  "visitas / móvil": "visites / mòbil",
  presupuestos: "pressupostos",
  "presupuestos · aprovisionamiento": "pressupostos · aprovisionament",
  "presupuestos · mensajería": "pressupostos · missatgeria",
  "compras · aprovisionamiento": "compres · aprovisionament",
  "ejecución · pagos": "execució · pagaments",
  cobros: "cobraments",
  pagos: "pagaments",
  "proyectos · FIN": "projectes · FIN",
  "mensajería · reputación": "missatgeria · reputació",
  "✓ Enviado": "✓ Enviat",
  "⤓ Borrador de Outlook + PDF (.eml)": "⤓ Esborrany de l'Outlook + PDF (.eml)",
  "Redactado para su revisión — se envía solo con su visto bueno":
    "Redactat perquè el reviseu — només s'envia amb el vostre vistiplau",
  "Fotos de la visita de obra · guardadas en “02 Visita de obra”":
    "Fotos de la visita d'obra · desades a “02 Visita d'obra”",
  "📷 Hacer / añadir fotos": "📷 Fer / afegir fotos",
  "Abre la cámara en el móvil.": "Obre la càmera al mòbil.",
  "＋ Adjuntar archivo": "＋ Adjuntar fitxer",
  "Recorrido completado ✓": "Recorregut completat ✓",
  "Avanzar — Visita de obra ▸": "Avançar — Visita d'obra ▸",
  "Avanzar — Estimación ▸": "Avançar — Estimació ▸",
  "Avanzar — Presupuesto ▸": "Avançar — Pressupost ▸",
  "Avanzar — Aceptación ▸": "Avançar — Acceptació ▸",
  "Avanzar — Proyecto ▸": "Avançar — Projecte ▸",
  "Avanzar — Compras ▸": "Avançar — Compres ▸",
  "Avanzar — Ejecución ▸": "Avançar — Execució ▸",
  "Avanzar — Facturación ▸": "Avançar — Facturació ▸",
  "Avanzar — Cobros ▸": "Avançar — Cobraments ▸",
  "Avanzar — Pagos a proveedores ▸": "Avançar — Pagaments a proveïdors ▸",
  "Avanzar — Cierre · beneficio ▸": "Avançar — Tancament · benefici ▸",
  "Avanzar — Solicitud de reseña ▸": "Avançar — Sol·licitud de ressenya ▸",
  "Detalles de este paso — rellene lo que aplique":
    "Detalls d'aquest pas — ompliu el que correspongui",
  "Origen del lead": "Origen del lead",
  Web: "Web",
  "Consulta / alcance": "Consulta / abast",
  "Qué quiere el cliente": "Què vol el client",
  "Presupuesto objetivo (€)": "Pressupost objectiu (€)",
  opcional: "opcional",
  "Fecha de la visita": "Data de la visita",
  "Superficie (m²)": "Superfície (m²)",
  "p. ej. 12,5": "p. ex. 12,5",
  "Acceso / condicionantes": "Accés / condicionants",
  "ascensor, aparcamiento, horario…": "ascensor, aparcament, horari…",
  "Validez del presupuesto (días)": "Validesa del pressupost (dies)",
  "Nota para el cliente": "Nota per al client",
  "se muestra en el presupuesto": "es mostra al pressupost",
  Método: "Mètode",
  "Anticipo acordado (%)": "Bestreta acordada (%)",
  "p. ej. 40": "p. ex. 40",
  "Fecha de inicio": "Data d'inici",
  "quién dirige la obra": "qui dirigeix l'obra",
  "proveedor elegido": "proveïdor triat",
  "Número de pedido": "Número de comanda",
  "Entrega prevista": "Lliurament previst",
  "N.º de factura de proveedor": "Núm. de factura de proveïdor",
  "Fecha de la factura": "Data de la factura",
  "Avance de obra (%)": "Avanç d'obra (%)",
  "Fecha de factura": "Data de factura",
  "Tipo de factura": "Tipus de factura",
  Completa: "Completa",
  Anticipo: "Bestreta",
  Certificación: "Certificació",
  "Retención IRPF (%)": "Retenció IRPF (%)",
  "Retención B2B — deje 0 para clientes particulares":
    "Retenció B2B — deixeu 0 per a clients particulars",
  "Pago recibido": "Pagament rebut",
  "Proveedor pagado el": "Proveïdor pagat el",
  "Fecha de fin real": "Data de fi real",
  "Notas de cierre / lecciones": "Notes de tancament / lliçons",
  "qué presupuestar distinto la próxima vez": "què pressupostar diferent la propera vegada",
  "Plataforma de reseñas": "Plataforma de ressenyes",
  "Enviar el": "Enviar el",
  "Estimador — números aproximados (cantidad × precio unitario)":
    "Estimador — números aproximats (quantitat × preu unitari)",
  "＋ Añadir del catálogo…": "＋ Afegir del catàleg…",
  "＋ Línea en blanco": "＋ Línia en blanc",
  "Imprevistos (%)": "Imprevistos (%)",
  "→ Usar como presupuesto": "→ Usar com a pressupost",
  "✓ Enviado al presupuesto": "✓ Enviat al pressupost",
  "una horquilla para alinearse antes del presupuesto detallado":
    "una forquilla per alinear-se abans del pressupost detallat",
  "Trabajo / estancia": "Treball / estança",
  "cant.": "quant.",
  "€/unidad": "€/unitat",
  "Añada al menos una línea con descripción y precio.":
    "Afegiu almenys una línia amb descripció i preu.",
  "Constructor de presupuestos — catálogo primero (concepto · cantidad · precio · importe)":
    "Constructor de pressupostos — catàleg primer (concepte · quantitat · preu · import)",
  "＋ Añadir subpartida del catálogo…": "＋ Afegir subpartida del catàleg…",
  "＋ Línea personalizada": "＋ Línia personalitzada",
  "✓ Actualizar presupuesto": "✓ Actualitzar el pressupost",
  "Base del presupuesto:": "Base del pressupost:",
  "agrupadas por partida en el PDF": "agrupades per partida al PDF",
  "Añada al menos una línea con concepto, cantidad y precio.":
    "Afegiu almenys una línia amb concepte, quantitat i preu.",
  "Otros trabajos": "Altres treballs",
  "Planificación — tareas y cronograma": "Planificació — tasques i cronograma",
  "＋ Añadir tarea": "＋ Afegir tasca",
  "Añada fechas para ver el cronograma.": "Afegiu dates per veure el cronograma.",
  "Órdenes de cambio (modificaciones) — la línea base permanece congelada":
    "Ordres de canvi (modificacions) — la línia base roman congelada",
  "＋ Añadir orden de cambio": "＋ Afegir ordre de canvi",
  "Modificación (p. ej. añadir suelo radiante)": "Modificació (p. ex. afegir terra radiant)",
  "Línea base:": "Línia base:",
  "Modificaciones:": "Modificacions:",
  "Contrato actual:": "Contracte actual:",
  "Su proyecto": "El vostre projecte",
  "Ejecute todo el proceso con sus propios datos":
    "Executeu tot el procés amb les vostres pròpies dades",
  "Introduzca un cliente y un presupuesto reales — o cargue el ejemplo. Cada paso se ejecuta entonces automáticamente y archiva sus documentos en la carpeta del proyecto. Guardado en este dispositivo.":
    "Introduïu un client i un pressupost reals — o carregueu l'exemple. Cada pas s'executa aleshores automàticament i arxiva els vostres documents a la carpeta del projecte. Desat en aquest dispositiu.",
  "NIF / CIF (identificación fiscal)": "NIF / CIF (identificació fiscal)",
  "p. ej. Laura Puig": "p. ex. Laura Puig",
  "Calle, código postal, población": "Carrer, codi postal, població",
  "Notas y mediciones": "Notes i mesuraments",
  "Estancias, medidas, qué picar / desplazar, materiales…":
    "Estances, mides, què picar / desplaçar, materials…",
  "Las fotos se hacen después, en el paso de Visita de obra, y se guardan en la carpeta.":
    "Les fotos es fan després, al pas de Visita d'obra, i es desen a la carpeta.",
  "Facturación y costes": "Facturació i costos",
  "Número de presupuesto": "Número de pressupost",
  "Número de factura": "Número de factura",
  "Tipo de IVA": "Tipus d'IVA",
  "10% — reforma de vivienda": "10% — reforma d'habitatge",
  "21% — general": "21% — general",
  "4% — superreducido": "4% — superreduït",
  "Exento / no sujeto": "Exempt / no subjecte",
  "Plazo de pago (días)": "Termini de pagament (dies)",
  "Coste estimado como % del precio — usado para mostrar comprometido vs real y margen":
    "Cost estimat com a % del preu — utilitzat per mostrar compromès vs real i marge",
  "Iniciar el recorrido ▸": "Iniciar el recorregut ▸",
  "Cargar datos de ejemplo": "Carregar dades d'exemple",
  Limpiar: "Netejar",
  "Sus datos se guardan solo en este navegador. “Descargar carpeta (.zip)” lo exporta todo.":
    "Les vostres dades es desen només en aquest navegador. “Descarregar carpeta (.zip)” ho exporta tot.",
  "Presupuesto — partidas": "Pressupost — partides",
  "Partidas rápidas — los detallará desde el catálogo en el":
    "Partides ràpides — les detallareu des del catàleg al",
  "Cargado del catálogo. Refine cada línea en el": "Carregat del catàleg. Refineu cada línia al",
  "paso 4 · Presupuesto": "pas 4 · Pressupost",
  "Partida / trabajo (p. ej. Fontanería)": "Partida / treball (p. ex. Lampisteria)",
  "€ presupuesto": "€ pressupost",
  "＋ Añadir partida": "＋ Afegir partida",
  "Trabajo opcional (se muestra aparte)": "Treball opcional (es mostra a part)",
  "p. ej. Mampara": "p. ex. Mampara",
  "Importe opcional (€)": "Import opcional (€)",
  "Introduzca el nombre del cliente.": "Introduïu el nom del client.",
  "Añada al menos una partida con presupuesto (o cargue el ejemplo).":
    "Afegiu almenys una partida amb pressupost (o carregueu l'exemple).",
  "Introduzca un NIF / CIF válido del cliente — es obligatorio para una factura legal.":
    "Introduïu un NIF / CIF vàlid del client — és obligatori per a una factura legal.",
  "Introduzca un correo electrónico válido del cliente — los documentos se envían a esta dirección.":
    "Introduïu una adreça electrònica vàlida del client — els documents s'envien a aquesta adreça.",
  "¿Empezar de nuevo? Esto borra el proyecto guardado y todos sus archivos en este dispositivo.":
    "Voleu començar de nou? Això esborra el projecte desat i tots els seus fitxers en aquest dispositiu.",
  "Borrado — introduzca su proyecto para comenzar.":
    "Esborrat — introduïu el vostre projecte per començar.",
  "Proyecto guardado cargado — pulse “Iniciar el recorrido”.":
    "Projecte desat carregat — premeu “Iniciar el recorregut”.",
  "Visita de obra registrada": "Visita d'obra registrada",
  "Presupuesto aceptado — versión congelada": "Pressupost acceptat — versió congelada",
  "Solicitud de reseña redactada — círculo cerrado":
    "Sol·licitud de ressenya redactada — cercle tancat",
  "Proyecto confirmado — Canei Subirats": "Projecte confirmat — Canei Subirats",
  "Su proyecto está confirmado": "El vostre projecte està confirmat",
  "Ver mi proyecto": "Veure el meu projecte",
  "Gracias por confiar en nosotros para su reforma. Adjuntamos su factura.":
    "Gràcies per confiar en nosaltres per a la vostra reforma. Us adjuntem la factura.",
  "Total a pagar:": "Total a pagar:",
  "Realizar el pago": "Fer el pagament",
  "Un recordatorio amable": "Un recordatori amable",
  "Si ya ha realizado el pago, disculpe este mensaje — y muchas gracias.":
    "Si ja heu fet el pagament, disculpeu aquest missatge — i moltes gràcies.",
  "Pagar la factura": "Pagar la factura",
  "¿Qué le ha parecido? — un pequeño favor": "Què us ha semblat? — un petit favor",
  "¿Qué le ha parecido?": "Què us ha semblat?",
  "Ha sido un placer reformar su hogar y esperamos que disfrute del resultado cada día.":
    "Ha estat un plaer reformar la vostra llar i esperem que gaudiu del resultat cada dia.",
  "Si dispone de 30 segundos, una breve reseña significa muchísimo para una empresa familiar como la nuestra — y ayuda a sus vecinos a encontrar un equipo de confianza.":
    "Si disposeu de 30 segons, una breu ressenya significa moltíssim per a una empresa familiar com la nostra — i ajuda els vostres veïns a trobar un equip de confiança.",
  "★ Dejar una reseña en Google": "★ Deixar una ressenya a Google",
  "Adjuntamos su presupuesto aceptado por un importe de":
    "Us adjuntem el pressupost acceptat per un import de",
  "(IVA no incluido). Programaremos los trabajos y le mantendremos informado/a en cada hito, con un único punto de contacto durante todo el proyecto y 2 años de garantía sobre los trabajos.":
    "(IVA no inclòs). Programarem els treballs i us mantindrem informat/da a cada fita, amb un únic punt de contacte durant tot el projecte i 2 anys de garantia sobre els treballs.",
  "Un cordial saludo,": "Una salutació cordial,",
  "El equipo de Canei Subirats": "L'equip de Canei Subirats",
  "PDF · adjunto a este correo": "PDF · adjunt a aquest correu",
  "hola@caneisubirats.com · 659 87 67 00 · 2 años de garantía sobre los trabajos":
    "hola@caneisubirats.com · 659 87 67 00 · 2 anys de garantia sobre els treballs",
  "CANT.": "QUANT.",
  PRECIO: "PREU",
  IMPORTE: "IMPORT",
  "Valido hasta": "Valid fins a",
  "Base (IVA no incl.)": "Base (IVA no incl.)",
  "Aceptado (con opcional)": "Acceptat (amb opcional)",
  "Opcional (aparte)": "Opcional (a part)",
  "Total aceptado (IVA no incl.)": "Total acceptat (IVA no incl.)",
  "Los trabajos opcionales solo se incluyen si los acepta.":
    "Els treballs opcionals només s'inclouen si els accepteu.",
  "El IVA se aplica en la factura final segun la regla legal aplicable.":
    "L'IVA s'aplica a la factura final segons la regla legal aplicable.",
  "Precios validos 30 dias. Los trabajos se planifican al aceptar el presupuesto.":
    "Preus valids 30 dies. Els treballs es planifiquen en acceptar el pressupost.",
  "Este presupuesto se convierte en su proyecto con una linea base fija — sin re-introducir datos.":
    "Aquest pressupost es converteix en el vostre projecte amb una linia base fixa — sense reintroduir dades.",
  "Tipo de IVA reducido aplicado cuando concurren las condiciones de renovación de vivienda (art. 91.Uno.2.10º LIVA).":
    "Tipus d'IVA reduït aplicat quan concorren les condicions de renovació d'habitatge (art. 91.U.2.10è LIVA).",
  "Las correcciones se emiten como factura rectificativa; esta factura es inmutable.":
    "Les correccions s'emeten com a factura rectificativa; aquesta factura és immutable.",
  "Garantía de 2 años sobre los trabajos. Un único punto de contacto durante todo el proyecto.":
    "Garantia de 2 anys sobre els treballs. Un únic punt de contacte durant tot el projecte.",
  "Demolición y trabajos previos": "Enderroc i treballs previs",
  "Albañilería y tabiquería": "Paleteria i envans",
  Impermeabilización: "Impermeabilització",
  "Calefacción y climatización": "Calefacció i climatització",
  "Revestimientos y acabados": "Revestiments i acabats",
  "Carpintería y equipamiento": "Fusteria i equipament",
  "Ventanas y cristalería": "Finestres i vidrieria",
  "Picado de alicatado / solado antiguo": "Picat d'enrajolat / paviment antic",
  "Desmontaje de sanitarios": "Desmuntatge de sanitaris",
  "Demolición de tabique": "Enderroc d'envà",
  "Retirada de escombros y contenedor": "Retirada de runa i contenidor",
  "Tabique nuevo (ladrillo + enfoscado)": "Envà nou (maó + arrebossat)",
  "Apertura de hueco de puerta": "Obertura de forat de porta",
  "Nivelación de solera": "Anivellament de solera",
  "Membrana impermeabilizante en zona húmeda": "Membrana impermeabilitzant en zona humida",
  "Formación de pendiente de ducha + desagüe": "Formació de pendent de dutxa + desguàs",
  "Desplazamiento de punto de agua": "Desplaçament de punt d'aigua",
  "Plato de ducha + grifería (suministro y colocación)":
    "Plat de dutxa + aixetes (subministrament i col·locació)",
  "Inodoro + lavabo (suministro y colocación)": "Vàter + lavabo (subministrament i col·locació)",
  "Columna de ducha termostática": "Columna de dutxa termostàtica",
  "Circuito nuevo + puntos": "Circuit nou + punts",
  "Actualización del cuadro eléctrico": "Actualització del quadre elèctric",
  "Foco LED (suministro y colocación)": "Focus LED (subministrament i col·locació)",
  "Toallero eléctrico": "Tovalloler elèctric",
  "Instalación de aerotermia": "Instal·lació d'aerotèrmia",
  "Radiador (suministro y colocación)": "Radiador (subministrament i col·locació)",
  "Split de aire acondicionado (suministro y colocación)":
    "Split d'aire condicionat (subministrament i col·locació)",
  "Alicatado porcelánico (suministro y colocación)":
    "Enrajolat porcellànic (subministrament i col·locació)",
  "Solado porcelánico (suministro y colocación)":
    "Paviment porcellànic (subministrament i col·locació)",
  "Acabado en microcemento": "Acabat en microciment",
  "Falso techo con LED": "Fals sostre amb LED",
  "Puerta interior (suministro y colocación)": "Porta interior (subministrament i col·locació)",
  "Armario empotrado": "Armari encastat",
  "Mueble de baño (suministro y colocación)": "Moble de bany (subministrament i col·locació)",
  "Muebles bajos + altos": "Mobles baixos + alts",
  "Encimera de cuarzo (suministro y colocación)":
    "Taulell de quars (subministrament i col·locació)",
  "Instalación de electrodomésticos": "Instal·lació d'electrodomèstics",
  "Preparación y pintado de paredes y techo": "Preparació i pintat de parets i sostre",
  "Alisado / enlucido": "Allisat / enlluït",
  "Ventana de aluminio (suministro y colocación)":
    "Finestra d'alumini (subministrament i col·locació)",
  "Mampara de vidrio templado": "Mampara de vidre trempat",
  "Mampara premium de vidrio templado": "Mampara premium de vidre trempat",
  "Reforma integral de baño + calefacción por aerotermia":
    "Reforma integral de bany + calefacció per aerotèrmia",
  "Ascensor disponible · aparcamiento en la calle · horario de trabajo 8:00–17:00":
    "Ascensor disponible · aparcament al carrer · horari de treball 8:00–17:00",
  "Incluye protección de zonas adyacentes y limpieza diaria.":
    "Inclou protecció de zones adjacents i neteja diària.",
  "Puesta en marcha de la aerotermia sin incidencias; añadir +5% a la mano de obra de alicatado la próxima vez.":
    "Posada en marxa de l'aerotèrmia sense incidències; afegir +5% a la mà d'obra d'enrajolat la propera vegada.",
  "Baño de 5,0 × 2,5 m (12,5 m² de suelo, ~37 m² de paredes) · picar alicatado antiguo · desplazar fontanería · equipo de aerotermia en la terraza · ascensor disponible, aparcamiento en la calle.":
    "Bany de 5,0 × 2,5 m (12,5 m² de terra, ~37 m² de parets) · picar enrajolat antic · desplaçar la lampisteria · equip d'aerotèrmia a la terrassa · ascensor disponible, aparcament al carrer.",
  Desactivar: "Desactivar",
  Reactivar: "Reactivar",
  "Forma de pago": "Forma de pagament",
  Aplica: "Aplica",
  "Recibo domiciliado": "Rebut domiciliat",
  "Contabilidad y finanzas": "Comptabilitat i finances",
  "Contab.": "Comptab.",
  Reportes: "Informes",
  "Presupuestos y ofertas": "Pressupostos i ofertes",
  "Facturas y cobros": "Factures i cobraments",
  Subcontratos: "Subcontractes",
  "Seguimiento técnico": "Seguiment tècnic",
  "Seguimiento económico": "Seguiment econòmic",
  "Modificaciones contractuales": "Modificacions contractuals",
  "Personal y horas": "Personal i hores",
  "Conciliación bancaria": "Conciliació bancària",
  "Catálogos y configuración": "Catàlegs i configuració",
  "↻ Datos de ejemplo": "↻ Dades d'exemple",
  "Abrir Proyectos": "Obrir Projectes",
  "Abrir Precios y costes": "Obrir Preus i costos",
  "Abrir Banco y caja": "Obrir Banc i caixa",
  "Órdenes de compra, recepciones y compromiso por partida.":
    "Comandes de compra, recepcions i compromís per partida.",
  "El registro de compras y el compromiso de coste por partida ya existen y se consultan hoy dentro de cada proyecto.":
    "El registre de compres i el compromís de cost per partida ja existeixen i es consulten avui dins de cada projecte.",
  "Contratación de industriales, ofertas comparadas y documentación obligatoria.":
    "Contractació d'industrials, ofertes comparades i documentació obligatòria.",
  "Las ofertas por proveedor y su comparativa se gestionan hoy en Precios y costes; la contratación se registra en el proyecto.":
    "Les ofertes per proveïdor i la seva comparativa es gestionen avui a Preus i costos; la contractació es registra al projecte.",
  "Avance de obra por partida, partes de trabajo y replanificación.":
    "Avanç d'obra per partida, parts de treball i replanificació.",
  "El avance por partida se marca hoy en la ficha del proyecto y alimenta la certificación.":
    "L'avanç per partida es marca avui a la fitxa del projecte i alimenta la certificació.",
  "Venta, coste comprometido, coste real y desviación por partida.":
    "Venda, cost compromès, cost real i desviació per partida.",
  "La economía de cada obra se ve hoy en la ficha del proyecto, con coste previsto frente a real por partida.":
    "L'economia de cada obra es veu avui a la fitxa del projecte, amb cost previst enfront de real per partida.",
  "Extras y modificaciones contractuales con su aprobación.":
    "Extres i modificacions contractuals amb la seva aprovació.",
  "Los extras se registran, valoran y aprueban hoy en la ficha del proyecto; sin aprobación no son facturables.":
    "Els extres es registren, es valoren i s'aproven avui a la fitxa del projecte; sense aprovació no són facturables.",
  "Importación de extracto y emparejamiento asistido con los registros.":
    "Importació d'extracte i aparellament assistit amb els registres.",
  "Los movimientos y su estado sin asignar están en Banco y caja; el emparejamiento asistido todavía no existe.":
    "Els moviments i el seu estat sense assignar són a Banc i caixa; l'aparellament assistit encara no existeix.",
  "Plantillas, reglas de envío y cola de comunicaciones al cliente.":
    "Plantilles, regles d'enviament i cua de comunicacions al client.",
  "Aún no construido. Nada se envía al cliente sin confirmación: cuando exista, seguirá esa regla.":
    "Encara no construït. No s'envia res al client sense confirmació: quan existeixi, seguirà aquesta regla.",
  "Espacio reservado en el menú (spec §8).": "Espai reservat al menú (spec §8).",
  Fechas: "Dates",
  Holgura: "Marge",
  "＋ Hito/recordatorio": "＋ Fita/recordatori",
  "Generar desde las partidas": "Generar des de les partides",
  ninguno: "cap",
  "en la ruta crítica": "a la ruta crítica",
  "días de margen": "dies de marge",
  "Planificación generada desde las partidas": "Planificació generada des de les partides",
  "Picado de alicatado / solado antiguo · 18 €/m²": "Picat d'enrajolat / paviment antic · 18 €/m²",
  "Desmontaje de sanitarios · 35 €/ud": "Desmuntatge de sanitaris · 35 €/ud",
  "Demolición de tabique · 22 €/m²": "Enderroc d'envà · 22 €/m²",
  "Retirada de escombros y contenedor · 280 €/partida":
    "Retirada de runa i contenidor · 280 €/partida",
  "Tabique nuevo (ladrillo + enfoscado) · 48 €/m²": "Envà nou (maó + arrebossat) · 48 €/m²",
  "Apertura de hueco de puerta · 180 €/ud": "Obertura de forat de porta · 180 €/ud",
  "Nivelación de solera · 26 €/m²": "Anivellament de solera · 26 €/m²",
  "Membrana impermeabilizante en zona húmeda · 24 €/m²":
    "Membrana impermeabilitzant en zona humida · 24 €/m²",
  "Formación de pendiente de ducha + desagüe · 140 €/ud":
    "Formació de pendent de dutxa + desguàs · 140 €/ud",
  "Desplazamiento de punto de agua · 120 €/ud": "Desplaçament de punt d'aigua · 120 €/ud",
  "Plato de ducha + grifería (suministro y colocación) · 640 €/ud":
    "Plat de dutxa + aixetes (subministrament i col·locació) · 640 €/ud",
  "Inodoro + lavabo (suministro y colocación) · 520 €/ud":
    "Vàter + lavabo (subministrament i col·locació) · 520 €/ud",
  "Columna de ducha termostática · 380 €/ud": "Columna de dutxa termostàtica · 380 €/ud",
  "Circuito nuevo + puntos · 85 €/ud": "Circuit nou + punts · 85 €/ud",
  "Actualización del cuadro eléctrico · 420 €/ud": "Actualització del quadre elèctric · 420 €/ud",
  "Foco LED (suministro y colocación) · 28 €/ud":
    "Focus LED (subministrament i col·locació) · 28 €/ud",
  "Toallero eléctrico · 210 €/ud": "Tovalloler elèctric · 210 €/ud",
  "Instalación de aerotermia · 1850 €/ud": "Instal·lació d'aerotèrmia · 1850 €/ud",
  "Radiador (suministro y colocación) · 260 €/ud":
    "Radiador (subministrament i col·locació) · 260 €/ud",
  "Split de aire acondicionado (suministro y colocación) · 980 €/ud":
    "Split d'aire condicionat (subministrament i col·locació) · 980 €/ud",
  "Alicatado porcelánico (suministro y colocación) · 42 €/m²":
    "Enrajolat porcellànic (subministrament i col·locació) · 42 €/m²",
  "Solado porcelánico (suministro y colocación) · 46 €/m²":
    "Paviment porcellànic (subministrament i col·locació) · 46 €/m²",
  "Acabado en microcemento · 58 €/m²": "Acabat en microciment · 58 €/m²",
  "Falso techo con LED · 38 €/m²": "Fals sostre amb LED · 38 €/m²",
  "Puerta interior (suministro y colocación) · 290 €/ud":
    "Porta interior (subministrament i col·locació) · 290 €/ud",
  "Armario empotrado · 340 €/ml": "Armari encastat · 340 €/ml",
  "Mueble de baño (suministro y colocación) · 450 €/ud":
    "Moble de bany (subministrament i col·locació) · 450 €/ud",
  "Muebles bajos + altos · 520 €/ml": "Mobles baixos + alts · 520 €/ml",
  "Encimera de cuarzo (suministro y colocación) · 320 €/ml":
    "Taulell de quars (subministrament i col·locació) · 320 €/ml",
  "Instalación de electrodomésticos · 90 €/ud": "Instal·lació d'electrodomèstics · 90 €/ud",
  "Preparación y pintado de paredes y techo · 12 €/m²":
    "Preparació i pintat de parets i sostre · 12 €/m²",
  "Alisado / enlucido · 9 €/m²": "Allisat / enlluït · 9 €/m²",
  "Ventana de aluminio (suministro y colocación) · 480 €/ud":
    "Finestra d'alumini (subministrament i col·locació) · 480 €/ud",
  "Mampara de vidrio templado · 480 €/ud": "Mampara de vidre trempat · 480 €/ud",
  // DMT-05 · Notas — la bandeja de entrada de la fase de pruebas.
  "Nueva nota": "Nova nota",
  Pantalla: "Pantalla",
  "Pantalla *": "Pantalla *",
  "Dónde exactamente": "On exactament",
  "Cómo está ahora": "Com està ara",
  "Cómo debería estar": "Com hauria d'estar",
  "Cómo debería estar *": "Com hauria d'estar *",
  "Por qué": "Per què",
  Prioridad: "Prioritat",
  "No se hará": "No es farà",
  Urgente: "Urgent",
  Menor: "Menor",
  "— elegir la pantalla —": "— triar la pantalla —",
  "Ir a la pantalla": "Anar a la pantalla",
  "＋ Nueva nota": "＋ Nova nota",
  "Buscar por pantalla o por texto": "Cercar per pantalla o per text",
  nota: "nota",
  notas: "notes",
  "Ninguna nota coincide con la búsqueda.": "Cap nota coincideix amb la cerca.",
  "Ninguna nota en este estado.": "Cap nota en aquest estat.",
  "Guardar la nota": "Desar la nota",
  "Guardar los cambios": "Desar els canvis",
  "Abierta el": "Oberta el",
  "Quién la abrió": "Qui la va obrir",
  "Cerrada el": "Tancada el",
  "Nota guardada": "Nota desada",
  "Nota actualizada": "Nota actualitzada",
  "la tarjeta de fase, en el móvil": "la targeta de fase, al mòbil",
  "Una nota necesita decir en qué pantalla": "Una nota ha de dir en quina pantalla",
  "Una nota necesita decir cómo debería estar": "Una nota ha de dir com hauria d'estar",
  "Qué falla, qué debería hacer y por qué. Cada nota apunta a una pantalla real y se cierra con una decisión.":
    "Què falla, què hauria de fer i per què. Cada nota apunta a una pantalla real i es tanca amb una decisió.",
};
window.CANEI_DICT.rxEs2Ca = [
  /* See the note on the English side. Origen and Alta are the Catalan words
     too, so the output matches the input — but the rule MATCHING is the point:
     it is the difference between a string nobody has translated and one whose
     translation happens to be identical, and only the first is a gap. */
  [new RegExp("^Origen: (.+) · Alta: (.+)$"), "Origen: $1 · Alta: $2"],
  [new RegExp("^Semana (\\d\\d/\\d\\d) – (\\d\\d/\\d\\d)$"), "Setmana $1 – $2"],
  [new RegExp("^mes (\\d{4}-\\d{2})$"), "mes $1"],
  [new RegExp("^Revisar ([\\d.,]+) sin aprobar$"), "Revisar $1 sense aprovar"],
  [new RegExp("^Revisar ([\\d.,]+) h sin aprobar$"), "Revisar $1 h sense aprovar"],
  [new RegExp("^Revisar y aprobar ([\\d.,]+)h$"), "Revisar i aprovar $1h"],
  [new RegExp("^＋ Añadir horas para (.+)$"), "＋ Afegir hores per a $1"],
  [new RegExp("^([\\d.,]+ h) · (\\d+)% del coste del periodo$"), "$1 · $2% del cost del període"],
  [new RegExp("^(\\d+) de (\\d+) apuntes$"), "$1 de $2 apunts"],
  [new RegExp("^(\\d+) de (\\d+) apunte$"), "$1 de $2 apunt"],
  [new RegExp("^Ver los (\\d+) apuntes$"), "Veure els $1 apunts"],
  [new RegExp("^(\\d+) apuntes enviados para el (\\S+)$"), "$1 apunts enviats per al $2"],
  [new RegExp("^(\\d+) apunte enviado para el (\\S+)$"), "$1 apunt enviat per al $2"],
  [new RegExp("^Semana (\\S+) – (\\S+)$"), "Setmana $1 – $2"],
  [new RegExp("^＋ Añadir otro trabajo para (.+)$"), "＋ Afegir una altra feina per a $1"],
  [
    new RegExp("^Reabrir la semana de (.+) para poder editarlo ›$"),
    "Reobrir la setmana de $1 per poder editar-ho ›",
  ],
  [new RegExp("^Apunte movido al (\\S+)$"), "Apunt mogut al $1"],
  [new RegExp("^([\\d.,]+) h · ([\\d.,]+) línea$"), "$1 h · $2 línia"],
  [new RegExp("^([\\d.,]+) h · ([\\d.,]+) líneas$"), "$1 h · $2 línies"],
  [new RegExp("^([\\d.,]+) extra$"), "$1 extra"],

  /* Same shape as the English rules: the worker's own screens compose a date or
     a count into a sentence, and only the words round it translate. */
  [new RegExp("^¿Dónde has trabajado el (\\S+)\\?$"), "On has treballat el $1?"],
  [new RegExp("^Ya apuntado el (\\S+)$"), "Ja apuntat el $1"],
  [new RegExp("^(\\d+) días trabajados$"), "$1 dies treballats"],
  [new RegExp("^(\\d+) día trabajado$"), "$1 dia treballat"],
  [new RegExp("^(\\d+) sin aprobar$"), "$1 sense aprovar"],
  // openDrawer splits a "phrase · record" title into two nodes and marks the
  // record translate="no", so only the phrase needs a rule.
  [new RegExp("^Reseña ·\\s*(.*)$"), "Ressenya · $1"],
  [new RegExp("^(P-[\\w-]+ · .+) \\(cerrada\\)$"), "$1 (tancada)"],
  [new RegExp("^Email: (\\S+@\\S+)$"), "Email: $1"],
  [new RegExp("^(.+) · caja$"), "$1 · caixa"],
  [new RegExp("^(.+) · tarjeta$"), "$1 · targeta"],
  [new RegExp("^Documento · (.+)$"), "Document · $1"],
  [new RegExp("^Justificante · (.+)$"), "Justificant · $1"],
  [new RegExp("^Costes · (.+)$"), "Costos · $1"],
  [new RegExp("^Importar extracto · (.+)$"), "Importar extracte · $1"],
  [
    new RegExp("^Ya existe un registro activo con ese NIF: (.+)$"),
    "Ja existeix un registre actiu amb aquest NIF: $1",
  ],
  [
    new RegExp(
      "^NIF/CIF no válido — DNI/NIF: la letra de control no corresponde — para (\\d{8}) debería ser ([A-Z]), no ([A-Z])\\.$",
    ),
    "NIF/CIF no vàlid — DNI/NIF: la lletra de control no correspon — per a $1 hauria de ser $2, no $3.",
  ],
  [
    new RegExp(
      "^NIF/CIF no válido — NIE: la letra de control no corresponde — para ([XYZ]\\d{7}) debería ser ([A-Z]), no ([A-Z])\\.$",
    ),
    "NIF/CIF no vàlid — NIE: la lletra de control no correspon — per a $1 hauria de ser $2, no $3.",
  ],
  [
    new RegExp(
      "^NIF/CIF no válido — CIF: el dígito o letra de control no corresponde a los siete dígitos (\\d{7})\\.$",
    ),
    "NIF/CIF no vàlid — CIF: el dígit o lletra de control no correspon als seus set dígits, $1.",
  ],
  [
    new RegExp("^«¿Qué se pregunta sobre (.+)\\?» es obligatorio\\.$"),
    "«Què es pregunta sobre $1?» és obligatori.",
  ],
  [new RegExp("^([\\d.,]+)% margen$"), "$1% marge"],
  [new RegExp("^(\\d+) obra abierta$"), "$1 obra oberta"],
  [new RegExp("^(\\d+) obras abiertas$"), "$1 obres obertes"],
  [
    new RegExp("^Margen previsto sobre (.+) de presupuesto aprobado$"),
    "Marge previst sobre $1 de pressupost aprovat",
  ],
  [new RegExp("^(\\d+) seleccionada$"), "$1 seleccionada"],
  [new RegExp("^(\\d+) seleccionadas$"), "$1 seleccionades"],
  [new RegExp("^Todas las partidas \\((\\d+)\\)$"), "Totes les partides ($1)"],
  [new RegExp("^(\\d+) subpartida del catálogo$"), "$1 subpartida del catàleg"],
  [new RegExp("^(\\d+) subpartidas del catálogo$"), "$1 subpartides del catàleg"],
  [new RegExp("^¿Eliminar la partida «(.+)»\\?$"), "Voleu eliminar la partida «$1»?"],
  [
    new RegExp("^Se eliminan también sus (\\d+) subpartidas\\. No se puede deshacer\\.$"),
    "Les seves $1 subpartides se'n van amb ella. No es pot desfer.",
  ],
  [
    new RegExp("^Se eliminan también su (\\d+) subpartida\\. No se puede deshacer\\.$"),
    "La seva $1 subpartida se'n va amb ell. No es pot desfer.",
  ],
  [new RegExp("^Partida eliminada · (\\d+) subpartidas$"), "Partida eliminata · $1 subpartides"],
  [new RegExp("^Partida eliminada · (\\d+) subpartida$"), "Partida eliminata · $1 subpartida"],
  [new RegExp("^(\\d+) subpartida del catálogo$"), "$1 subpartida del catàleg"],
  [
    new RegExp(
      "^Ninguna subpartida coincide con «(.+)» en esta partida\\. Prueba en «Todas las partidas», o créala con el botón de abajo y queda en el catálogo para la próxima vez\\.$",
    ),
    "Cap subpartida coincideix amb «$1» en aquesta partida. Prova a «Totes les partides», o crea-la amb el botó de sota i queda al catàleg per a la propera vegada.",
  ],
  [
    new RegExp(
      "^Ninguna subpartida coincide con «(.+)»\\. Créala con el botón de abajo y queda en el catálogo para la próxima vez\\.$",
    ),
    "Cap subpartida coincideix amb «$1». Crea-la amb el botó de sota i queda al catàleg per a la propera vegada.",
  ],
  [new RegExp("^(\\S+) — (.+) \\(cerrada\\)$"), "$1 — $2 (tancada)"],
  [new RegExp("^(\\S+) — (.+) \\(activa\\)$"), "$1 — $2 (activa)"],
  [new RegExp("^Siguiente paso · (\\S+)$"), "Següent pas · $1"],
  [
    new RegExp("^(\\d+) excepción sin justificar bloquean el envío del (\\S+)\\.$"),
    "$1 excepció sense justificar bloqueja l'enviament del $2.",
  ],
  [
    new RegExp("^(\\d+) excepciones sin justificar bloquean el envío del (\\S+)\\.$"),
    "$1 excepcions sense justificar bloquegen l'enviament del $2.",
  ],
  [new RegExp("^outstanding ([\\d.,]+ €)$"), "pendent $1"],
  [new RegExp("^(\\d+) meses$"), "$1 mesos"],
  [new RegExp("^(\\d+) mes$"), "$1 mes"],
  [new RegExp("^(\\d+) semanas$"), "$1 setmanes"],
  [new RegExp("^(\\d+) semana$"), "$1 setmana"],
  [
    new RegExp(
      "^(.+) · (\\d{2}\\/\\d{2}\\/\\d{4}) → (\\d{2}\\/\\d{2}\\/\\d{4}) · (\\d+) d · crítica$",
    ),
    "$1 · $2 → $3 · $4 d · crítica",
  ],
  [
    new RegExp("^(.+) · (\\d{2}\\/\\d{2}\\/\\d{4}) → (\\d{2}\\/\\d{2}\\/\\d{4}) · (\\d+) d$"),
    "$1 · $2 → $3 · $4 d",
  ],
  [
    new RegExp("^(\\S+) · hito (\\d+) · ([\\d.,]+ €) · (\\d{2}\\/\\d{2}\\/\\d{4}) · facturado$"),
    "$1 · fita $2 · $3 · $4 · facturat",
  ],
  [
    new RegExp("^(\\S+) · hito (\\d+) · ([\\d.,]+ €) · (\\d{2}\\/\\d{2}\\/\\d{4})$"),
    "$1 · fita $2 · $3 · $4",
  ],
  [new RegExp("^A la finalización · ([\\d.,]+ €) · (\\S+)$"), "A la finalització · $1 · $2"],
  [
    new RegExp("^(\\d+) traspaso\\(s\\) marcados — no cuentan como ingreso ni gasto$"),
    "$1 traspàs(os) marcats — no compten com a ingrés ni despesa",
  ],
  [
    new RegExp("^(\\d+) mensaje\\(s\\) en cola — ninguno se ha enviado$"),
    "$1 missatge(s) a la cua — cap no s'ha enviat",
  ],
  [
    new RegExp("^(\\d+) borrador\\(es\\) archivado\\(s\\) en el buz\u00f3n$"),
    "$1 esborrany(s) arxivat(s) a la b\u00fastia",
  ],
  [new RegExp("^Enviar (\\S+) · v([\\d.]+)$"), "Enviar $1 · v$2"],
  [new RegExp("^(.+) · (\\d+) d · (\\d+) visita$"), "$1 · $2 d · $3 visita"],
  [new RegExp("^(.+) · (\\d+) d · (\\d+) visitas$"), "$1 · $2 d · $3 visites"],
  [new RegExp("^(\\d+) oportunidades$"), "$1 oportunitats"],
  [new RegExp("^(\\d+) oportunidad$"), "$1 oportunitat"],
  [new RegExp("^margen previsto ([\\d.,]+\\s*€)$"), "marge previst $1"],
  [new RegExp("^de ([\\d.,]+\\s*€) facturados$"), "de $1 facturats"],
  [new RegExp("^(\\d+) cuentas$"), "$1 comptes"],
  [new RegExp("^(\\d+) clientes$"), "$1 clients"],
  [new RegExp("^(\\d+) activas · (\\d+) cerradas$"), "$1 actives · $2 tancades"],
  [new RegExp("^(\\d+) en decisión · ([\\d.,]+\\s*€)$"), "$1 en decisió · $2"],
  [new RegExp("^(\\d+) proveedores$"), "$1 proveïdors"],
  [new RegExp("^(\\d+) precios con origen$"), "$1 preus amb origen"],
  [new RegExp("^([\\d.,]+\\s*€) por cobrar$"), "$1 per cobrar"],
  [new RegExp("^(\\d+) movimientos sin asignar$"), "$1 moviments sense assignar"],
  [new RegExp("^(\\d+) contratos · (\\d+) sin firmar$"), "$1 contractes · $2 sense signar"],
  [new RegExp("^(\\d+) por validar$"), "$1 per validar"],
  [new RegExp("^(\\d+) paquetes enviados$"), "$1 paquets enviats"],
  [new RegExp("^(\\d+) visitas pendientes$"), "$1 visites pendents"],
  [new RegExp("^(\\d+) extras por valorar$"), "$1 extres per valorar"],
  [new RegExp("^(\\d+) obras con horas por registrar$"), "$1 obres amb hores per registrar"],
  [new RegExp("^(\\d+) tareas vencidas$"), "$1 tasques vençudes"],
  [new RegExp("^(\\d+) presupuestos por preparar$"), "$1 pressupostos per preparar"],
  [new RegExp("^(\\d+) cobros por reclamar$"), "$1 cobraments per reclamar"],
  [new RegExp("^(\\d+) facturas sin asignar$"), "$1 factures sense assignar"],
  [new RegExp("^(\\d+) documentos por validar$"), "$1 documents per validar"],
  [new RegExp("^con margen previsto (.+)$"), "amb marge previst $1"],
  [new RegExp("^cobrado (.+)$"), "cobrat $1"],
  [new RegExp("^(.+) en juego$"), "$1 en joc"],
  [new RegExp("^visitas pendientes — (.+)$"), "visites pendents — $1"],
  [new RegExp("^hitos de obra esta semana (.+)$"), "fites d'obra aquesta setmana $1"],
  /* «falta» is Spanish; Catalan wants «en falta / en falten».
     This rule used to map the string to ITSELF — `"$1% · falta $2"` — which
     made it untranslatable in the worst possible way: a rule that matches has
     handled the string, so the layer reported success and the miss ledger
     never recorded it. The only thing that found it was the rendered-page
     audit, comparing the Spanish and Catalan renders and seeing them
     identical. An identity rule is not a decision that two languages agree; it
     is a gap wearing the costume of one.
     Two rules because the verb agrees with the count, and the singular is not
     a rounding error on a completeness pill somebody reads every day. The
     percentage accepts a decimal — `12,5% · falta 3` is a real value. */
  [new RegExp("^([\\d.,]+)% · falta 1$"), "$1% · en falta 1"],
  [new RegExp("^([\\d.,]+)% · falta (\\d+)$"), "$1% · en falten $2"],
  [new RegExp("^(\\d+) abiertas$"), "$1 obertes"],
  [new RegExp("^Perdida · (.+)$"), "Perduda · $1"],
  [new RegExp("^≤(\\d+) días · (.+)$"), "≤$1 dies · $2"],
  [new RegExp("^≤(\\d+) días$"), "≤$1 dies"],
  [new RegExp("^(\\d+) días$"), "$1 dies"],
  [new RegExp("^(\\d+) categorías$"), "$1 categories"],
  [new RegExp("^pendiente total (.+)$"), "pendent total $1"],
  [new RegExp("^pendiente ((?!total ).+)$"), "pendent $1"],
  [new RegExp("^Vencida (\\d+)d$"), "Vençuda $1d"],
  [new RegExp("^Cobro registrado: (.+)$"), "Cobrament registrat: $1"],
  [new RegExp("^Pago registrado: (.+)$"), "Pagament registrat: $1"],
  [new RegExp("^Movimiento asignado a (.+)$"), "Moviment assignat a $1"],
  [new RegExp("^Caducado (.+)$"), "Caducat $1"],
  [new RegExp("^por (\\S{1,8})$"), "per $1"],
  [new RegExp("^merma (\\d+(?:[.,]\\d+)?)%$"), "minva $1%"],
  [new RegExp("^mín\\. (.+)$"), "mín. $1"],
  [new RegExp("^envase (.+)$"), "envàs $1"],
  [new RegExp("^coste 10 ([^:]+): (.+)$"), "cost 10 $1: $2"],
  [new RegExp("^repercutido (.+) − soportado (.+)$"), "repercutit $1 − suportat $2"],
  [new RegExp("^terceros sin NIF válido: (.+)$"), "tercers sense NIF vàlid: $1"],
  [new RegExp("^facturas sin documento adjunto: (.+)$"), "factures sense document adjunt: $1"],
  [new RegExp("^🔴 Huecos en la serie de facturas: (.+)$"), "🔴 Buits en la sèrie de factures: $1"],
  [new RegExp("^(\\d+) emitidas · (\\d+) recibidas$"), "$1 emeses · $2 rebudes"],
  [new RegExp("^(\\d+) excepciones$"), "$1 excepcions"],
  [new RegExp("^Paquete (\\S+) generado y registrado$"), "Paquet $1 generat i registrat"],
  [new RegExp("^incompleta — (.+)$"), "incompleta — $1"],
  [new RegExp("^Dirección: (.+)$"), "Adreça: $1"],
  [new RegExp("^Contacto: (.+) · sin email$"), "Contacte: $1 · sense correu"],
  [new RegExp("^Contacto: (.+)$"), "Contacte: $1"],
  [
    new RegExp("^Condiciones: (.+?) · (\\d+) días( · IRPF [\\d.,]+%)?$"),
    "Condicions: $1 · $2 dies$3",
  ],
  [new RegExp("^Origen: (.+) · Línea: (.+)$"), "Origen: $1 · Línia: $2"],
  [new RegExp("^· enviada por (.+)$"), "· enviada per $1"],
  [
    new RegExp("^\\((-?\\d+(?:[.,]\\d+)?)%\\) · (\\d+) líneas modificadas$"),
    "($1%) · $2 línies modificades",
  ],
  [new RegExp("^(\\S+) · v(\\d+) \\(aceptada\\)$"), "$1 · v$2 (acceptada)"],
  [new RegExp("^(\\d+)\\. (.+) \\(opcional — aparte\\)$"), "$1. $2 (opcional — a part)"],
  [new RegExp("^(\\d+)\\. (.+) \\(fuera de presupuesto\\)$"), "$1. $2 (fora de pressupost)"],
  [new RegExp("^PRESUPUESTO (\\S+) · v(\\d+)$"), "PRESSUPOST $1 · v$2"],
  [new RegExp("^IVA (\\d+(?:[.,]\\d+)?)%$"), "IVA $1%"],
  [
    new RegExp("^Validez: (\\d{2}\\/\\d{2}\\/\\d{4}) · (.+) · IBAN (.+)$"),
    "Validesa: $1 · $2 · IBAN $3",
  ],
  [new RegExp("^(\\d+) (.+?): venta (.+?) · coste (.+?) ·$"), "$1 $2: venda $3 · cost $4 ·"],
  [new RegExp("^base (.+) \\+ extras (.+)$"), "base $1 + extres $2"],
  [new RegExp("^(-?\\d+(?:[.,]\\d+)?)% · real (.+) de (.+)$"), "$1% · real $2 de $3"],
  [new RegExp("^Avance: (\\d+)%$"), "Avanç: $1%"],
  [new RegExp("^(\\d+) aprobados · (.+) sin aprobar$"), "$1 aprovats · $2 sense aprovar"],
  [new RegExp("^(.+) — venta (.+) · coste (.+)$"), "$1 — venda $2 · cost $3"],
  [new RegExp("^Tarifa · (.+)$"), "Tarifa · $1"],
  [new RegExp("^Oferta · (.+)$"), "Oferta · $1"],
  [new RegExp("^Albarán valorado · (.+)$"), "Albarà valorat · $1"],
  [new RegExp("^Factura · (.+)$"), "Factura · $1"],
  [new RegExp("^Confirmación · (.+)$"), "Confirmació · $1"],
  [new RegExp("^(\\d+) oportunidades abiertas$"), "$1 oportunitats obertes"],
  [new RegExp("^PASO (\\d+)$"), "PAS $1"],
  [new RegExp("^([\\d.,]+\\s?€) pendiente de cobro$"), "$1 pendent de cobrament"],
  [new RegExp("^([\\d.,]+\\s?€) · (\\d+) día\\(s\\) de retraso$"), "$1 · $2 dia(es) de retard"],
  [new RegExp("^([\\d.,]+\\s?€) entradas · ([\\d.,]+\\s?€) salidas$"), "$1 entrades · $2 sortides"],
  [new RegExp("^([\\d.,]+)% de los ingresos$"), "$1% dels ingressos"],
  [new RegExp("^(\\d+)d vencida$"), "$1d vençuda"],
  [new RegExp("^vence en (\\d+)d$"), "venç en $1d"],
  [new RegExp("^(\\d+)d de antigüedad$"), "$1d d'antiguitat"],
  [new RegExp("^([\\d.,]+\\s?€) \\(estim\\.\\)$"), "$1 (estim.)"],
  [
    new RegExp("^([\\d.,]+)% \\(por debajo del mínimo del 12%\\)$"),
    "$1% (per sota del mínim del 12%)",
  ],
  [new RegExp("^([\\d.,]+)% del coste previsto comprometido$"), "$1% del cost previst compromès"],
  [new RegExp("^enviado hace (\\d+)d, sin aceptación$"), "enviat fa $1d, sense acceptació"],
  [new RegExp("^Factura vencida — (.+)$"), "Factura vençuda — $1"],
  [new RegExp("^Margen ajustado — (.+)$"), "Marge ajustat — $1"],
  [
    new RegExp("^Comprometido por encima del coste previsto — (.+)$"),
    "Compromès per sobre del cost previst — $1",
  ],
  [
    new RegExp("^Presupuesto a la espera de respuesta — (.+)$"),
    "Pressupost a l'espera de resposta — $1",
  ],
  [new RegExp("^✓ Enviado (.+)$"), "✓ Enviat $1"],
  [new RegExp("^(.+) · Enviado «(.+)» → (.+)\\.$"), "$1 · Enviat «$2» → $3."],
  [
    new RegExp(
      "^(.+) · Borrador de Outlook descargado — «(.+)» \\(se abre editable en Outlook, no se envía\\)\\.$",
    ),
    "$1 · Esborrany d'Outlook descarregat — «$2» (s'obre editable a l'Outlook, no s'envia).",
  ],
  [new RegExp("^(W\\d+) entradas: ([\\d.,]+\\s?€)$"), "$1 entrades: $2"],
  [new RegExp("^(W\\d+) salidas: ([\\d.,]+\\s?€)$"), "$1 sortides: $2"],
  [new RegExp("^Ingresos ([\\d.,]+)k€$"), "Ingressos $1k€"],
  [new RegExp("^Margen ([\\d.,]+)k€$"), "Marge $1k€"],
  [new RegExp("^No vencido: ([\\d.,]+\\s?€)$"), "No vençut: $1"],
  [new RegExp("^60d\\+ \\/ vencido: ([\\d.,]+\\s?€)$"), "60d+ / vençut: $1"],
  [new RegExp("^Pedido de compra — (.+) \\((P-\\d+)\\)$"), "Comanda de compra — $1 ($2)"],
  [new RegExp("^Su presupuesto de reforma — (.+)$"), "El seu pressupost de reforma — $1"],
  [
    new RegExp("^Gracias — su proyecto está confirmado \\((P-\\d+)\\)$"),
    "Gràcies — el seu projecte està confirmat ($1)",
  ],
  [new RegExp("^(.+) · escrito a mano$"), "$1 · escrit a mà"],
  [new RegExp("^(.+) · tomada del catálogo$"), "$1 · agafada del catàleg"],
  [new RegExp("^Factura (FAC-\\d{4}-\\d{4}) — (.+)$"), "Factura $1 — $2"],
  [new RegExp("^Recordatorio amable — factura de (.+)$"), "Recordatori amable — factura de $1"],
  [new RegExp("^Contacto — (\\d+) proyecto\\(s\\)$"), "Contacte — $1 projecte(s)"],
  [new RegExp("^Visita a obra — (\\d+) proyecto\\(s\\)$"), "Visita a obra — $1 projecte(s)"],
  [new RegExp("^Estimación — (\\d+) proyecto\\(s\\)$"), "Estimació — $1 projecte(s)"],
  [new RegExp("^Presupuesto — (\\d+) proyecto\\(s\\)$"), "Pressupost — $1 projecte(s)"],
  [new RegExp("^Aceptación — (\\d+) proyecto\\(s\\)$"), "Acceptació — $1 projecte(s)"],
  [new RegExp("^Proyecto — (\\d+) proyecto\\(s\\)$"), "Projecte — $1 projecte(s)"],
  [new RegExp("^Compras — (\\d+) proyecto\\(s\\)$"), "Compres — $1 projecte(s)"],
  [new RegExp("^Ejecución — (\\d+) proyecto\\(s\\)$"), "Execució — $1 projecte(s)"],
  [new RegExp("^Facturación — (\\d+) proyecto\\(s\\)$"), "Facturació — $1 projecte(s)"],
  [new RegExp("^Cobros — (\\d+) proyecto\\(s\\)$"), "Cobraments — $1 projecte(s)"],
  [
    new RegExp("^Pagos a proveedores — (\\d+) proyecto\\(s\\)$"),
    "Pagaments a proveïdors — $1 projecte(s)",
  ],
  [
    new RegExp("^Cierre · beneficio — (\\d+) proyecto\\(s\\)$"),
    "Tancament · benefici — $1 projecte(s)",
  ],
  [
    new RegExp("^Solicitud de reseña — (\\d+) proyecto\\(s\\)$"),
    "Sol·licitud de ressenya — $1 projecte(s)",
  ],
  [new RegExp("^(.+) · Contacto · (\\S+@\\S+)$"), "$1 · Contacte · $2"],
  [new RegExp("^(.+) · Visita a obra · (\\S+@\\S+)$"), "$1 · Visita a obra · $2"],
  [new RegExp("^(.+) · Estimación · (\\S+@\\S+)$"), "$1 · Estimació · $2"],
  [new RegExp("^(.+) · Presupuesto · (\\S+@\\S+)$"), "$1 · Pressupost · $2"],
  [new RegExp("^(.+) · Aceptación · (\\S+@\\S+)$"), "$1 · Acceptació · $2"],
  [new RegExp("^(.+) · Proyecto · (\\S+@\\S+)$"), "$1 · Projecte · $2"],
  [new RegExp("^(.+) · Compras · (\\S+@\\S+)$"), "$1 · Compres · $2"],
  [new RegExp("^(.+) · Ejecución · (\\S+@\\S+)$"), "$1 · Execució · $2"],
  [new RegExp("^(.+) · Facturación · (\\S+@\\S+)$"), "$1 · Facturació · $2"],
  [new RegExp("^(.+) · Cobros · (\\S+@\\S+)$"), "$1 · Cobraments · $2"],
  [new RegExp("^(.+) · Pagos a proveedores · (\\S+@\\S+)$"), "$1 · Pagaments a proveïdors · $2"],
  [new RegExp("^(.+) · Cierre · beneficio · (\\S+@\\S+)$"), "$1 · Tancament · benefici · $2"],
  [new RegExp("^(.+) · Solicitud de reseña · (\\S+@\\S+)$"), "$1 · Sol·licitud de ressenya · $2"],
  [new RegExp("^(\\d+) de (\\d+)$"), "$1 de $2"],
  [new RegExp("^(\\d+) registros$"), "$1 registres"],
  [new RegExp("^(\\d+) registro$"), "$1 registre"],
  [new RegExp("^Obligatorio: (.+)$"), "Obligatori: $1"],
  [
    new RegExp('^Otro (.+) ya usa ([A-Z]+) "(.+)"\\. ¿Guardar un duplicado igualmente\\?$'),
    'Un altre $1 ja fa servir $2 "$3". Voleu desar un duplicat igualment?',
  ],
  [new RegExp("^Editar (.+)$"), "Editar $1"],
  [new RegExp("^Nuevo (.+)$"), "Nou $1"],
  [new RegExp("^Nueva (.+)$"), "Nova $1"],
  [
    new RegExp(
      "^Panel en vivo para (.+)\\. Todo se calcula a partir de los libros — edita una entrada y todas las cifras se actualizan\\.$",
    ),
    "Panell en viu per a $1. Tot es calcula a partir dels llibres — edita una entrada i totes les xifres s'actualitzen.",
  ],
  [new RegExp("^≈ (-?[\\d.,]+\\s?€) anualizado$"), "≈ $1 anualitzat"],
  [new RegExp("^(-?[\\d.,]+\\s?€) de beneficio bruto$"), "$1 de benefici brut"],
  [new RegExp("^(-?[\\d.,]+%|—) de los ingresos$"), "$1 dels ingressos"],
  [new RegExp("^(-?[\\d.,]+%|—) de margen neto$"), "$1 de marge net"],
  [new RegExp("^Caja neta (-?[\\d.,]+\\s?€)$"), "Caixa neta $1"],
  [new RegExp("^Deuda neta (-?[\\d.,]+\\s?€)$"), "Deute net $1"],
  [new RegExp("^Prueba ácida (-?[\\d.,]+) · FM (-?[\\d.,]+\\s?€)$"), "Prova àcida $1 · FM $2"],
  [new RegExp("^(-?[\\d.,]+\\s?€) de pipeline abierto$"), "$1 de cartera oberta"],
  [new RegExp("^descuadre de (-?[\\d.,]+\\s?€)$"), "descompensació de $1"],
  [new RegExp("^(.+) · Real vs presupuesto$"), "$1 · Real vs pressupost"],
  [
    new RegExp(
      "^Margen bruto (-?[\\d.,]+%|—) · Margen EBITDA (-?[\\d.,]+%|—) · Margen neto (-?[\\d.,]+%|—)\\. El presupuesto es el plan anual distribuido uniformemente en (\\d+) meses\\.$",
    ),
    "Marge brut $1 · Marge EBITDA $2 · Marge net $3. El pressupost és el pla anual distribuït uniformement en $4 mesos.",
  ],
  [
    new RegExp(
      "^Margen bruto (-?[\\d.,]+%|—) · Margen EBITDA (-?[\\d.,]+%|—) · Margen neto (-?[\\d.,]+%|—)\\. El presupuesto es el plan anual distribuido uniformemente en 1 mes\\.$",
    ),
    "Marge brut $1 · Marge EBITDA $2 · Marge net $3. El pressupost és el pla anual distribuït uniformement en 1 mes.",
  ],
  [new RegExp("^⚠ Descuadre de (-?[\\d.,]+\\s?€)\\.$"), "⚠ Descompensació de $1."],
  [
    new RegExp("^⚠ Diferencia de conciliación de (-?[\\d.,]+\\s?€)\\.$"),
    "⚠ Diferència de conciliació de $1.",
  ],
  [
    new RegExp(
      "^La antigüedad se calcula en vivo a partir de las partidas abiertas a (\\d{2}\\/\\d{2}\\/\\d{4}) — los tramos de >60 días son sobre los que actúa la automatización de cobros y pagos a proveedores\\.$",
    ),
    "L'antiguitat es calcula en viu a partir de les partides obertes a $1 — els trams de >60 dies són sobre els quals actua l'automatització de cobraments i pagaments a proveïdors.",
  ],
  [new RegExp("^(-?[\\d.,]+\\s?€) pendiente$"), "$1 pendent"],
  [new RegExp("^(\\d+) filas$"), "$1 files"],
  [new RegExp("^Nuevo — (.+)$"), "Nou — $1"],
  [new RegExp("^Paso (\\d+) de (\\d+)$"), "Pas $1 de $2"],
  [new RegExp("^(📂|📁|→) 01 Captación$"), "$1 01 Captació"],
  [new RegExp("^(📂|📁|→) 02 Visita de obra$"), "$1 02 Visita d'obra"],
  [new RegExp("^(📂|📁|→) 03 Estimación$"), "$1 03 Estimació"],
  [new RegExp("^(📂|📁|→) 04 Presupuesto$"), "$1 04 Pressupost"],
  [new RegExp("^(📂|📁|→) 05 Aceptación$"), "$1 05 Acceptació"],
  [new RegExp("^(📂|📁|→) 06 Proyecto$"), "$1 06 Projecte"],
  [new RegExp("^(📂|📁|→) 07 Compras$"), "$1 07 Compres"],
  [new RegExp("^(📂|📁|→) 08 Ejecución$"), "$1 08 Execució"],
  [new RegExp("^(📂|📁|→) 09 Facturación$"), "$1 09 Facturació"],
  [new RegExp("^(📂|📁|→) 10 Cobros$"), "$1 10 Cobraments"],
  [new RegExp("^(📂|📁|→) 11 Pagos a proveedores$"), "$1 11 Pagaments a proveïdors"],
  [new RegExp("^(📂|📁|→) 12 Cierre$"), "$1 12 Tancament"],
  [new RegExp("^(📂|📁|→) 13 Solicitud de reseña$"), "$1 13 Sol·licitud de ressenya"],
  [new RegExp("^“(.+)” · fase$"), "“$1” · fase"],
  [
    new RegExp("^(.+)–(.+) \\(IVA no incl\\.\\), 3–4 semanas\\.$"),
    "$1–$2 (IVA no incl.), 3–4 setmanes.",
  ],
  [
    new RegExp(
      "^Un PDF de presupuesto con imagen corporativa completa \\((.+), válido 30 días, partes y condiciones\\) se archiva en “04 Presupuesto”\\.$",
    ),
    "Un PDF de pressupost amb imatge corporativa completa ($1, vàlid 30 dies, parts i condicions) s'arxiva a “04 Pressupost”.",
  ],
  [new RegExp("^◇ Opcional — (.+)$"), "◇ Opcional — $1"],
  [new RegExp("^Opcional — (.+)$"), "Opcional — $1"],
  [
    new RegExp(
      "^e incluye los trabajos opcionales \\((.+)\\)\\. La versión aceptada queda congelada; se redacta automáticamente un correo de aceptación con imagen corporativa y el presupuesto aceptado adjunto\\.$",
    ),
    "i inclou els treballs opcionals ($1). La versió acceptada queda congelada; es redacta automàticament un correu d'acceptació amb imatge corporativa i el pressupost acceptat adjunt.",
  ],
  [
    new RegExp("^base \\+ opcional = (.+) \\(IVA no incl\\.\\)\\. Versión congelada\\.$"),
    "base + opcional = $1 (IVA no incl.). Versió congelada.",
  ],
  [
    new RegExp("^(.+) \\(IVA no incl\\.\\)\\. Versión congelada\\.$"),
    "$1 (IVA no incl.). Versió congelada.",
  ],
  [
    new RegExp("^línea base (.+) en (\\d+) partidas\\. Estado: activo\\.$"),
    "línia base $1 en $2 partides. Estat: actiu.",
  ],
  [
    new RegExp("^(.+) recibido y asignado a (\\S+)\\. Pendiente: (.+)\\.$"),
    "$1 rebut i assignat a $2. Pendent: $3.",
  ],
  [
    new RegExp("^(.+) en (\\d+) facturas de proveedor\\. Posición de caja:$"),
    "$1 en $2 factures de proveïdor. Posició de caixa:",
  ],
  [
    new RegExp(
      "^Margen (.+) \\((.+)%\\)\\. Posición de caja (.+)\\. Cada euro trazado desde la primera consulta hasta el beneficio final\\.$",
    ),
    "Marge $1 ($2%). Posició de caixa $3. Cada euro traçat des de la primera consulta fins al benefici final.",
  ],
  [
    new RegExp(
      "^para (.+)\\. Enlace de reseña de Google incluido · archivado en “13 Solicitud de reseña”\\.$",
    ),
    "per a $1. Enllaç de ressenya de Google inclòs · arxivat a “13 Sol·licitud de ressenya”.",
  ],
  [
    new RegExp("^Fecha de emisión · vencimiento \\((\\d+)d\\)$"),
    "Data d'emissió · venciment ($1d)",
  ],
  [
    new RegExp("^Correo redactado automáticamente · CI completa · para (.+)$"),
    "Correu redactat automàticament · CI completa · per a $1",
  ],
  [
    new RegExp("^(.+) – (.+) \\(IVA no incl\\., incl\\. ([\\d.,]+)% de imprevistos\\) ·$"),
    "$1 – $2 (IVA no incl., incl. $3% d'imprevistos) ·",
  ],
  [
    new RegExp("^(.+) \\(IVA no incl\\.\\) · (\\d+) línea\\(s\\) ·$"),
    "$1 (IVA no incl.) · $2 línia(es) ·",
  ],
  [new RegExp("^Presupuesto — (\\d+) líneas detalladas$"), "Pressupost — $1 línies detallades"],
  [new RegExp("^Proyecto cargado — (.+)\\.$"), "Projecte carregat — $1."],
  [new RegExp("^CRM · lead abierto para (.+)$"), "CRM · lead obert per a $1"],
  [new RegExp("^Estimación compartida: ~(.+)$"), "Estimació compartida: ~$1"],
  [new RegExp("^Presupuesto (\\S+) creado: base (.+)$"), "Pressupost $1 creat: base $2"],
  [
    new RegExp("^Proyecto creado — línea base (.+) congelada$"),
    "Projecte creat — línia base $1 congelada",
  ],
  [new RegExp("^Pedidos emitidos — comprometido (.+)$"), "Comandes emeses — compromès $1"],
  [
    new RegExp("^Facturas contabilizadas — real (.+) \\((\\d+) facturas\\)$"),
    "Factures comptabilitzades — real $1 ($2 factures)",
  ],
  [new RegExp("^Factura (\\S+) emitida — ingresos (.+)$"), "Factura $1 emesa — ingressos $2"],
  [new RegExp("^Cobro asignado — cobrado (.+)$"), "Cobrament assignat — cobrat $1"],
  [new RegExp("^Proveedores pagados — (.+)$"), "Proveïdors pagats — $1"],
  [new RegExp("^Proyecto cerrado — margen (.+) \\((.+)%\\)$"), "Projecte tancat — marge $1 ($2%)"],
  [
    new RegExp(
      "^Carpeta del proyecto descargada — (\\d+) archivo\\(s\\) en (\\d+) subcarpetas\\.$",
    ),
    "Carpeta del projecte descarregada — $1 fitxer(s) en $2 subcarpetes.",
  ],
  [new RegExp("^(\\d+) archivo\\(s\\) añadido\\(s\\) a (.+)$"), "$1 fitxer(s) afegit(s) a $2"],
  [new RegExp("^Correo enviado → (.+)$"), "Correu enviat → $1"],
  [new RegExp("^Borrador de Outlook descargado — (\\w+)$"), "Esborrany d'Outlook descarregat — $1"],
  [new RegExp("^1 archivo$"), "1 fitxer"],
  [new RegExp("^(\\d+) archivos$"), "$1 fitxers"],
  [
    new RegExp("^Estimación aplicada al presupuesto — (.+) en (\\d+) partida\\(s\\)\\.$"),
    "Estimació aplicada al pressupost — $1 en $2 partida(s).",
  ],
  [
    new RegExp("^Presupuesto actualizado — (\\d+) partida\\(s\\), (\\d+) línea\\(s\\), base (.+)$"),
    "Pressupost actualitzat — $1 partida(s), $2 línia(es), base $3",
  ],
  [new RegExp("^Proveedor (\\d+)$"), "Proveïdor $1"],
  [new RegExp("^Estimado\\/a (.+):$"), "Benvolgut/da $1:"],
  [
    new RegExp("^Nos complace confirmar la aceptación de los trabajos de su reforma en (.+)\\.$"),
    "Ens plau confirmar l'acceptació dels treballs de la seva reforma a $1.",
  ],
  [new RegExp("^Factura (\\S+) — Canei Subirats$"), "Factura $1 — Canei Subirats"],
  [new RegExp("^Factura ([A-Z0-9-]+)$"), "Factura $1"],
  [
    new RegExp("^\\(base (.+) \\+ IVA (.+)\\)\\. Forma de pago: transferencia a (\\d+) días\\.$"),
    "(base $1 + IVA $2). Forma de pagament: transferència a $3 dies.",
  ],
  [new RegExp("^Recordatorio — factura (\\S+)$"), "Recordatori — factura $1"],
  [
    new RegExp(
      "^Le recordamos amablemente que la factura (\\S+) \\((.+)\\) está pendiente de pago\\.$",
    ),
    "Li recordem amablement que la factura $1 ($2) està pendent de pagament.",
  ],
  [
    new RegExp("^Forma de pago: transferencia · (\\d+) días\\.$"),
    "Forma de pagament: transferència · $1 dies.",
  ],
  [new RegExp("^IBAN: (.+) · Referencia: (\\S+)$"), "IBAN: $1 · Referència: $2"],
  [new RegExp("^Retencion IRPF -([\\d.,]+)%$"), "Retenció IRPF -$1%"],
  [new RegExp("^Opcional \\(aparte\\) — (.+)$"), "Opcional (a part) — $1"],
  [new RegExp("^(\\d+) órdenes$"), "$1 comandes"],
  [new RegExp("^(\\d+) orden$"), "$1 comanda"],
  [new RegExp("^(\\d+) facturas$"), "$1 factures"],
  [new RegExp("^(\\d+) factura$"), "$1 factura"],
  [new RegExp("^Repartir · (.+)$"), "Repartir · $1"],
  [new RegExp("^Cobros previstos · (.+)$"), "Cobraments previstos · $1"],
  [new RegExp("^(\\d+) industriales$"), "$1 industrials"],
  [new RegExp("^(\\d+) industrial$"), "$1 industrial"],
  [new RegExp("^Fecha (\\S+) · vigencia (.+)$"), "Data $1 · vigència $2"],
  [new RegExp("^vigencia sin límite$"), "sense límit"],
  [new RegExp("^Lista ([\\d.,]+ €) · descuento ([\\d.,]+)% →$"), "Llista $1 · descompte $2% →"],
  [new RegExp("^neto ([\\d.,]+ €)$"), "net $1"],
  [new RegExp("^(\\d+) registrados$"), "$1 registrats"],
  [new RegExp("^(\\d+) registrado$"), "$1 registrat"],
  [new RegExp("^(\\d+) trabajadores$"), "$1 treballadors"],
  [new RegExp("^(\\d+) trabajador$"), "$1 treballador"],
  [new RegExp("^([\\d.,]+ €)/hora vigente$"), "$1/hora vigent"],
  [new RegExp("^desde (\\S+) · ([\\d.,]+ €)/hora$"), "des de $1 · $2/hora"],
  [new RegExp("^(\\d+) · caducado$"), "$1 · caducat"],
  [new RegExp("^Reconocimiento médico — vence (\\S+)$"), "Reconeixement mèdic — venç $1"],
  [new RegExp("^Retirar «(.+)»$"), "Retirar «$1»"],
  [
    new RegExp(
      "^Se usa en (\\d+) registros\\. Retirarla la quita de los desplegables; los registros que ya la llevan la conservan y se siguen viendo con su nombre\\.$",
    ),
    "S'usa en $1 registres. Retirar-la la treu dels desplegables; els registres que ja la porten la conserven i se segueixen veient amb el seu nom.",
  ],
  [
    new RegExp(
      "^Se usa en (\\d+) registro\\. Retirarla la quita de los desplegables; los registros que ya la llevan la conservan y se siguen viendo con su nombre\\.$",
    ),
    "S'usa en $1 registre. Retirar-la la treu dels desplegables; els registres que ja la porten la conserven i se segueixen veient amb el seu nom.",
  ],
  [new RegExp("^Fin previsto (\\S+) → proyectado$"), "Fi previst $1 → projectat"],
  [
    new RegExp("^Proyectado al ritmo observado \\(×([\\d.,]+)\\)$"),
    "Projectat al ritme observat (×$1)",
  ],
  [
    new RegExp("^planificado ([\\d.,]+)% · real ([\\d.,]+)% · ([+-][\\d.,]+) pts$"),
    "planificat $1% · real $2% · $3 pts",
  ],
  [new RegExp("^Semana laboral: (\\d+) días$"), "Setmana laboral: $1 dies"],
  [new RegExp("^(\\d+) justificadas$"), "$1 justificades"],
  [new RegExp("^(\\d+) justificada$"), "$1 justificada"],
  [new RegExp("^(\\d+) incidencia$"), "$1 incidència"],
  [new RegExp("^(\\d+) incidencias$"), "$1 incidències"],
  [new RegExp("^(\\d+) pendientes de (\\d+)$"), "$1 pendents de $2"],
  [new RegExp("^(\\d+) pendiente de (\\d+)$"), "$1 pendent de $2"],
  [new RegExp("^(\\d+)/(\\d+) bloques en verde$"), "$1/$2 blocs en verd"],
  [new RegExp("^Nada bloquea el envío del (\\S+)\\.$"), "Res no bloqueja l'enviament del $1."],
  [new RegExp("^(\\d+) para (.+)$"), "$1 per a $2"],
  [
    new RegExp("^(\\d+) traspaso\\(s\\) interno\\(s\\) detectado\\(s\\)$"),
    "$1 traspàs(os) intern(s) detectat(s)",
  ],
  [new RegExp("^(\\d+) h · (\\d+) líneas$"), "$1 h · $2 línies"],
  [new RegExp("^(\\d+) h · (\\d+) línea$"), "$1 h · $2 línia"],
  [new RegExp("^(\\d+) proyectos$"), "$1 projectes"],
  [new RegExp("^(\\d+) proyecto$"), "$1 projecte"],
  [new RegExp("^Parte del (\\S+)$"), "Part del $1"],
  [new RegExp("^(\\d+) en el anexo gráfico$"), "$1 a l'annex gràfic"],
  [
    new RegExp("^Margen ([\\d.,]+ €) · ([\\d.,]+)% — no sale del documento$"),
    "Marge $1 · $2% — no surt del document",
  ],
  [new RegExp("^IRPF: aplica ([\\d.,]+)%$"), "IRPF: s'aplica $1%"],
  [new RegExp("^Email: (.+)$"), "Correu: $1"],
  [new RegExp("^enero (\\d{4})$"), "gener $1"],
  [new RegExp("^febrero (\\d{4})$"), "febrer $1"],
  [new RegExp("^marzo (\\d{4})$"), "març $1"],
  [new RegExp("^abril (\\d{4})$"), "abril $1"],
  [new RegExp("^mayo (\\d{4})$"), "maig $1"],
  [new RegExp("^junio (\\d{4})$"), "juny $1"],
  [new RegExp("^julio (\\d{4})$"), "juliol $1"],
  [new RegExp("^agosto (\\d{4})$"), "agost $1"],
  [new RegExp("^septiembre (\\d{4})$"), "setembre $1"],
  [new RegExp("^octubre (\\d{4})$"), "octubre $1"],
  [new RegExp("^noviembre (\\d{4})$"), "novembre $1"],
  [new RegExp("^diciembre (\\d{4})$"), "desembre $1"],
  [new RegExp("^outstanding ([\\d.,]+ €)$"), "pendent $1"],
  [new RegExp("^v([\\d.]+) · (\\d+) versiones$"), "v$1 · $2 versions"],
  [new RegExp("^v([\\d.]+) · 1 versión$"), "v$1 · 1 versió"],
  [new RegExp("^(\\d+) documentos$"), "$1 documents"],
  [new RegExp("^(\\d+) documento$"), "$1 document"],
  [new RegExp("^Caducado (\\S+)$"), "Caducat $1"],
  [new RegExp("^Albarán valorado · Albarán (\\S+)$"), "Albarà valorat · Albarà $1"],
  [new RegExp("^(\\d+) partidas$"), "$1 partides"],
  [new RegExp("^(\\d+) partida$"), "$1 partida"],
  [new RegExp("^(\\d+) subpartidas$"), "$1 subpartides"],
  [new RegExp("^(\\d+) precios$"), "$1 preus"],
  [new RegExp("^(\\d+) activas de (\\d+)$"), "$1 actives de $2"],
  [new RegExp("^(\\d+) por encolar$"), "$1 per encuar"],
  [new RegExp("^(\\d+) de (\\d+) registros$"), "$1 de $2 registres"],
  [
    new RegExp("^(\\d+) de (\\d+) registros · (\\d+) fuera del periodo$"),
    "$1 de $2 registres · $3 fora del període",
  ],
  [new RegExp("^(\\d+) imágenes en (\\d+) página\\(s\\)$"), "$1 imatges en $2 pàgina(es)"],
  [
    new RegExp("^(\\d+) facturas? de proveedor sin asignar$"),
    "$1 factures de proveïdor sense assignar",
  ],
  [
    new RegExp("^(\\d+) movimientos bancarios sin asignar$"),
    "$1 moviments bancaris sense assignar",
  ],
  [
    new RegExp("^(\\d+) jornada\\(s\\) sin registrar — (.+)$"),
    "$1 jornada(es) sense registrar — $2",
  ],
  [
    new RegExp("^Partida (\\d+) por encima de coste previsto — (.+)$"),
    "Partida $1 per sobre del cost previst — $2",
  ],
  [new RegExp("^Documentación caducada — (.+)$"), "Documentació caducada — $1"],
  [new RegExp("^Exclusiones y supuestos \\((\\d+)\\)$"), "Exclusions i supòsits ($1)"],
  [
    new RegExp("^Hito de cobro (.+) · Hito de cobro (.+)$"),
    "Fita de cobrament $1 · Fita de cobrament $2",
  ],
  [new RegExp("^Hito de cobro (.+) · Fin previsto (.+)$"), "Fita de cobrament $1 · Fi previst $2"],
  [new RegExp("^Oportunidad sin avance (\\d+) días$"), "Oportunitat sense avanç $1 dies"],
  [new RegExp("^Periodo · (\\d+)$"), "Període · $1"],
  [new RegExp("^Presupuesto (\\S+) caduca el (\\S+)$"), "Pressupost $1 caduca el $2"],
  [
    new RegExp("^✓ Realizada (\\S+) · (\\d+) mediciónes · (\\d+) 📷$"),
    "✓ Realitzada $1 · $2 mesuraments · $3 📷",
  ],
  [
    new RegExp("^✓ Realizada (\\S+) · (\\d+) medición · (\\d+) 📷$"),
    "✓ Realitzada $1 · $2 mesurament · $3 📷",
  ],
  [new RegExp("^(\\d+) críticas · (\\d+) altas$"), "$1 crítiques · $2 altes"],
  [new RegExp("^(\\d+) crítica · (\\d+) altas$"), "$1 crítica · $2 altes"],
  [new RegExp("^(\\d+) visitas programadas$"), "$1 visites programades"],
  [new RegExp("^(\\d+) visita programada$"), "$1 visita programada"],
  [new RegExp("^(\\d+) visitas realizadas$"), "$1 visites realitzades"],
  [new RegExp("^(\\d+) visita realizada$"), "$1 visita realitzada"],
  [new RegExp("^(\\d+) obras$"), "$1 obres"],
  [new RegExp("^(\\d+) obra$"), "$1 obra"],
  [new RegExp("^(\\d+) contratos$"), "$1 contractes"],
  [new RegExp("^(\\d+) contrato$"), "$1 contracte"],
  [new RegExp("^Calculado a las (.+?) ·$"), "Calculat a les $1 ·"],
  [new RegExp("^Calculado a las (.+)$"), "Calculat a les $1"],
  [new RegExp("^(\\d+) proyectos activos$"), "$1 projectes actius"],
  [new RegExp("^(\\d+) proyecto activo$"), "$1 projecte actiu"],
  [new RegExp("^Vencido (.+)$"), "Vençut $1"],
  [new RegExp("^Proyección 30 d (.+)$"), "Projecció 30 d $1"],
  [new RegExp("^Ver las (\\d+) alertas en la torre$"), "Veure les $1 alertes a la torre"],
  [new RegExp("^Ver las (\\d+)$"), "Veure les $1"],
  [new RegExp("^Periodo aplicado: (.+)$"), "Període aplicat: $1"],
  [
    new RegExp("^Factura (\\S+) vencida (\\d+) días \\((.+)\\)$"),
    "Factura $1 vençuda fa $2 dies ($3)",
  ],
  [
    new RegExp("^Factura (\\S+) vencida (\\d+) día \\((.+)\\)$"),
    "Factura $1 vençuda fa $2 dia ($3)",
  ],
  [new RegExp("^Hito de cobro vencido — (.+)$"), "Fita de cobrament vençuda — $1"],
  [
    new RegExp("^(\\d+)–(\\d+) de (\\d+) · página (\\d+) de (\\d+)$"),
    "$1–$2 de $3 · pàgina $4 de $5",
  ],
  [new RegExp("^(\\d+) de (\\d+) · página (\\d+) de (\\d+)$"), "$1 de $2 · pàgina $3 de $4"],
  [new RegExp("^Duración contractual excedida — (\\S+)$"), "Durada contractual excedida — $1"],
  [new RegExp("^Garantía próxima a vencer — (\\S+)$"), "Garantia a punt de vèncer — $1"],
  [
    new RegExp("^Subcontrata sin factura tras varios días — (\\S+)$"),
    "Subcontracta sense factura després de diversos dies — $1",
  ],
  [new RegExp("^(\\d+) facturas recibidas$"), "$1 factures rebudes"],
  [new RegExp("^(\\d+) recibos$"), "$1 rebuts"],
  [new RegExp("^(\\d+) presupuestos$"), "$1 pressupostos"],
  [new RegExp("^(\\d+) presupuesto$"), "$1 pressupost"],
  [new RegExp("^Fin previsto (.+) · Hito de cobro (.+)$"), "Fi previst $1 · Fita de cobrament $2"],
  [new RegExp("^Programar visita — (.+)$"), "Programar visita — $1"],
  [new RegExp("^Visita — (.+)$"), "Visita — $1"],
  // P1-P5 i18n backfill: dynamic strings that carry a variable.
  [new RegExp("^Programar visita de seguimiento — (.+)$"), "Programar visita de seguiment — $1"],
  [new RegExp("^Enviar el paquete (.+)$"), "Envia el paquet $1"],
  [new RegExp("^Reabrir (.+)$"), "Reobre $1"],
  [new RegExp("^¿Qué se pregunta sobre (.+)\\?$"), "Què es pregunta sobre $1?"],
  [new RegExp("^Ya hay (\\d+) visitas$"), "Ja hi ha $1 visites"],
  [new RegExp("^Ya hay (\\d+) visita$"), "Ja hi ha $1 visita"],
  [new RegExp("^(\\d+) subpartidas$"), "$1 subpartides"],
  [new RegExp("^(\\d+) subpartida$"), "$1 subpartida"],
  // PK2-B: the version-picker <option> text.
  [new RegExp("^(v[\\d.]+) · aceptada$"), "$1 · acceptada"],
  [new RegExp("^(v[\\d.]+) · emitida$"), "$1 · emesa"],
  [new RegExp("^(v[\\d.]+) · borrador$"), "$1 · esborrany"],
];

/**
 * EN → CA interpolation rules.
 *
 * Spanish is the hub for EXACT matches, and that is enough for them. It is
 * not enough for interpolated lines on a page authored in English: there is
 * no Spanish form to hub through, so `16 rows` and `Net debt 80.000 €` had
 * no route to Catalan at all and rendered in English on every financial
 * screen. Written against the English string, applied straight to Catalan.
 */
window.CANEI_DICT.rxEn2Ca = [
  [new RegExp("^(\\d+) rows$"), "$1 files"],
  [new RegExp("^(\\d+) row$"), "$1 fila"],
  [new RegExp("^(\\d+) records$"), "$1 registres"],
  [new RegExp("^(\\d+) record$"), "$1 registre"],
  [new RegExp("^([\\d.,]+ €) open$"), "$1 obert"],
  [new RegExp("^([\\d.,]+ €) open pipeline$"), "$1 de cartera oberta"],
  [new RegExp("^(-?[\\d.,]+ €) gross profit$"), "$1 de benefici brut"],
  [new RegExp("^([\\d.,]+)% net margin$"), "$1% de marge net"],
  [new RegExp("^([\\d.,]+)% of revenue$"), "$1% dels ingressos"],
  [new RegExp("^Net debt ([\\d.,]+ €)$"), "Deute net $1"],
  [new RegExp("^≈ ([\\d.,]+ €) annualised$"), "≈ $1 anualitzat"],
  [new RegExp("^⚠ Reconciliation gap ([\\d.,]+ €)\\.$"), "⚠ Diferència de conciliació $1."],
  [new RegExp("^Quick ([\\d.,]+) · WC ([\\d.,]+ €)$"), "Quick $1 · CC $2"],
  [new RegExp("^Quote — (\\d+) itemised lines$"), "Pressupost — $1 línies detallades"],
  [new RegExp("^Capital circulante — (.+)$"), "Capital circulant — $1"],
  [new RegExp("^Estados financieros — (.+)$"), "Estats financers — $1"],
  [new RegExp("^Libros — (.+)$"), "Llibres — $1"],
  [new RegExp("^Resumen — (.+)$"), "Resum — $1"],
  [
    new RegExp(
      "^Aging is computed live from open items as of (\\S+) — the >60-day buckets are what the collections and supplier-payment automation acts on\\.$",
    ),
    "L'antiguitat es calcula en directe a partir de les partides obertes a $1 — els trams de més de 60 dies són els que fan actuar l'automatisme de cobraments i de pagaments a proveïdors.",
  ],
  [
    new RegExp(
      "^Gross margin ([\\d.,]+)% · EBITDA margin ([\\d.,]+)% · Net margin ([\\d.,]+)%\\. Budget is the annual plan phased evenly across (\\d+) months\\.$",
    ),
    "Marge brut $1% · Marge EBITDA $2% · Marge net $3%. El pressupost és el pla anual repartit uniformement al llarg de $4 mesos.",
  ],
];
