...
echo "HEALTH=$($C exec -T app node -e 'fetch("http://127.0.0.1:3000/api/health").then(r=>r.text()).then(t=>console.log(t.replace(/\s+/g,"")))' 2>/dev/null | tail -1)"
echo "WORKSPACE=$($C exec -T app node -e 'fetch("http://127.0.0.1:3000/workspace/erp.html").then(r=>console.log(r.status))' 2>/dev/null | tail -1)"
echo "BYPASS=$($C exec -T db psql -U "${POSTGRES_USER:-canei}" -d "${POSTGRES_DB:-canei_erp}" -tAc "SELECT rolsuper OR rolbypassrls FROM pg_roles WHERE rolname='${APP_DB_USER:-canei_app}'" 2>/dev/null | tr -d ' ')"
echo "TENANTS=$($C exec -T db psql -U "${POSTGRES_USER:-canei}" -d "${POSTGRES_DB:-canei_erp}" -tAc "SELECT count(*) FROM erp_state" 2>/dev/null | tr -d ' ')"
