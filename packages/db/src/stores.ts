...
/**
 * Binary attachments — the site photographs.
 *
 * Deliberately not a `KeyValueStore`: the value is bytes with a content type,
 * and squeezing that through JSON would base64 it, costing a third more storage
 * and a decode on every read of something that is only ever handed straight to
 * an <img>.
 *
 * Writes are upserts keyed on (tenant, key), and the key is generated by the
 * client from a timestamp and randomness. That makes a second write of the same
 * key overwrite rather than duplicate, which is the right behaviour for a
 * retried upload and the only reason a flaky connection does not leave two
 * copies of one photograph.
 */
export class PrismaErpBlobStore {
  constructor(
    private readonly prisma: PrismaClient,
    private readonly tenantId: string,
  ) {}
  ...
  async get(key: string): Promise<StoredBlob | null> {
    const row = await withTenant(this.prisma, this.tenantId, (tx) =>
      tx.erpBlob.findUnique({
        where: { tenantId_key: { tenantId: this.tenantId, key } },
      }),
    );
    if (!row) return null;
    return { bytes: row.bytes, mime: row.mime, size: row.size };
  }

  async put(key: string, bytes: Uint8Array, mime: string, user = ""): Promise<void> {
    const data = {
      mime,
    ...
    await withTenant(this.prisma, this.tenantId, (tx) =>
      tx.erpBlob.upsert({
        where: { tenantId_key: { tenantId: this.tenantId, key } },
        create: { tenantId: this.tenantId, key, ...data },
        update: data,
      }),
    );
  }
  ...
  async delete(key: string): Promise<void> {
    await withTenant(this.prisma, this.tenantId, (tx) =>
      tx.erpBlob.deleteMany({ where: { tenantId: this.tenantId, key } }),
    );
  }
}
