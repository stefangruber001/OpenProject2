/**
 * MARKDOWN → A4 PDF, shared by the document binders.
 *
 * Extracted from scripts/market-pack.mjs when a second document needed the same
 * machinery. Two markdown renderers in one repository is how the next report
 * quietly acquires a different table style, so there is one — and the roadmap
 * pack's page count is the regression test for the move: it was 183 pages
 * before the extraction and must be 183 after.
 *
 * The workspace has no markdown library and adding one for a report binder
 * would put a dependency on the lockfile for the sake of two scripts; the
 * artefacts are model-written markdown — headings, paragraphs, lists, GFM
 * tables, bold, code, blockquotes — which this handles, and anything it does
 * not is left as text rather than dropped.
 */
...
/* One A4 page set, with the running footer the bound documents share. */
export async function renderPdf(pg, file, html, footer) {
  ...
  await pg.pdf({
    ...
    ...(footer
      ? {
          ...
          footerTemplate: `<div style="width:100%;font:7.5pt Georgia,serif;color:#6B6B6B;padding:0 15mm;display:flex;justify-content:space-between"><span>${footer}</span><span class="pageNumber"></span></div>`,
        }
      : {}),
...
export const pagesOf = (f) =>
  (/^Pages:\s*(\d+)/m.exec(spawnSync("pdfinfo", [f], { encoding: "utf8" }).stdout || "") || [
    ,
    "?",
  ])[1];

/* The document has to contain what it claims to: a heading that never reached
   the text layer is a section the reader cannot find and nobody would notice.
   Whitespace is collapsed on both sides first — `pdftotext` wraps a long
   heading across two lines, and a check that failed on that would be reporting
   the extractor's line length rather than a missing section. */
export function assertInText(pdf, titles) {
  const flat = (s) => String(s).replace(/\s+/g, " ").trim();
  const text = flat(spawnSync("pdftotext", [pdf, "-"], { encoding: "utf8" }).stdout || "");
  const lost = titles.filter((t) => !text.includes(flat(t)));
  if (lost.length) throw new Error("missing from the text layer: " + lost.join(", "));
}
